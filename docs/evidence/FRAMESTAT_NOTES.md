> Evidence note, moved verbatim from `jupiter_240k5_byte/FRAMESTAT_NOTES.md` on 2026-09-09.
...
This file is the handoff spec for the host workstream (`host/qpsk_tun.c`,
...
| 0x1C4  | R    | `framestat_stallcnt`| FREE-RUNNING 32-bit count of byte_rx STALL cycles (`byte_rx_valid && !byte_rx_ready`), wraps mod 2^32 -- the DIRECT witness of inter-transfer DMAC backpressure (`docs/evidence/FWD_SINGLES_ROOT_CAUSE.md`) |
...
(`SEGMENTED_LOOPBACK_DESIGN.md`, tag archive/pre-cleanup-2026-09-09): the host differences successive reads
...
signature to `floatgap_n3` (tag archive/pre-cleanup-2026-09-09; S1B does not cover it). **NEITHER IMAGE
