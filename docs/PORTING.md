...
| Flash tool | `two_jup/deploy_image.sh <ip> A\|B\|/path/to/BOOT.BIN` | yes |
...
| **BOOT.BIN image** | **in the repo**: `boot_known_good/CURRENT.txt` names the current A/B images (md5-checked by the flash tool) — see Step 0 | image, not board-specific |
...
- **Use the committed current images** (fastest, 2026-09-04): `boot_known_good/CURRENT.txt`
  lists the image for role A and role B with md5s; `two_jup/deploy_image.sh <ip> A` (or `B`)
  picks it and refuses to flash if the md5 does not match. Both roles are the same
  hardware; A/B only fixes which board is the forward-link receiver. (The historical
  default without a role, the **lean build** tree
  `jupiter_byte_lean_build/.../boot/BOOT.BIN`, exists only on a build host.)
...
```sh
./deploy_image.sh <A_ip> A      # backs up current /boot -> .pregeneric, flashes, reboots, waits
./deploy_image.sh <B_ip> B      # only after A is confirmed back up
```
