...
int main(int argc, char** argv){
    ...
    FILE* fps = openout("ps");   // clk,psOff,psDone,psSucc,psNewpk,taArmed,taOffValid,taSync (on change of any flag)
    FILE* fag = dumpsamp ? openout("agc") : nullptr;
    FILE* frr = dumpsamp ? openout("rrc") : nullptr;

    Vwrap_byte_taps_e5* t = new Vwrap_byte_taps_e5;
    ...
    while(clk < total){
        ...
        if(t->railEnb){
            { static unsigned lastps=0xFFFFFFFFu; unsigned ps=(t->psOff&0x3FFF)|(t->psDone<<14)|(t->psSucc<<15)|(t->psNewpk<<16)|(t->taArmed<<17)|(t->taOffValid<<18)|(t->taSync<<19);
              if(ps!=lastps){ fprintf(fps,"%ld,%u,%u,%u,%u,%u,%u,%u\n",clk,t->psOff&0x3FFF,(unsigned)t->psDone,(unsigned)t->psSucc,(unsigned)t->psNewpk,(unsigned)t->taArmed,(unsigned)t->taOffValid,(unsigned)t->taSync); lastps=ps; } }
    ...
    fclose(fpd); fclose(fpa); fclose(fco); fclose(fde); fclose(ffe); fclose(fmu); fclose(fps);
    if(fag) fclose(fag);
    if(frr) fclose(frr);
