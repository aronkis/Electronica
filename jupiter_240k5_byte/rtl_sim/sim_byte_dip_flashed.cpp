// sim_byte_dip.cpp -- STAGE-1 instrumentation driver (2026-08-13 staged task).
...
int main(int argc, char** argv){
    ...
    if(argc < 8){
        fprintf(stderr,"usage: sim_byte_dip iq nsamp vphase cadence rstcs_end skip out_prefix [dumptaps]\n");
        return 2;
    }
    ...
    Vwrap_byte_dip* t=new Vwrap_byte_dip;
    ...
    // ready-dip scheduler (2026-08-14 overnight): silicon replay was always
    // driven with byte_rx_ready=1 -- backpressure is the one input replay
    // never reproduced. QSIM_DIP_PERIOD (accepted words, 0=off),
    // QSIM_DIP_LEN (clk), QSIM_DIP_OFF (word offset of first dip).
    long dip_period=0, dip_len=0, dip_off=40, dip_cnt=0, dips_done=0;
    { const char* e;
      if((e=getenv("QSIM_DIP_PERIOD"))) dip_period=atol(e);
      if((e=getenv("QSIM_DIP_LEN")))    dip_len=atol(e);
      if((e=getenv("QSIM_DIP_OFF")))    dip_off=atol(e); }
    ...
    while(clk<total){
        ...
        if(dip_cnt>0){ dip_cnt--; t->byte_rx_ready=0; } else t->byte_rx_ready=1;
        tick();
        ...
        if(t->byte_rx_valid && t->byte_rx_ready){
            ...
            nrxw++; nwords++;
            if(dip_period>0 && (nrxw % dip_period)==(dip_off % dip_period) && dip_cnt==0){
                dip_cnt=dip_len; dips_done++; }
    ...
    fprintf(fo,"dips_done=%ld dip_period=%ld dip_len=%ld\n",dips_done,dip_period,dip_len);
