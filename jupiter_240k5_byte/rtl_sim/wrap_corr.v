// wrap_corr.v -- [sim] COMB32 SRO reproduction wrapper (2026-09-03).
...
module wrap_corr
  (input  wire clk, input wire reset,
   ...
   output wire [1:0] rhPhase,
   // ---- correlator dump taps (task 6): characterise the competing peak ----
   output wire signed [31:0] psCorr,   // Correlator_dataOut  (sfix32_En26)
   output wire signed [31:0] psThr,    // Correlator_threshold (sfix32_En28)
   output wire psThrEx,                // Relational_Operator_out1 (thresholdExceeded)
   output wire psCorrV);               // Correlator_validOut  // Rate_Handle mod-4 pop phase (HDL_Counter_out1)
  ...
  assign psCorr   = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Preamble_Detector.Correlator_dataOut;
  assign psThr    = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Preamble_Detector.Correlator_threshold;
  assign psThrEx  = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Preamble_Detector.Relational_Operator_out1;
  assign psCorrV  = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Preamble_Detector.Correlator_validOut;
endmodule
