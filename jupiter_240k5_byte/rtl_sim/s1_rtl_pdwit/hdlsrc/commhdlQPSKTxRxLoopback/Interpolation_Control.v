// -------------------------------------------------------------
// 
// File Name: hdl_prj_jupiter_composite/hdlsrc/commhdlQPSKTxRxLoopback/Interpolation_Control.v
...
// Source Path: commhdlQPSKTxRxLoopback/TxRxComposite/Receiver/QPSK Rx/Frequency and Time Synchronizer/Symbol Synchronizer/Interpolation 
// Contro
// Hierarchy Level: 5
...
module Interpolation_Control
  ...
  reg signed [10:0] mu_last_value;  // sfix11_En10
  reg  Underflow_last_value;
  reg signed [10:0] Delta_1;  // sfix11_En10
  ...
  always @(Delta, countReg, muReg, underflowReg) begin
    ...
    if (Delta > 11'sb00011111111) begin
      Delta_1 = 11'sb00011111111;
    end
    else if (Delta < 11'sb11100000001) begin
      Delta_1 = 11'sb11100000001;
    end
    and_temp = countReg & 11'sb01111111111;
    sub_cast = {and_temp[10], and_temp};
    sub_cast_0 = {Delta_1[10], Delta_1};
  ...
  always @(posedge clk or posedge reset)
    begin : out0_bypass_process
      if (reset == 1'b1) begin
        mu_last_value <= 11'sb00000000000;
      end
      else begin
        if (enb_1_2_0_gated) begin
          mu_last_value <= mu;
  ...
  assign mu = (stateControl_2 == 1'b0 ? mu_last_value :
              mu_1);

  always @(posedge clk or posedge reset)
    begin : out1_bypass_process
      if (reset == 1'b1) begin
        Underflow_last_value <= 1'b0;
      end
      else begin
        if (enb_1_2_0_gated) begin
          Underflow_last_value <= Underflow;
        end
      end
    end

  assign Underflow = (stateControl_2 == 1'b0 ? Underflow_last_value :
              Underflow_1);
