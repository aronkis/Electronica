// wrap_byte_sro3s.v -- [sim] Task 11 (RXFIX_R3S) SRO harness wrapper (2026-09-04).
//
// A SEPARATE FILE, not an edit of wrap_byte_sro.v: Task 7's harness and its banked
// baselines (b_p000 / b_m10 / b_m40 / tb_m10) must not move under Task 11.  The
// module NAME is deliberately still `wrap_byte_sro` so that sim_sro.cpp -- Task 7's
// driver, byte for byte -- links against it unmodified; that is what makes the
// n_p000 md5 identity gate a real test rather than a test of a re-typed driver.
//
// BECAUSE two files now declare `module wrap_byte_sro`, this one announces itself at
// time 0.  A run that does not print WRAP3S_FILE / WRAP3S_DEFINE compiled the WRONG
// wrapper, and that failure would otherwise masquerade as "R3S never armed" -- which
// is the n_p000 PASS condition.  build_sro_rxfix3s.sh greps the verilate log too.
//
// R3S witnesses are carried on Task 7's three existing witness ports so that the
// driver needs no change:
//   r3Skips  = r3s_skips  (count of steered pop SKIPS)
//   r3Extras = 32'hA5A50001 while r3s_armed, else 0.  It is a SENTINEL, NOT a count:
//              R3S has no extra-pop branch at all (that branch is what got R3
...
// the single arming instant.
//
// It also adds read-only per-stage taps (no effect on any port sim_sro.cpp drives)
// for the brief's falsifier dump; sim_stagewin.cpp is the only consumer.
//
// Original header of the file this was cut from:
...
module wrap_byte_sro
  ...
  // ---- T11: RXFIX_R3S witnesses, carried on T7's three witness ports ----
  // build_sro_rxfix3s.sh passes +define+RXFIX_R3S with the R3S tree on -y.
`ifdef RXFIX_R3S
  assign r3Skips  = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r3s_skips;
  // SENTINEL, not a count -- R3S has no extra-pop branch.  See the header.
  assign r3Extras = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r3s_armed
                    ? 32'hA5A50001 : 32'd0;
...
`endif
  ...
  // ---- T11: which wrapper actually compiled.  Two files declare this module. ----
  initial begin
    $display("WRAP3S_FILE wrap_byte_sro3s.v e7c1");
`ifdef RXFIX_R3S
    $display("WRAP3S_DEFINE RXFIX_R3S");
`else
    $display("WRAP3S_DEFINE none");
`endif
  end
endmodule
