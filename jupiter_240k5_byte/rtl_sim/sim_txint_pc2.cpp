...
#include "Vwrap_byte_ce_txint_pc2.h"
#include "Vwrap_byte_ce_txint_pc2___024root.h"
#include "verilated.h"
#include <cstdio>

static void tick(Vwrap_byte_ce_txint_pc2* t, long& clk) {
    t->clk = 0; t->eval(); t->clk = 1; t->eval(); clk++;
}
static void driveIdle(Vwrap_byte_ce_txint_pc2* t) {
...
int main() {
    ...
    for (int pass = 0; pass < 2; pass++) {
        for (int i = 0; i < 5; i++) {
            Vwrap_byte_ce_txint_pc2* t = new Vwrap_byte_ce_txint_pc2;
            long clk = 0;
            driveIdle(t);
