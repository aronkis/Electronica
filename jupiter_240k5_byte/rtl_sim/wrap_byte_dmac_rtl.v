...
`timescale 1ns/1ps
module wrap_byte_dmac_rtl #(parameter FIFO_V4 = 0) (
  input  wire        clk,
  input  wire        resetn,
  ...
  output wire        irq,
  // serializer-side generator inputs -> REAL egress glue -> DMAC s_axis
  input  wire        clk_enable,
  input  wire [63:0] ser_word, input wire ser_tog, input wire ser_last, input wire ser_first,
  input  wire        tlast_en, input wire user_mask,
  output wire        s_axis_valid, output wire s_axis_ready, output wire s_axis_user_o, output wire [63:0] s_axis_data_o,
  output wire [31:0] fifo_ovf, output wire enb_1_2_0_o, output wire fifo_valid_o,
  ...
  wire [63:0] s_axis_data; wire s_axis_last_raw, s_axis_user_raw, s_axis_last, s_axis_user;
  egress_glue #(.FIFO_V4(FIFO_V4)) u_egress (.clk(clk), .reset(~resetn), .clk_enable(clk_enable),
    .ser_word(ser_word), .ser_tog(ser_tog), .ser_last(ser_last), .ser_first(ser_first),
    .byte_rx_ready(s_axis_ready), .byte_rx_data(s_axis_data), .byte_rx_valid(s_axis_valid),
    .byte_rx_last(s_axis_last_raw), .byte_rx_user(s_axis_user_raw), .ovf(fifo_ovf), .enb_1_2_0_o(enb_1_2_0_o), .fifo_valid_o(fifo_valid_o));
  assign s_axis_last = s_axis_last_raw & tlast_en;       // util_axis_byte_breakout_m: tlast gated by byte_ctrl_gpio bit0 (0 in -M mode)
  assign s_axis_user = s_axis_user_raw & ~user_mask;     // Option E probe gate (v2 injector semantics)
  assign s_axis_user_o = s_axis_user; assign s_axis_data_o = s_axis_data;
