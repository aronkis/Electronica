// wrap_byte_taps_mu.v -- campaign wrapper: wrap_byte.v + hierarchical Rx STAGE TAPS
...
module wrap_byte_taps_e5
  (input  wire clk, input wire reset,
   ...
   output wire fecV,  output wire fecB, output wire fecS,
   output wire [31:0] icState,
   output wire [13:0] psOff, output wire psDone, output wire psSucc, output wire psNewpk,
   output wire taArmed, output wire taOffValid, output wire taSync);                               // FEC-decoded bit / valid / startOut
  ...
  // Interpolation_Control telemetry (T8.7 stateWord): present in the Jul-25
  // generation only; surfaces as a flat wire at QPSK_Rx level
  // (QPSK_Rx.v:171 Frequency_and_Time_Synchronizer_t_ic).
  assign icState = dut.u_Receiver.u_QPSK_Rx.Frequency_and_Time_Synchronizer_t_ic;
  // E5 localisation taps (Preamble_Detector internals, Jul-25 gen): Peak_Search result and
  // Timing_Adjust arming, to see what a deleted symbol-sync strobe does downstream.
  assign psOff   = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Preamble_Detector.Peak_Search_timingOffset;
  assign psDone  = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Preamble_Detector.done;
  assign psSucc  = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Preamble_Detector.success;
  ...
  assign taOffValid = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Preamble_Detector.Delay12_out1;
