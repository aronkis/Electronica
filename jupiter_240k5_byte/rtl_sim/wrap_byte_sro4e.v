// wrap_byte_sro4e.v -- [sim] Task 21 (RXFIX_R4E) SRO harness wrapper (2026-09-05).
//
// A SIXTH SEPARATE FILE, not an edit of wrap_byte_sro.v (Task 7), wrap_byte_sro3s.v
// (Task 11), wrap_byte_sro4.v (Task 12), wrap_byte_sro4b.v (Task 12b) or
// wrap_byte_sro4d.v (Task 14): all five, and every result banked from them, must not
// move under Task 21 -- and Task 20's R4D+R1 legs run against wrap_byte_sro4d.v while
// this file is written.  The module NAME is deliberately still `wrap_byte_sro` so that
// sim_sro.cpp -- Task 7's driver, byte for byte -- links against it unmodified; that is
// what makes the content-identity gate a test of the RTL and not of a re-typed driver.
//
// BECAUSE SIX files now declare `module wrap_byte_sro`, this one announces itself at
// time 0.  A run that does not print WRAP4E_FILE / WRAP4E_DEFINE compiled the WRONG
// wrapper, and that failure would otherwise masquerade as "R4E never dropped".
// build_sro_rxfix4e.sh greps the verilate log for the other FIVE wrappers as well.
//
// R4E witnesses are carried on Task 7's three existing witness ports so that the driver
// needs no change:
//   r3Skips  = r4e_skips  (R4B's EMPTY-side steered pop SKIPS; <p>_ep.txt kind 4)
//   r3Extras = r4e_drops  (the FULL-side DROPPED PUSHES; <p>_ep.txt kind 5).  It IS a
//              count, as task 7 originally defined that kind.  Task 14 used the same port
//              for extra POPS; R4E adds no pop at all -- it drops a PUSH, which is the
//              natural push_on_full event's own shape (pops unchanged, valid density
//              unchanged, one symbol vanishes).
//   r3Guard  = Packet_Controller_endOut, the pulse that OPENS the structural window.
//
// THE EVENT TRACE IS WRITTEN BY THIS WRAPPER, NOT BY THE DRIVER, for the same reason
// tasks 12b/14 did it: sim_sro.cpp MUST NOT CHANGE and its <p>_ep.txt has no pcEnd record
// kind.  One line per event to the file named by +r4ewin=<path> (argv[8], past every
// argument the driver parses), with kind 0 = skip and kind 1 = DROP.
//
// WHAT MAKES THE DROP'S LANDING-SLOT RECOUNT INDEPENDENT.  The gate's central claim is
// that the dropped push's symbol would have been popped inside slots [pcEnd+1, pcEnd+13].
// The RTL measures that itself (r4e_land_last), so the harness must measure it from nets
// that share NOTHING with the R4E logic:
//   * occupancy at the drop from the RING POINTERS, (fifoPush - fifoPop) mod 32 -- NOT
//     occTrue/Delay_out1, which is the very net r4eOcc carries into the steering;
//   * m, the validated pops in [drop, pcEnd], counted from fifoVPop;
//   * the pcEnd reference from Packet_Controller_endOut.
// Then land_harness = occ_ptr + 1 - m, the formula derived in the pre-registration from
// FIFO_block's pointer arithmetic.  If the two disagree, the steering and the measurement
// disagree, and that is a finding rather than a silent pass.
//
// AND THE PRECONDITION THE WHOLE SCHEDULE RESTS ON.  R4E predicts the next pcEnd from the
// MEASURED frame period in validated pops.  That period has never been measured, so this
// wrapper also writes one line per pcEnd to +r4eper=<path>: the interval in validated
// pops.  The smoke leg does not pass unless its post-lock spread is <= 6.
//
// It also keeps the read-only per-stage taps of the Task 11/12/14 wrappers (no effect on
// any port sim_sro.cpp drives) so a stagewin dump can be built against R4E if the K8
// falsifier fires; sim_stagewin.cpp is the only consumer.
//
// Original header of the file this was cut from (via the Task 11, 12, 12b and 14
// wrappers):
...
module wrap_byte_sro
  ...
  // ---- T21: RXFIX_R4E witnesses, carried on T7's three witness ports ----
  // build_sro_rxfix4e.sh passes +define+RXFIX_R4E with the R4E tree on -y.
`ifdef RXFIX_R4E
  assign r3Skips  = {16'd0, dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4e_skips};
  assign r3Extras = {16'd0, r4e_drops_w};
  assign r3Guard  = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.Packet_Controller_endOut;

  // ---- the event trace (see the header) ----
  wire r4e_skip_ev  = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4e_do_skip;
  wire r4e_drop_ev  = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4e_do_drop;
  wire [15:0] r4e_drops_w = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4e_drops;
  wire [3:0] r4e_slot_rtl = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4e_wslot;
  wire [14:0] r4e_opens_w = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4e_opens;
  wire r4e_lock_w   = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4e_locked;
  wire [3:0] r4e_land_rtl = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4e_land_last;
  wire [3:0] r4e_land_out_w = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4e_land_out;
  wire [13:0] r4e_period_w = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4e_period;
  // INDEPENDENT of every R4E net: the nominal pop opportunity is the input-sample beat
  // at mod-4 phase 0, read off the Task 7 taps.
  wire r4e_popnom_h = rhValidIn & (rhPhase == 2'b00);
  // INDEPENDENT occupancy: the ring POINTERS, not Delay_out1.  Unambiguous over the
  // [24,31] band where drops fire; a true occupancy of 32 would alias to 0, which is why
  // occTrue is logged beside it in every drop record.
  wire [31:0] r4e_occ_ptr = ({27'd0, fifoPush} - {27'd0, fifoPop}) & 32'd31;

  integer r4e_fh, r4e_ph;
  reg [1023:0] r4e_fn, r4e_pn;
  reg [31:0] h_sidx, h_beat, h_dt, h_slot, h_nskip, h_ndrop;
  reg [31:0] h_vp, h_npce;                       // validated pops since pcEnd; pcEnd count
  reg        h_dpend;                            // a drop is awaiting its pcEnd
  reg [31:0] h_dm, h_docc, h_dtrue, h_dsidx, h_dbeat, h_dslot, h_ddt, h_dtref, h_dopens;
  reg [31:0] h_dland, h_dmhold, h_wr;
  initial begin
    if (!$value$plusargs("r4ewin=%s", r4e_fn)) r4e_fn = "r4e_win.txt";
    if (!$value$plusargs("r4eper=%s", r4e_pn)) r4e_pn = "r4e_period.txt";
    r4e_fh = $fopen(r4e_fn, "w");
    r4e_ph = $fopen(r4e_pn, "w");
    $fwrite(r4e_fh, "# n,sidx,beat,slot_rtl,slot_harness,dt_enb,occ,tref,locked,opens,kind,land_rtl,land_harness,m,occ_ptr,land_out\n");
    $fwrite(r4e_ph, "# n,sidx,vpops_in_interval,rtl_period,locked\n");
  end
  always @(posedge clk) begin
    if (reset) begin
      h_sidx <= 32'd0; h_beat <= 32'd0; h_dt <= 32'd0; h_slot <= 32'd0;
      h_nskip <= 32'd0; h_ndrop <= 32'd0; h_vp <= 32'd0; h_npce <= 32'd0;
      h_dpend <= 1'b0; h_dm <= 32'd0; h_wr <= 32'd0;
    end
    else begin
      ...
      if (railEnb) begin
        ...
        if (r3Guard) begin
          h_dt <= 32'd1;
          h_slot <= r4e_popnom_h ? 32'd1 : 32'd0;
        end
        else begin
          h_dt <= h_dt + 32'd1;
          if (r4e_popnom_h) h_slot <= h_slot + 32'd1;
        end
        // ---- the VALIDATED-POP clock, the unit the pointer arithmetic is in ----
        if (r3Guard) begin
          $fwrite(r4e_ph, "%0d,%0d,%0d,%0d,%0d\n", h_npce, h_sidx,
                  h_vp + (fifoVPop ? 32'd1 : 32'd0), r4e_period_w, r4e_lock_w);
          h_npce <= h_npce + 32'd1;
          h_vp <= 32'd0;
        end
        else if (fifoVPop) begin
          h_vp <= h_vp + 32'd1;
        end
        // ---- kind 0: a steered SKIP (the EMPTY edge), logged immediately ----
        if (r4e_skip_ev) begin
          h_nskip <= h_nskip + 32'd1;
          $fwrite(r4e_fh, "%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,0,0,0,0,%0d,%0d\n",
                  h_nskip, h_sidx, h_beat, r4e_slot_rtl + 4'd1, h_slot + 32'd1, h_dt,
                  occTrue, tref, r4e_lock_w, r4e_opens_w, r4e_occ_ptr, r4e_land_out_w);
        end
        // ---- kind 1: a DROPPED PUSH.  Its landing slot is not known until the pcEnd it
        // ---- was scheduled against, so the record is held and written two enb beats
        // ---- after that pcEnd, when the RTL's own r4e_land_last has been registered.
        if (h_dpend && fifoVPop) h_dm <= h_dm + 32'd1;
        if (h_dpend && r3Guard) begin
          h_dpend <= 1'b0;
          h_dmhold <= h_dm + (fifoVPop ? 32'd1 : 32'd0);
          h_dland <= (h_docc + 32'd1) - (h_dm + (fifoVPop ? 32'd1 : 32'd0));
          h_wr <= 32'd2;
        end
        else if (h_wr != 32'd0) begin
          if (h_wr == 32'd1) begin
            $fwrite(r4e_fh, "%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,1,%0d,%0d,%0d,%0d,%0d\n",
                    h_ndrop, h_dsidx, h_dbeat, h_dslot, h_dslot, h_ddt,
                    h_dtrue, h_dtref, r4e_lock_w, h_dopens,
                    r4e_land_rtl, h_dland, h_dmhold, h_docc, r4e_land_out_w);
            h_wr <= 32'd0;
          end
          else h_wr <= h_wr - 32'd1;
        end
        // the drop itself: latch everything the record needs, start counting m
        if (r4e_drop_ev) begin
          h_ndrop <= h_ndrop + 32'd1;
          h_dpend <= 1'b1;
          h_dm <= fifoVPop ? 32'd1 : 32'd0;
          h_docc <= r4e_occ_ptr;
          h_dtrue <= {26'd0, occTrue};
          h_dsidx <= h_sidx; h_dbeat <= h_beat; h_ddt <= h_dt;
          h_dslot <= h_slot + 32'd1; h_dtref <= {18'd0, tref};
          h_dopens <= {17'd0, r4e_opens_w};
        end
      end
    end
  end
...
`endif
  ...
  // ---- T21: which wrapper actually compiled.  SIX files declare this module. ----
  initial begin
    $display("WRAP4E_FILE wrap_byte_sro4e.v t21");
`ifdef RXFIX_R4E
    $display("WRAP4E_DEFINE RXFIX_R4E");
`else
    $display("WRAP4E_DEFINE none");
`endif
  end
endmodule
