...
module wrap_byte_sro
  ...
  // ---- T7: RXFIX_R3 witnesses.  The baseline tree has no such nets, so the
  // taps are compiled only when the variant tree is the -y source
  // (build_sro_rxfix3.sh passes +define+RXFIX_R3); the ports exist either way
  // so ONE driver binary layout serves both builds.
`ifdef RXFIX_R3
  assign r3Skips  = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r3_skips;
  assign r3Extras = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r3_extras;
