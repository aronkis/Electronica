// wrap_byte_sro4b.v -- [sim] Task 12b (RXFIX_R4B) SRO harness wrapper (2026-09-04).
//
// A FOURTH SEPARATE FILE, not an edit of wrap_byte_sro.v (Task 7), wrap_byte_sro3s.v
// (Task 11) or wrap_byte_sro4.v (Task 12): all three, and every result banked from
...
//
// BECAUSE FOUR files now declare `module wrap_byte_sro`, this one announces itself at
// time 0.  A run that does not print WRAP4B_FILE / WRAP4B_DEFINE compiled the WRONG
// wrapper, and that failure would otherwise masquerade as "R4B never skipped".
// build_sro_rxfix4b.sh greps the verilate log for the other THREE wrappers as well.
//
// R4B witnesses are carried on Task 7's three existing witness ports so that the driver
// needs no change:
//   r3Skips  = r4b_skips     (steered pop SKIPS; <p>_ep.txt kind 4 timestamps each one)
//   r3Extras = 32'hA5A50002 while r4b_locked, else 0.  A SENTINEL, NOT a count: R4B has
//              no extra-pop branch at all, and a plain 1 here would read as "one extra
//              pop was taken".  The 0 -> A5A50002 transition is the LOCK instant, so
//              <p>_ep.txt kind 5 timestamps lock.  (Task 12 used this port for the
//              pre-fill sentinel A5A50001; R4B has no pre-fill -- see the gate doc.)
...
// wrapper $fwrites one line per skip to the file named by +r4bwin=<path> (default
// r4b_skipwin.txt), carrying BOTH the RTL's own window slot (r4b_wslot + 1) and an
...
module wrap_byte_sro
  ...
  // ---- T12b: RXFIX_R4B witnesses, carried on T7's three witness ports ----
  // build_sro_rxfix4b.sh passes +define+RXFIX_R4B with the R4B tree on -y.
`ifdef RXFIX_R4B
  assign r3Skips  = {16'd0, dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4b_skips};
  // SENTINEL, not a count -- R4B has no extra-pop branch.  See the header.
  assign r3Extras = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4b_locked
                    ? 32'hA5A50002 : 32'd0;
  ...
  wire r4b_skip_ev = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4b_do_skip;
  wire [3:0] r4b_slot_rtl = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4b_wslot;
  wire [14:0] r4b_opens_w = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4b_opens;
  wire r4b_lock_w = dut.u_Receiver.u_QPSK_Rx.u_Frequency_and_Time_Synchronizer.u_Symbol_Synchronizer.u_Rate_Handle.r4b_locked;
  ...
  wire r4b_popnom_h = rhValidIn & (rhPhase == 2'b00);

  integer r4b_fh;
  reg [1023:0] r4b_fn;
  reg [31:0] h_sidx, h_beat, h_dt, h_slot, h_nskip;
  initial begin
    if (!$value$plusargs("r4bwin=%s", r4b_fn)) r4b_fn = "r4b_skipwin.txt";
    r4b_fh = $fopen(r4b_fn, "w");
    $fwrite(r4b_fh, "# n,sidx,beat,slot_rtl,slot_harness,dt_enb,occ,tref,locked,opens\n");
  end
  always @(posedge clk) begin
    if (reset) begin
      h_sidx <= 32'd0; h_beat <= 32'd0; h_dt <= 32'd0; h_slot <= 32'd0; h_nskip <= 32'd0;
    end
    else begin
      ...
      if (railEnb) begin
        ...
        if (r3Guard) begin
          h_dt <= 32'd1;
          h_slot <= r4b_popnom_h ? 32'd1 : 32'd0;
        end
        else begin
          h_dt <= h_dt + 32'd1;
          if (r4b_popnom_h) h_slot <= h_slot + 32'd1;
        end
        if (r4b_skip_ev) begin
          h_nskip <= h_nskip + 32'd1;
          $fwrite(r4b_fh, "%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d\n",
                  h_nskip, h_sidx, h_beat, r4b_slot_rtl + 4'd1, h_slot + 32'd1, h_dt,
                  occTrue, tref, r4b_lock_w, r4b_opens_w);
        end
      end
    end
  end
...
`endif
  ...
  initial begin
    $display("WRAP4B_FILE wrap_byte_sro4b.v t12b");
`ifdef RXFIX_R4B
    $display("WRAP4B_DEFINE RXFIX_R4B");
`else
    $display("WRAP4B_DEFINE none");
`endif
  end
endmodule
