...
% Frame gate: the tick-compensation arithmetic is k5-HARDCODED -- the P1eArm
% wrap modulus is 1133 sym/frame (k5), and acc<=530 / FIFO-transit 566 / d==32
% are all k5-window constants (see p1eArm_src below). At another geometry
% (f1536: 12333 sym/frame) the mod-1133 residue is wrong, so P1eArm can arm
% SPURIOUS 64-coded-bit deint head-skips and corrupt every frame. p1e recovers
% one frame per ~1.5s ADRV9002 tick (a live-HW-only event; absent in
% sim/loopback), so skipping it costs nothing in sim and only the per-tick
% casualty on real non-k5 HW -- a proper f1536 tick-comp retune is separate RX
% work. Skip for non-k5 rather than fire wrong.
cfg = frame_config_k5();
if ~strcmp(cfg.Frame,'k5')
    fprintf('p1e_comp_overlay: SKIPPED for frame=%s (tick-comp math is k5-hardcoded: mod 1133, acc<=530, FIFO 566, d==32). A per-frame retune is separate RX work.\n', cfg.Frame);
    return;
end
...
src = rep1(src, 'persistent ramA ramB rdBank wptr pcnt hold0 hold1 rdGate rcol rrow;', ...
                'persistent ramA ramB rdBank wptr pcnt hold0 hold1 rdGate rcol rrow skipCnt pendD;');
src = rep1(src, 'rcol=uint16(0); rrow=uint16(0); end', ...
                'rcol=uint16(0); rrow=uint16(0); skipCnt=uint16(0); pendD=false; end');
src = rep1(src, 'rb=rdBank; w=wptr; pc=pcnt; g=rdGate; h0=hold0; h1=hold1; cc=rcol; rr=rrow;', ...
                ['rb=rdBank; w=wptr; pc=pcnt; g=rdGate; h0=hold0; h1=hold1; cc=rcol; rr=rrow;' nlc ...
                 'skc=skipCnt; pend=(pendD || logical(setIn)) && ~logical(clrIn);']);
src = rep1(src, 'if startIn, w=uint16(0); pc=uint16(0); rb=~rb; g=true; cc=uint16(0); rr=uint16(0); end', ...
                ['if startIn, w=uint16(0); pc=uint16(0); rb=~rb; g=true; cc=uint16(0); rr=uint16(0); ' ...
...
src = rep1(src, ['  if w < CODED' nlc], ...
                ['  if skc > uint16(0)' nlc '    skc = skc - uint16(1);' nlc ...
                 '  elseif w < CODED' nlc]);
src = rep1(src, 'rdBank=rb; wptr=w; pcnt=pc; rdGate=g; hold0=h0; hold1=h1; rcol=cc; rrow=rr;', ...
                'rdBank=rb; wptr=w; pcnt=pc; rdGate=g; hold0=h0; hold1=h1; rcol=cc; rrow=rr; skipCnt=skc; pendD=pend;');
chart.Script = src;
