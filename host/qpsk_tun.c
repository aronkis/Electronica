 ...
 * (qpsk_net_setup.sh single-board; byte_link_up.sh two-radio K5, tag
...
#include <stdio.h>
#include <stdio_ext.h>  /* __fpurge: discard a stdio write buffer WITHOUT writing it
                         * (glibc extension, present on both boards -- 146/Kuiper
                         * glibc 2.31 and 148/trixie glibc 2.41). Used only by the
                         * QPSK_ROTFIX rotate; see framelog_service(). */
#include <stdlib.h>
#include <stdint.h>
#include <string.h>
#include <unistd.h>
 ...
 *
 * ASYMMETRIC byte contract (contract/PACKET_K5.txt, gen_byte_vectors_k5.m):
 ...
 * 240 ksym = 51.39 ms (contract/PACKET_F1536.txt; the WPP=191 delivery frame
...
/* Forward declaration only -- the counters live beside the queued-RX drain code
 * that maintains them, for the same object-layout reason the rxqstat comment
 * above gives. */
...
/* ---- QPSK_ROTFIX: rotate without file I/O on the transmit-feed path -------
 * RXFIX Task 35 (ATARM_CLASS.md sec 2, 4.1, 7.1; tag
 * archive/pre-cleanup-2026-09-09). The SIGUSR2
 * rotate used to run `fclose()` (flush of up to 1 MiB of stdio buffer) +
 * `fopen(path,"wb")` (truncate of a 2-3 MB tmpfs file) INLINE in the single
 * event loop that also paces tx_send(). With 1-2 frames (0.8-1.6 ms of air)
 * queued that is a transmit-feed silence of 2 ms typically and 16-24 ms in the
 * measured tail -- and every observable >= 7 ms transmit silence collapsed the
 * FAR receiver into a carrier-reset storm (15/15 at the rotate, 23/23 at the
 * step-8 SIGUSR1 dump, 0/35 at <= 3 ms).
 *
 * The fix rests on one observation: the <= 1 MiB the old `fclose` flushed was
 * then thrown away by the `fopen(...,"wb")` truncate two statements later, so
 * that write was always pure waste. With QPSK_ROTFIX set the rotate does, in
 * the loop, only metadata:
 *   __fpurge(old)                -- O(1) pointer reset, no write
 *   rename(path.next, path)      -- the pulled name now IS the fresh inode
 *   framelog_fp = next           -- pointer swap
 *   fopen(path.next, "wb")       -- create an empty successor (nothing to truncate)
 * and parks the old FILE* in framelog_pending_close. The page-free of the old
 * (now unlinked) 2-3 MB inode is settled by framelog_drain_pending(), called at
 * the END of instr_usr1_dump() -- i.e. inside the step-8 stall that is already
 * outside the scored window -- and at exit.
 *
 * DEFAULT SINCE 2026-09-06 IS THE DEFERRED CLOSE. QPSK_ROTFIX="0" => framelog_rotfix == 0 and
 * framelog_service() takes the historical three-line inline branch verbatim.
 * That is what makes the Task 38 A/B a runtime knob on ONE binary rather than
 * two builds: CTL = QPSK_ROTFIX=0, FIX = default, same qpsk_tun.
 *
 * Fallback if the measured work_us on silicon is still >= 1 ms: ATARM sec 7.1's
 * helper thread (hand the old FILE* to a SCHED_OTHER closer). Not taken here --
 * it puts pthreads into a single-threaded chrt -f 50 daemon, needs -lpthread on
 * 146's glibc 2.31, and cannot be validated by a desk build on a modern-glibc
 * host. It stays the next A/B, not a silent upgrade. */
static int   framelog_rotfix = 1;             /* QPSK_ROTFIX: default 1 = deferred close; "0" = legacy inline */
static FILE *framelog_next_fp = NULL;         /* pre-opened successor (fresh, empty inode) */
static FILE *framelog_pending_close = NULL;   /* purged pre-rotate FILE*, closed off the feed path */
static char  framelog_next_path[512];
static unsigned framelog_rot_n;               /* rotates serviced (SIGUSR2), for the log line */

/* ROTATE witness -- STAGED on the feed path, WRITTEN off it (Task 38 review).
 * The witness line is the A/B's arm-identity record and its dose measurement, so
 * it must be emitted on both arms; but an unbuffered ~90-byte stderr write to
 * /dev/shm at the rotate instant is itself transmit-feed silence of exactly the
 * kind this patch exists to remove, and a line cannot contain the cost of
 * writing itself -- with the write inline, `work_us` structurally UNDER-REPORTS
 * the silence the patch adds. So framelog_service() only stores the numbers
 * (O(1), no syscall, inside the measured interval) and the line is written where
 * the deferred close is already settled: at the END of instr_usr1_dump() (step 8,
 * outside the scored window) and in framelog_close() at exit. `work_us` then
 * closes over EVERYTHING the rotate does on the transmit-feed path.
 * The ring is small on purpose -- capture_r3.sh sends one SIGUSR2 per leg and
 * drains at step 8. An overflow is counted and reported, never silent; the `n=`
 * sequence has a gap wherever a record was dropped. */
struct framelog_rot_witness {
    unsigned    n;              /* framelog_rot_n at this rotate */
    const char *mode;           /* branch that ACTUALLY ran: deferred / inline / nofile */
    const char *fallback;       /* why the deferred branch was not taken */
    int         err;            /* errno of a failed rename(), else 0 */
    uint64_t    work_ns;        /* the whole feed-path interval of this rotate */
    uint64_t    t_real_ns;      /* the rotate instant, CLOCK_REALTIME */
};
#define FRAMELOG_ROT_WIT_MAX 8
static struct framelog_rot_witness framelog_rot_wit[FRAMELOG_ROT_WIT_MAX];
static unsigned framelog_rot_wit_n;           /* staged, not yet written */
static unsigned framelog_rot_wit_drop;        /* staged records lost to overflow */
...
/* QPSK_ROTFIX: create the empty successor the next rotate will rename() into
 * place. A fresh name means fopen() has nothing to truncate. Returns 0 on
 * success; any failure leaves framelog_next_fp NULL and the rotate falls back
 * to the inline path, so a failure degrades to today's behaviour, never worse. */
static int framelog_preopen(void)
{
    int n;
    /* EVERY failure return sets errno (Task 38 review D3): the caller reports
     * `strerror(errno)`, and the two non-syscall failures below used to leave it
     * holding whatever some unrelated earlier call had set. */
    if (!framelog_path) { errno = EINVAL; return -1; }
    if (framelog_next_fp) return 0;
    n = snprintf(framelog_next_path, sizeof framelog_next_path, "%s.next", framelog_path);
    if (n < 0 || (size_t)n >= sizeof framelog_next_path) { errno = ENAMETOOLONG; return -1; }
    framelog_next_fp = fopen(framelog_next_path, "wb");
    if (!framelog_next_fp) return -1;           /* fopen() set errno */
    setvbuf(framelog_next_fp, NULL, _IOFBF, 1u << 20);
    return 0;
}

/* QPSK_ROTFIX: settle the pre-rotate file. Its buffer was already discarded by
 * __fpurge and its name was already taken over by the rename, so this is a
 * close() of an unlinked tmpfs inode -- the 2-3 MB page-free, and nothing else.
 * Called from instr_usr1_dump() (step 8, outside the scored window) and at exit,
 * NEVER from framelog_service(). */
static void framelog_drain_pending(void)
{
    if (framelog_pending_close) { fclose(framelog_pending_close); framelog_pending_close = NULL; }
}

/* Write the ROTATE witnesses staged by framelog_service(). Same rule as the
 * drain: called from instr_usr1_dump() (step 8, outside the scored window) and
 * at exit, NEVER from framelog_service(). */
static void framelog_rot_witness_emit(void)
{
    unsigned i;
    for (i = 0; i < framelog_rot_wit_n; i++) {
        const struct framelog_rot_witness *w = &framelog_rot_wit[i];
        char errbuf[64];
        errbuf[0] = '\0';
        /* w->err, saved at the rotate -- NOT strerror(errno), which here would
         * report whatever ran in between (the same stale-errno defect again). */
        if (w->err)
            snprintf(errbuf, sizeof errbuf, " err=%s", strerror(w->err));
        fprintf(stderr, "framelog: ROTATE n=%u mode=%s work_us=%.1f t_real=%.6f fallback=%s%s\n",
                w->n, w->mode, (double)w->work_ns / 1e3,
                (double)w->t_real_ns / 1e9, w->fallback, errbuf);
    }
    framelog_rot_wit_n = 0;
    if (framelog_rot_wit_drop) {
        fprintf(stderr, "framelog: ROTATE witness overflow -- %u record(s) dropped"
                        " (the gap is visible in the n= sequence)\n", framelog_rot_wit_drop);
        framelog_rot_wit_drop = 0;
    }
}
...
static void framelog_close(void)
{
    ...
    /* QPSK_ROTFIX teardown: settle any deferred close and remove the unused
     * successor, so a run leaves no stray <path>.next behind in /dev/shm. */
    framelog_drain_pending();
    /* A rotate with no step-8 dump after it still gets its witness line. */
    framelog_rot_witness_emit();
    if (framelog_next_fp) {
        fclose(framelog_next_fp); framelog_next_fp = NULL;
        unlink(framelog_next_path);
    }
}

static void instr_rotate_rings(void);   /* defined beside the TX log below */
static void rxresync_rotate(void);      /* defined beside the re-anchor below */

/* Consumed from the event loops (not the signal handler) when SIGUSR2 sets the
 * flag: fopen/fclose are not async-signal-safe.
 * SIGUSR2 rotates ALL THREE sinks, not just the framelog. capture_r3.sh:218
 * sends SIGUSR2 so the pulled frames.bin is the steady-state post-heal window
 * only; if the rings kept their pre-rotate contents the three files would span
 * DIFFERENT windows and the TX<->RX join would score every pre-rotate submit as
 * "never sent" -- a false epidemic of exactly the class the decision table is
 * most sensitive to. Rotating the rings alongside keeps one window. */
static void framelog_service(void)
{
    uint64_t t0, t1, t_real;
    struct framelog_rot_witness *w;
    int deferred = 0;                /* set ONLY inside the branch that actually runs */
    const char *mode = "nofile";     /* no framelog file open => neither branch runs */
    const char *fallback = "no-file";
    int err = 0;

    if (!framelog_rotate_req)
        return;
    framelog_rotate_req = 0;
    t0 = framelog_clock_ns(CLOCK_MONOTONIC);
    instr_rotate_rings();
    /* RXFIX-28: baseline the re-anchor counters too, so the `rxresync2:` line
     * describes the SAME window as the three rotated sinks. The cumulative
     * `rxresync:` counters are deliberately left alone. */
    rxresync_rotate();
    if (framelog_fp) {
        /* Decide first, and record WHY, so the witness reports the branch that
         * ran rather than the knob that was asked for (Task 38 review D1: a
         * failed rename -- ENOSPC on /dev/shm is the realistic one, it also holds
         * pair.iq, a 32 MiB txlog.bin, failhdr.bin and frames.bin -- fell back to
         * the legacy branch correctly and still logged mode=deferred, silently
         * voiding the arm under the brief's sec 5 rule). */
        if (!framelog_rotfix)
            fallback = "knob-off";
        else if (framelog_pending_close)
            /* A close is still parked (a second SIGUSR2 with no SIGUSR1 between:
             * the documented "drain a long capture without stopping the daemon"
             * use, see the QPSK_FRAMELOG comment above). Do NOT settle it here --
             * a drain on the feed path is the 0.4-4 ms page-free this patch
             * exists to remove, i.e. AT OR WORSE THAN the legacy rotate it
             * replaces. Take the legacy branch instead: same cost as today's
             * daemon, never worse, and it bounds the outstanding unlinked inodes
             * to ONE, which is what keeps repeated rotates off the /dev/shm
             * ENOSPC path. The witness says which branch ran. */
            fallback = "pending-close";
        else if (!framelog_next_fp)
            fallback = "no-preopen";
        else if (rename(framelog_next_path, framelog_path) != 0) {
            err = errno;
        ...
        if (deferred) {
            /* metadata only: no flush (the bytes are discarded by the rotate
             * anyway), no truncate (the successor is a fresh empty inode) and
             * no page-free (deferred to instr_usr1_dump/exit). */
            __fpurge(framelog_fp);
            framelog_pending_close = framelog_fp;
            framelog_fp = framelog_next_fp;
            framelog_next_fp = NULL;
            framelog_preopen();             /* ready for the next rotate; a failure
                                             * here is not fatal -- the next rotate
                                             * takes the legacy branch and says so
                                             * (fallback=no-preopen) */
        } else {
            /* legacy path, verbatim -- and the fallback whenever the knob is off,
             * a close is still pending, the pre-open failed or the rename failed,
             * so a broken ROTFIX degrades to today's daemon and never to worse. */
            fclose(framelog_fp);
            framelog_fp = fopen(framelog_path, "wb");   /* truncate + reopen */
            if (framelog_fp)
                setvbuf(framelog_fp, NULL, _IOFBF, 1u << 20);
        }
        mode = deferred ? "deferred" : "inline";        /* the branch that RAN */
    }
    /* The witness that never existed (ATARM_CLASS.md sec 1: both rotate instants
     * had to be reconstructed from a txlog seq join). It is STAGED here and
     * written off this path (see struct framelog_rot_witness), so work_us closes
     * over everything this rotate does on the transmit-feed path: the ring reset,
     * the resync snapshot, the branch above and this staging. The only work left
     * outside [t0, t1] is the store of work_ns itself -- no syscall, no write. */
    t_real = framelog_clock_ns(CLOCK_REALTIME);
    framelog_rot_n++;
    w = (framelog_rot_wit_n < FRAMELOG_ROT_WIT_MAX)
        ? &framelog_rot_wit[framelog_rot_wit_n++] : NULL;
    if (w) {
        w->n = framelog_rot_n; w->mode = mode; w->fallback = fallback;
        w->err = err; w->t_real_ns = t_real; w->work_ns = 0;
    } else {
        framelog_rot_wit_drop++;
    }
    t1 = framelog_clock_ns(CLOCK_MONOTONIC);
    if (w)
        w->work_ns = t1 - t0;
}
 ...
 * "<QIHH" records; txlog_gaps.py (tag archive/pre-cleanup-2026-09-09) and
 * ops/loss_ledger.py's tx_gaps_146() still assume that and must be updated
 * (Task 2). New dumps
...
/* SIGUSR2 rotate, ring half: discard everything logged before the rotate so all
 * three sinks describe the same window. Resetting txlog_head can leave an
 * in-flight transfer holding a now-stale record index that would then stamp a
 * record belonging to a different frame, so the pending completions are
 * cancelled outright (at most two completion stamps are lost per rotate). */
static void instr_rotate_rings(void)
{
    failhdr_head = 0;
    txlog_head = 0;
    memset(tx_log_cnt, 0, sizeof tx_log_cnt);
}
...
static void instr_usr1_dump(void)
{
    ...
    if (txlog_on_usr1)
        txlog_dump();
    /* QPSK_ROTFIX (last, deliberately): settle the pre-rotate file's page-free
     * here, where the loop is already stalled for 4-246 ms by the dumps above and
     * where the scored window has ended, and write the ROTATE witnesses staged by
     * framelog_service() for the same reason. Appended rather than inserted so
     * that framelog_flush() stays the LAST WRITE TO THE FRAMELOG before the step-8
     * stall -- the invariant ATARM_CLASS.md sec 4.2 uses to locate that stall in
     * the archive (these two touch the pending FILE* and stderr, never frames.bin).
     * The drain is a no-op (one pointer test) when ROTFIX is off; the emit writes
     * one line per rotate on BOTH arms. */
    framelog_drain_pending();
    framelog_rot_witness_emit();
}
 ...
 * THE DEFECT. The drain loop below carves the completed DMA area into frames at
 * a FIXED pkt_bytes stride from the area base, and the ONLY thing that ever
 * re-establishes frame alignment is the start of the NEXT transfer (each
 * request gates on the frame-sync tuser -- rr.v:161, quoted in the queued-RX
 * header comment above). So a byte-count anomaly in the fabric's byte stream
 * whose size is not a multiple of pkt_bytes puts every following frame in that
 * transfer at a constant nonzero phase inside its slot, and every one of them
 * fails to parse although its bytes are intact.
 *
 * THE MEASUREMENT (FWD_RESIDUAL_0p22.md, tag archive/pre-cleanup-2026-09-09;
 * RXFIX Task 23, forward
 * leg 20260904_201814_w1_air): 1,259 of the 1,305 magic-carrying class-1
 * failures sit at exactly phase 568 against a 2.3 % chance floor, with 25 at
 * 376 and 14 at 1,136 = 2 x 568. F1536 geometry: pkt_bytes = 1528, so the
 * stream lost 1528 - 568 = 960 B (or gained 568 B) at the event. One decode
 * error at the decoder pins therefore costs the host 8.59 frames instead of
 * ~2: 0.0540 % at the pins against 0.2145 % at the host, on 144 vs 143 events.
 *
 * THE CASCADE LENGTH is set by this file, not by the fabric. With -M 16
 * (bringup_r2r3.sh:171 RXM_EFF=${RXM:-16}) a transfer is 16 slices, so a burst
 * runs from the event to the end of that transfer: at most 16 displaced slices
 * plus the frame the event damaged = 17. Task 23 measured a hard ceiling of
 * exactly 17 and a mean of 1,956/213 = 9.18 slots per event against the 9.0 a
 * uniform position-in-transfer predicts.
 *
 * THE FIX. Make the drain cursor a BYTE offset into the area (rx_dphase) and,
 * when a frame fails to parse, scan the bytes that follow it for the next
 * offset at which a whole frame validates (qpsk_frame_resync, bounded to
 * < pkt_bytes so it can never "find" the ordinary next slot). The displaced
 * frame is SPLIT ACROSS TWO SLOTS -- at phase 568 only 960 of its 1528 bytes
 * are inside the slot the parser rejected -- so a scan confined to one slice
 * could never validate it; the scan must run over the contiguous carve.
 * Failing slices are still counted and classified exactly as before (the
 * re-anchor happens AFTER st.crc_drops++/framelog_record_fail), so the
 * fail-class taxonomy and every offline census keep their meaning.
 *
 * QPSK_RX_RESYNC=0 restores the historical byte-for-byte behaviour from the
...
static unsigned long long rxr_recovered; /* frames delivered at a displaced phase */
static unsigned long long rxr_tail;    /* transfer tails given up (no whole frame
                                        * left before the DMA-written limit)     */

/* ---- RXFIX Task 28: two instrument defects the Task 27 judge leg exposed ---
 * Both are fixed by ADDING output.  The `qpsk_tun rxresync:` line above and
 * every counter behind it stay byte-for-byte what they were, so past captures
 * remain comparable and any existing parser keeps working.
 *
 * (a) THE 568/other SPLIT HARD-CODES ONE LEG'S DISPLACEMENT.  Task 28 measured
 *     the slot phase on three captures: 568 B on the Task 13 leg and on the
 *     Task 27 fix-OFF control (94-97 % of the magic-carrying class-1 failures),
 *     but 952 B on the fix-ON leg of the same back-to-back A/B -- each value
 *     stable across the whole 700 s window.  The phase is a per-leg constant,
 *     not a universal one, so `resync_568=0 resync_other=171` was read as "the
 *     568 phase is refuted" when it only meant "this leg's phase was not 568".
 *     rxr_hist records the offset itself, on the 8-byte grid the scan uses.
 *
 * (b) THE COUNTERS ARE NEVER ROTATED.  Every other sink is reset by SIGUSR2
 *     (framelog_service), but rxr_* are cumulative from daemon start, so they
 *     include the pre-link acquisition phase in which the drain reads garbage
 *     and every scan necessarily fails: 9,478 of the judge leg's 9,757 failed
 *     scans happened before the link delivered its first frame, which made an
 *     in-window 169:279 success:fail ratio read as 171:9,757.  A baseline
 *     snapshot at the rotate lets a window-scoped set be printed ALONGSIDE the
 *     cumulative one; the cumulative counters themselves are never touched.
 *
 * QPSK_RX_RESYNC_LOG=1 additionally prints one line per successful re-anchor
 * (default OFF -- no cost, and nothing on the drain path changes when unset).
 * Its fields are the ones that would have settled Task 28 without inference:
 * the offset, the cursor phase before the move, the slot within the transfer,
 * the recovered seq, and how many scans failed since the previous hit. */
#define RXR_HIST_N   (QPSK_PKT_BYTES_MAX / QPSK_RESYNC_STEP + 1)
static unsigned long long rxr_hist[RXR_HIST_N];  /* re-anchors, indexed by d/8 */
static int rxr_off_last = -1;          /* last re-anchor offset, -1 = none yet */
...
static void rxresync_rotate(void)
{
    rxr_base[0] = rxr_568;
    rxr_base[1] = rxr_other;
    rxr_base[2] = rxr_fail;
...
static void rxresync_dump(void)
{
    int top[4], ntop = 0, i, j, k, mode = -1;
    unsigned long long mode_n = 0;

    fprintf(stderr, "qpsk_tun rxresync: on=%d phase=%d resync_568=%llu "
            "resync_other=%llu resync_fail=%llu recovered=%llu tail_lost=%llu\n",
            rx_resync, rx_dphase, rxr_568, rxr_other, rxr_fail,
            rxr_recovered, rxr_tail);
    /* top 4 populated histogram bins, descending -- 191 bins is far too many to
     * print every window, and the distribution is one mode plus a thin tail. */
    for (i = 1; i < RXR_HIST_N; i++) {
        if (!rxr_hist[i])
            continue;
        for (j = 0; j < ntop; j++)
            if (rxr_hist[i] > rxr_hist[top[j]])
                break;
        if (j >= 4)
            continue;                     /* smaller than the four already held */
        for (k = (ntop < 4 ? ntop : 3); k > j; k--)
            top[k] = top[k - 1];
        top[j] = i;
        if (ntop < 4)
            ntop++;
...
/* Re-anchor the drain cursor after a parse failure (RXFIX Task 26; see the
 * rx_resync block above for the defect this recovers).
 *
 *   area   the completed area being drained
 *   off    byte offset, within that area, of the frame that just failed
 *   limit  end of the DMA-written region (rx_multi * pkt_bytes)
 *
 * Copies one window out of the carve and hands it to the pure scanner. On
 * success the cursor (rx_dscan / rx_dphase) is moved to the recovered frame
 * PLUS one frame -- strictly forward of where the plain advance would have put
 * it, so no frame is ever delivered twice and the loop still terminates -- the
 * frame is decoded into out/seq, its payload length is stored in *plen (0 = a
 * recovered idle frame) and the offset is returned. Returns -1 on no match.
 *
 * The window is copied, not scanned in place, for two reasons: the carve is
 * non-cacheable device memory that must be read through carve_copy_from's
 * 64-bit volatile loads, and having the bytes in host memory means the frame
 * that validates is decoded from the copy instead of being read a second time.
 *
 * BOUND. max_shift is pkt_bytes - QPSK_RESYNC_STEP, so a re-anchor can only be
 * a genuine sub-frame displacement; the next aligned frame at +pkt_bytes is
 * outside the scan by construction. The window itself is capped at the
 * DMA-written region, so nothing beyond `limit` -- stale bytes from an earlier
 * lap, never zeroed for that part of the area -- can ever be parsed. */
static int rx_resync_try(unsigned area, size_t off, size_t limit,
                         unsigned char *out, uint32_t *seq, int *plen)
{
    static unsigned char rxr_win[2 * QPSK_PKT_BYTES_MAX];
    size_t span;
    size_t nx;
    int d, len = -1;
    ...
    if (!rx_resync)
        return -1;
    /* carve_copy_from issues 64-bit volatile loads; an unaligned one FAULTS on
     * ARM64 Device memory. Every offset this function produces is a multiple of
     * QPSK_RESYNC_STEP, which is only word-aligned if pkt_bytes is too (it is
     * in every shipped geometry -- 128, 280, 1528 -- but -p can override it). */
    if ((pkt_bytes % QPSK_RESYNC_STEP) != 0)
        return -1;
    if (off >= limit)
        return -1;
    span = limit - off;
    if (span > sizeof rxr_win)
        span = sizeof rxr_win;
    if (span < (size_t)pkt_bytes + QPSK_RESYNC_STEP)
        return -1;              /* transfer tail: no whole shifted frame left */
    carve_copy_from(rxr_win, rx_area_virt(area) + off, span);
    d = qpsk_frame_resync(rxr_win, span, pkt_bytes,
                          pkt_bytes - QPSK_RESYNC_STEP, out, seq, &len);
    if (d <= 0) {
        rxr_fail++;
        rxr_fails_since++;      /* RXFIX-28 */
        return -1;
    }
    if (d == QPSK_RESYNC_OFF_568) rxr_568++;
    else                          rxr_other++;
    /* RXFIX-28: record the offset ITSELF, not just "is it 568". d is always a
     * multiple of QPSK_RESYNC_STEP and strictly below pkt_bytes (the scanner's
     * bound), so the index is in range by construction; the guard is belt and
     * braces against a future -p geometry. */
    {   int hi = d / QPSK_RESYNC_STEP;
        if (hi > 0 && hi < RXR_HIST_N) rxr_hist[hi]++;
        rxr_off_last = d;
    }
    if (rxr_log) {
        struct timespec rts;
        clock_gettime(CLOCK_MONOTONIC, &rts);
        fprintf(stderr, "qpsk_tun rxresync@ t=%llu area=%u slot=%d off=%llu "
                "d=%d phase_before=%d seq=%u len=%d fails_since=%llu\n",
                (unsigned long long)rts.tv_sec * 1000000000ull
                    + (unsigned long long)rts.tv_nsec,
                area, (int)(off / (size_t)pkt_bytes), (unsigned long long)off,
                d, rx_dphase, seq ? *seq : 0u, len,
                rxr_fails_since);
    }
    rxr_fails_since = 0;
    nx = off + (size_t)d + (size_t)pkt_bytes;    /* consume the recovered frame */
    rx_dscan  = (int)(nx / (size_t)pkt_bytes);
    rx_dphase = (int)(nx % (size_t)pkt_bytes);
    rxr_recovered++;
    if (plen)
        *plen = len;
    return d;
...
static int rx_pump_queued(unsigned char *out, uint32_t *seq)
{
    ...
    if (rx_drain >= 0) {
        ...
        while (rx_dscan < rx_multi) {
            ...
            unsigned char slice[QPSK_PKT_BYTES_MAX];
            /* RXFIX-26: the cursor is a BYTE offset into the area. rx_dphase is
             * 0 -- and this address is then bit-for-bit the historical
             * rx_dscan*pkt_bytes -- until a re-anchor moves it. */
            const size_t rx_doff = (size_t)rx_dscan * (size_t)pkt_bytes
                                 + (size_t)rx_dphase;
            ...
            rx_dscan++;
            if (m >= 0 && rx_dphase) rxr_recovered++;   /* delivered while displaced */
            ...
            st.crc_drops++;
            framelog_record_fail(slice, pkt_bytes);
            /* RXFIX-26 re-anchor. Runs AFTER the failing slice has been counted
             * and classified, so crc_drop, the framelog and the failhdr census
             * mean exactly what they meant before; what changes is only where
             * the cursor goes next. On success the cursor is already past the
             * recovered frame (rx_resync_try), so fall straight into delivering
             * it. */
            {   int rlen = -1;
                if (rx_resync_try((unsigned)rx_drain, rx_doff, rx_dlimit,
                                  out, seq, &rlen) > 0) {
                    rx_q_progress = rx_q_delivered = now_s();
                    if (rlen > 0) return rlen;
                    st.idle_rx++;      /* recovered an idle/keepalive frame */
                }
            }
        }
 ...
 * Deploy the 2 MB qpsk dtb (ops/deploy_dtb.sh) before provisioning the
 * 2 MB host build; see docs/build-and-flash.rst. */
...
int main(int argc, char **argv)
{
    ...
    /* QPSK_RX_RESYNC (RXFIX Task 26): re-anchor the queued drain cursor on a
     * parse failure. DEFAULT ON -- it is the fix for the forward 0.22 %
     * byte-alignment cascade. Set 0 for a same-binary A/B control: the drain
     * then behaves byte-for-byte as it did before Task 26 (rx_dphase can never
     * leave 0, so every carve address is the historical rx_dscan*pkt_bytes). */
    { const char *e = getenv("QPSK_RX_RESYNC");
      if (e && *e) rx_resync = (atoi(e) != 0);
      if (!rx_resync)
          fprintf(stderr, "RX carve re-anchor DISABLED (QPSK_RX_RESYNC=0)\n"); }
    /* QPSK_RX_RESYNC_LOG=1 (RXFIX Task 28): one stderr line per SUCCESSFUL
     * re-anchor, carrying the offset, the cursor phase before the move, the
     * slot within the transfer, the recovered seq and the number of scans that
     * failed since the previous hit. Default OFF: unset, rxr_log is 0 and the
     * drain path is byte-for-byte what it was. At the forward leg's ~0.24
     * re-anchors/s one fprintf per hit is free; do NOT enable it on a leg whose
     * link is unlocked (the acquisition phase scans thousands of garbage
     * slices, though those all FAIL and so print nothing). */
    { const char *e = getenv("QPSK_RX_RESYNC_LOG");
      rxr_log = (e && atoi(e) != 0);
      if (rxr_log)
          fprintf(stderr, "RX carve re-anchor per-hit log ENABLED "
                          "(QPSK_RX_RESYNC_LOG=1)\n"); }
    ...
    { const char *e = getenv("QPSK_FRAMELOG");
      if (e && *e) {
          ...
          /* QPSK_ROTFIX (RXFIX Task 35/38): the deferred-close rotate is the
           * DEFAULT since 2026-09-06 (operator decision after the Task 38 A/B:
           * the measured rotate stall predicted the collapse on 15/15 control
           * arms, the one control arm above the 7 ms threshold was the one that
           * collapsed, and every deferred arm ran 98-143 us against an inline
           * 375-12,849 us -- two orders of magnitude below the threshold).
           *   QPSK_ROTFIX unset or "1"  => deferred close (default)
           *   QPSK_ROTFIX="0"           => the historical inline rotate, kept as
           *                                the escape hatch and as the CTL arm of
           *                                any future A/B.
           * The mode line below is the arm-identity witness: a leg whose
           * qpsk_tun.log (RX *and* peer) does not show the expected mode is VOID,
           * not a data point. */
          { const char *r = getenv("QPSK_ROTFIX");
            framelog_rotfix = !(r && strcmp(r, "0") == 0);
            if (framelog_rotfix && framelog_preopen() != 0) {
                fprintf(stderr, "framelog: pre-open of %s.next FAILED (%s)"
                                " -- falling back to the inline rotate\n",
                        framelog_path, strerror(errno));
                framelog_rotfix = 0;
            }
            fprintf(stderr, "framelog: rotate mode = %s\n",
                    framelog_rotfix ? "deferred-close (QPSK_ROTFIX)" : "inline (legacy)"); }
      } }
