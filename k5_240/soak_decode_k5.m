function res = soak_decode_k5(capfile, varargin)
...
%
% Usage:
%   soak_decode_k5('/mnt/onetb/scratch/qpsk_variants/k5_240/air/linkA_k5_00.iq')
%   soak_decode_k5(file, 'label','A', 'diag',true)  % diag=true -> failing-frame breakdown
%
...
p.addParameter('csv',fullfile(KITDIR,'soak_results_k5.csv'),@(x)ischar(x)||isstring(x));
p.addParameter('cfomax',15e3,@isnumeric);   % coarse-CFO search half-range, Hz (nominal-LO quiet pair runs ~4.7 kHz)
p.parse(varargin{:});
label=char(p.Results.label); doDiag=p.Results.diag; csvfile=char(p.Results.csv);
cfoMax=p.Results.cfomax;
if isempty(label)
...
G=load(fullfile(KITDIR,'golden_k5.mat'));  % co-located: words,payload,coded,info,msgBits,capOut,trellis,TB,ROWS,COLS
...
if abs(f4)>0.9*cfoMax
  warning('soak_decode_k5:cfoEdge','4th-power CFO %.0f Hz is at the +-%.0f Hz mask edge -- raise ''cfomax''',f4,cfoMax);
end
...
for k=1:nF
  ...
  bestBER=1; bestCap=uint32(0); bestRot=NaN; bestSwap=NaN; gotGolden=false;
  for rr=[0 90 180 270]
    ...
    for sw=[false true]
      ...
      if co==capGolden, bestBER=ber; bestCap=co; bestRot=rr; bestSwap=sw; gotGolden=true; break; end
      if ber<bestBER, bestBER=ber; bestCap=co; bestRot=rr; bestSwap=sw; end
    end
    if gotGolden, break; end
  end
  ierr=round(bestBER*120);
...
% pi/4-QPSK data modulation), find the spectral peak; CFO = peak_freq/4.
...
% Constrain the search to +-cfomax (4th-power line at 4x) so far spurs cannot
% win. NOTE: the quiet pair runs NOMINAL LOs (~4.7 kHz XO offset) -- the legacy
% hardwired +-5 kHz sat AT that edge; caller passes 'cfomax' (default 15 kHz).
