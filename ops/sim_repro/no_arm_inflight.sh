...
arm_guard(){ if ps -eo args | grep -qE "^(/bin/bash|bash|/bin/sh) (/mnt/onetb/scratch/qpsk-jupiter-modem/ops/)?(bringup_r2r3|restore_known_good|capture_r3|soak_bidir)\.sh"; then
