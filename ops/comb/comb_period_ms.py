...
# a1r2 (ops/comb/runs/20260903_191410_legA_a1r2/cap/frames.bin) this tool
...
def _null(K, n_slots, pmin, pmax, n_shuffle, rng, pad=8, support=None):
    """95th pct of max-over-band R for uniformly random event sets of size K.

    `support` (RXFIX Task 44) is the array of slot indices an event COULD have
    fallen on. On a leg with a harness-recovered mid-window collapse the
    perturbed interval is excluded, so the real events cannot land there and a
    null that draws uniformly over the whole axis would be scored against a
    different support than the data -- and null95 gates the pre-registered
    comb_line verdict. None (every leg with no recovery.txt) => the whole axis,
    ...
    for _ in range(n_shuffle):
        idx = rng.choice(support, size=int(K), replace=False)
...
def analyze(path, pmin=24.0, pmax=44.0, singles_only=False, n_shuffle=8, seed=1234, pad=8):
    # honours_recovery=True: the exclusion is applied below, so common.py's
    # "this tool is scoring the perturbed interval as loss" notice would be wrong.
    fr = read_frames(path, honours_recovery=True)
    lt = loss_slot_trains(fr, path=path)
    rec = lt.get("recovery") or {"events": [], "excluded_s": 0.0}
    out = {"file": path, "usable": bool(lt.get("usable")),
           "recovery_events": len(rec.get("events", [])),
           "excluded_s": float(rec.get("excluded_s", 0.0))}
    if not lt.get("usable"):
        out["reason"] = lt.get("reason_recovery") or \
            "loss_slot_trains UNUSABLE (live window too short / wedged)"
    ...
    # THE DENOMINATOR. n_slots is the FFT/phase axis length and still spans the
    # perturbed interval; n_slots_scored is what was actually measured. They are
    # equal on every leg with no recovery record.
    n_scored = int(lt.get("n_slots_scored", n_slots))
    support = np.flatnonzero(lt["scored"]) if "scored" in lt and n_scored != n_slots else None
    ...
    null95, peaks = _null(K, n_slots, pmin, pmax, n_shuffle, np.random.default_rng(seed),
                          pad=pad, support=support)
    R32 = float(_R_at(onsets, [32.0])[0])
    out.update(
        n_slots=n_slots, n_slots_scored=n_scored, n_segments=int(lt.get("n_segments", 1)),
        n_lost=int(lost.size), n_events=K,
        live_end=float(lt["live_end"]), dur=float(lt["dur"]), wedged=bool(lt["wedged"]),
        per_pct=100.0 * lost.size / n_scored,
...
def main():
    ...
    for r in res:
        ...
        if r.get("recovery_events"):
            print(f"  PERTURBED LEG: {r['recovery_events']} harness-recovered mid-window "
                  f"collapse(s), {r['excluded_s']:.1f}s EXCLUDED -- events and denominator "
                  f"both drop those slots, and the random-event null draws only from the "
                  f"scored support")
        if not r["usable"]:
            print(f"  UNUSABLE: {r.get('reason')}")
            continue
        if r.get("n_slots_scored", r["n_slots"]) != r["n_slots"]:
            print(f"  scored slots={r['n_slots_scored']} of {r['n_slots']} on the axis, "
                  f"in {r.get('n_segments', 1)} clean segment(s)")
