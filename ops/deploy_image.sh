...
# boards. See docs/bringup.rst.
#
#   deploy_image.sh 10.0.0.170 A                     # current image for role A (images/CURRENT.txt)
...
W=wssh
# Default image = the current shipped lean build (see docs/provenance.rst). Override
# with $2 or $BOOT. Build one from source per docs/build-and-flash.rst if you have no artifact.
ROOT=$(cd "$D/.." && pwd)
BKG=$ROOT/images
case "${2:-}" in
  A|B)
