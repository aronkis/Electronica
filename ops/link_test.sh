...
# Every board call is bounded: SSH_T seconds (default 60) per anyssh call, so a
# password-prompt hang or a blocked register read fails loudly instead of hanging.
# Long calls set SSH_T inline (arm, ber, perf, iperf, ping sweeps, compiles).
SSH_T_DEFAULT=${SSH_T_DEFAULT:-60}
wssh(){ timeout "${SSH_T:-$SSH_T_DEFAULT}" "$DIR/anyssh.sh" "$@"; local rc=$?; [ $rc -eq 124 ] && echo "  [TIMEOUT] anyssh $1 exceeded ${SSH_T:-$SSH_T_DEFAULT}s" >&2; return $rc; }
W=wssh
scpput(){ SSH_ASKPASS=$DIR/askpass.sh SSH_ASKPASS_REQUIRE=force DISPLAY=:0 timeout "${SCP_T:-300}" setsid -w scp -o StrictHostKeyChecking=no -o PreferredAuthentications=password -o PubkeyAuthentication=no "$@" </dev/null 2>/dev/null; }
...
Subcommands:
  preflight   verify BOTH boards are ready (reachable; /root/host_app_k5/qpsk_tun
              executable; /root/lvds_1p92_mhz.{bin,json} + lvds_61p44_fdd_jupiter.{bin,json} present; lock_watchdog.sh
...
# ---------------------------------------------------------------------------
arm_ber(){ SSH_T=120 $W $1 "P=/sys/bus/iio/devices/iio:device2; DB=/sys/kernel/debug/iio/iio:device2
...
coldstart_tun(){ # $1 ip $2 txlo $3 rxlo $4 tunaddr $5 peer
  SSH_T=120 $W $1 "P=/sys/bus/iio/devices/iio:device2; DB=/sys/kernel/debug/iio/iio:device2
   cat /root/lvds_1p92_mhz.bin > \$P/stream_config 2>/dev/null; cat /root/lvds_1p92_mhz.json > \$P/profile_config 2>/dev/null; sleep 1
   echo calibrated > \$P/out_voltage1_ensm_mode; echo calibrated > \$P/in_voltage1_ensm_mode
   for g in 4 5 6 7; do echo 1 > \$DB/agpio\${g}_direction; echo 1 > \$DB/agpio\${g}_value; done; echo tx_a > \$P/out_voltage0_port_select
   echo $2 > \$P/out_altvoltage2_TX1_LO_frequency; echo 0 > \$P/out_voltage0_hardwaregain; echo rf_enabled > \$P/out_voltage0_ensm_mode
...
cmd_preflight(){
  ...
  for ip in "$A_IP" "$B_IP"; do
    ...
    local out
    # Host-side hard timeout (ssh/auth hang) + board-side timeouts on the register
    # reads (a debugfs direct_reg_access read can block forever on a board in a bad
    # state or on a non-modem image); both print a diagnosable line instead of hanging.
    echo "  (ssh to $ip ...)"
    out=$(SSH_T=60 $W $ip '
      echo reach=OK
      ...
      [ -f /root/lvds_61p44_fdd_jupiter.bin ] && [ -f /root/lvds_61p44_fdd_jupiter.json ] && echo lvds_r3=OK || echo lvds_r3=FAIL
      [ -f /root/lock_watchdog.sh ]     && echo watchdog=OK  || echo watchdog=FAIL
      DRA=/sys/kernel/debug/iio/iio:device0/direct_reg_access
      [ -e "$DRA" ] || echo "regaccess=NODEV (no /sys/kernel/debug/iio/iio:device0 -- debugfs not mounted, or not the modem image)"
      timeout 5 sh -c "echo enabled > /sys/bus/iio/devices/iio:device0/reg_access" 2>/dev/null
      rd(){ timeout 5 sh -c "echo $1 > $DRA && cat $DRA" 2>/dev/null; }
      CAP=$(rd 0x144); RST=$(rd 0x150); LVL=$(rd 0x15C)
      [ -n "$CAP" ] && echo regaccess=OK || echo regaccess=FAIL
      echo "regs cap=$CAP rstcs=$RST level=$LVL"
    ' 2>/dev/null); rc=$?
    [ "$rc" -eq 124 ] && echo "  [FAIL] TIMEOUT after 60 s (ssh/password prompt hang, or a blocked register read on the board) -- bisect with: ops/anyssh.sh $ip 'echo ok'"
    ...
    local chk
    for chk in qpsk_tun lvds_bin lvds_json lvds_r3 watchdog regaccess; do
...
cmd_tun(){
  ...
  echo "=== PING 146->148 (B->A over RF) ==="; SSH_T=90 $W $B_IP 'ping -c 10 -W 3 10.66.0.2 2>&1 | tail -4' 2>/dev/null
  echo "=== PING 148->146 (A->B over RF) ==="; SSH_T=90 $W $A_IP 'ping -c 10 -W 3 10.66.0.1 2>&1 | tail -4' 2>/dev/null
  if [ "$TEARDOWN" = 1 ]; then
...
cmd_ssh(){
 ...
 [ -f /etc/ssh/ssh_host_ed25519_key.pub ] && echo '  server ed25519 hostkey present'" 2>/dev/null
  ...
  local sshout
  sshout=$(SSH_T=200 $W $B_IP 'timeout 150 ssh -i /root/.ssh/id_ed25519 \
...
provision_perf(){
  ...
  for ip in "$A_IP" "$B_IP"; do
    ...
    SSH_T=240 $W $ip 'cd /root/host_app_k5 && gcc -O2 -Wall -o qpsk_perf qpsk_perf.c 2>&1 | head -3; ./qpsk_perf --selftest 2>&1 | tail -1' 2>/dev/null | sed "s/^/  $ip: /"
  done
}
...
udp_rung(){
  ...
  sleep 1
  SSH_T=$((dur+90)) $W $txip "/root/host_app_k5/qpsk_perf -c $rxtun -e -b $rate -l 88 -t $dur -p 5001 2>&1 | tail -3" 2>/dev/null | sed "s/^/  [$tag] /"
...
cmd_perf(){
  ...
  SSH_T=120 $W $B_IP "iperf3 -c $TUN_A -p 5201 -t 30 -J 2>/dev/null" 2>/dev/null > "$OUT/tcp_fwd.json" || echo "  TCP-CONTROL-FAIL fwd"
...
cmd_lat(){
  ...
  for s in 8 16 32 56 88 120 200; do
    ...
    SSH_T=$((N/2+60)) $W $B_IP "ping -c $N -i 0.2 -s $s -W 2 10.66.0.2 2>&1 | tail -3" 2>/dev/null | tee "$OUT/ping_fwd_s${s}.log"
    SSH_T=$((N/2+60)) $W $A_IP "ping -c $N -i 0.2 -s $s -W 2 10.66.0.1 2>&1 | tail -3" 2>/dev/null > "$OUT/ping_rev_s${s}.log"
  done
  for iv in 1.0 0.2 0.05; do
    echo "=== interval -i $iv -s32 (fwd) ==="
    SSH_T=$((N*2+60)) $W $B_IP "ping -c $N -i $iv -s 32 -W 2 10.66.0.2 2>&1 | tail -3" 2>/dev/null | tee "$OUT/ping_fwd_i${iv}.log"
  done
