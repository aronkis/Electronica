...
import io, os, re, shutil, sys, tempfile, unittest, zipfile
...
def _r4x_word_map(pfx, rh, dec):
    """Task 33: resolve, from the GENERATED Verilog text alone, which fields answer bytes
    0x234 and 0x238 for an R4D ('r4d') or R4E ('r4e') witness pair.

    Two facts are read off the text and combined the way the hardware combines them:
      * the 64-bit witness concatenation in Rate_Handle -- in Verilog the LEFTMOST
        element is the MOST significant, so it is bus word 1 ([63:32]) and the
        rightmost is bus word 0 ([31:0]);
      * the decoder's latch (word i <= bus[32*i +: 32]) and its read-mux INDEX RULE,
        i.e. which bus word answers word address 0x8D (byte 0x234) and 0x8E (0x238).
    Only the two index rules that have ever been generated are recognised; anything
    else is a failure, never a guess.  This is the check that, applied to the first
    cut, would have shown the 9acbe2ebe1db swap before it was flashed (test_166)."""
    m = re.search(r"assign %sWit = \{(.*?)\};" % pfx, rh, re.S)
    assert m, f'no {pfx}Wit assign in the Rate_Handle text'
    groups, depth, cur = [], 0, ''
    for ch in m.group(1):
        if ch == '{':
            depth += 1
        ...
        if ch == '}':
            depth -= 1
            if depth == 0:
                groups.append(re.sub(r'\s+', ' ', cur).strip())
                cur = ''
    assert len(groups) == 2, groups
    word = {1: groups[0], 0: groups[1]}          # leftmost = MSB = word 1
    assert f"{pfx}_reg[{pfx}_i] <= read_{pfx}_wit[32*{pfx}_i +: 32];" in dec, \
        'decoder latch is not word i <= bus[32*i +: 32]'
    if (f"{pfx}_reg[{pfx}_idx]" in dec
            and f"assign {pfx}_idx = (address_select_level1 == 8'h8E);" in dec):
        idx = {0x8D: 0, 0x8E: 1}                  # word index = address - 0x8D
    elif f"{pfx}_reg[address_select_level1[0]]" in dec:
        idx = {0x8D: 1, 0x8E: 0}                  # the first cut: address LSB
    else:
        raise AssertionError('unrecognised read-mux index rule in the decoder text')
    return {0x234: word[idx[0x8D]], 0x238: word[idx[0x8E]]}


R4D_CANON = {0x234: '{r4d_locked, r4d_skips, r4d_opens}', 0x238: "{16'b0, r4d_extras}"}
R4D_SILICON_9ACBE2EBE1DB = {0x234: R4D_CANON[0x238], 0x238: R4D_CANON[0x234]}
...
class TestR4D(TmpMixin):
    ...
    def test_133_two_read_words_at_0x234_and_0x238_leave_w1_alone(self):
        ...
        # Task 33: word index = address - 0x8D (r4d_idx), never the address LSB
        self.assertIn("assign r4d_idx = (address_select_level1 == 8'h8E);", both)
        self.assertIn('r4d_reg[r4d_idx]', both)
        self.assertNotIn('r4d_reg[address_select_level1[0]]', both)
        for keep in ("assign w1_hit = (address_select_level1 >= 8'h85) &&",
                     'reg [31:0] w1_reg [0:7];'):
            self.assertIn(keep, both)

    def _kit_decoder(self, variant):
        f = _kit_name('TxRxCompo_ip_addr_decoder.v')
        src = os.path.join(KIT_DIR, f)
        if not os.path.exists(src):
            self.skipTest('seqbist kit not present')
        p = self.write(f, open(src).read(), sub='dec')
        R.patch_ip_addr_decoder_w1(p)
        R.PATCHERS[variant]['TxRxCompo_ip_addr_decoder.v'](p)
        return open(p).read()

    def test_164_r4d_words_land_at_the_canonical_addresses(self):
        """Task 33 (controller ruling): 0x234 = {locked, skips, opens} -- R4B's layout on
        every image -- and 0x238 = {16'b0, extras}.  Resolved from the generated text
        of BOTH files the way the hardware resolves it (see _r4x_word_map)."""
        d = self._tree('s1', sub='rh')
        self.assertEqual(R.main(d, 'W1', sim_tree=True), 0)
        self.assertEqual(R.main(d, 'R4D', sim_tree=True), 0)
        rh = open(os.path.join(d, 'Rate_Handle.v')).read()
        dec = self._kit_decoder('R4D')
        self.assertEqual(_r4x_word_map('r4d', rh, dec), R4D_CANON)
        # and the comment above the assign says the same thing as the text does
        self.assertIn("word 0 = r4dWit[31:0]  = byte 0x234 = {locked, skips[15:0],", rh)
        self.assertIn("word 1 = r4dWit[63:32] = byte 0x238 = {16'b0, extras[15:0]}", rh)

    def test_166_the_resolver_reproduces_the_9acbe2ebe1db_swap(self):
        """Test of the test: the same resolver applied to the FIRST cut's index rule
        (address_select_level1[0]) must give exactly the swapped table Task 22 measured
        on image 9acbe2ebe1db -- 0x234 = {16'b0, extras}, 0x238 = {locked, skips,
        opens}.  If it did not, test_164 would prove nothing."""
        d = self._tree('s1', sub='rh')
        self.assertEqual(R.main(d, 'W1', sim_tree=True), 0)
        self.assertEqual(R.main(d, 'R4D', sim_tree=True), 0)
        rh = open(os.path.join(d, 'Rate_Handle.v')).read()
        dec = self._kit_decoder('R4D')
        old = dec.replace("  assign r4d_idx = (address_select_level1 == 8'h8E);\n\n", '')
        old = old.replace('r4d_reg[r4d_idx]', 'r4d_reg[address_select_level1[0]]')
        self.assertNotEqual(old, dec)
        self.assertEqual(_r4x_word_map('r4d', rh, old), R4D_SILICON_9ACBE2EBE1DB)
        # an index rule the resolver has never seen is a failure, not a guess
        with self.assertRaises(AssertionError):
            _r4x_word_map('r4d', rh, dec.replace('r4d_reg[r4d_idx]', 'r4d_reg[1]'))

    # ---- Task 33 fix1: the text-identity claim, made precise and pinned ----------
    PRE_TASK33_INJECTOR = '10b27b7'     # the injector commit that built image 9acbe2ebe1db

    @staticmethod
    def _code(txt):
        """A file's NON-COMMENT text: every `// ...` tail removed, blank lines dropped."""
        out = []
    ...
    def _old_injector(self):
        """The pre-Task-33 injector, loaded from git as a second module (skip if absent)."""
        import importlib.util
        import subprocess
        try:
            src = subprocess.run(['git', '-C', REPO, 'show',
                                  f'{self.PRE_TASK33_INJECTOR}:ops/skidfix/rxfix_inject.py'],
                                 capture_output=True, text=True, timeout=60)
        except (OSError, subprocess.TimeoutExpired):
            self.skipTest('git not available')
        if src.returncode != 0 or 'R4D_WIT_ASSIGN' not in src.stdout:
            self.skipTest(f'commit {self.PRE_TASK33_INJECTOR} not reachable')
        p = self.write('rxfix_inject_pre33.py', src.stdout, sub='old')
        spec = importlib.util.spec_from_file_location('rxfix_inject_pre33', p)
        m = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(m)
        return m

    def _snapshot(self, d):
        """Every .v file under d, loose or zip member, by relative name."""
        out = {}
            ...
            for f in files:
                ...
                if f.endswith('.zip'):
                    with zipfile.ZipFile(p) as z:
                        for m in z.namelist():
                            out[rel + '::' + m] = z.read(m).decode()
                elif f.endswith('.v'):
                    out[rel] = open(p).read()
        return out

    def _pre33_vs_now(self, build, apply):
        """Run `apply(module, dir)` with the pre-Task-33 injector and with this one on two
        copies of the tree `build(dir)` makes; return (old, new) snapshots."""
        old_mod = self._old_injector()
        snaps = []
        for tag, mod in (('pre33', old_mod), ('now', R)):
            d = os.path.join(self.d, tag)
            build(d)
            apply(mod, d)
            snaps.append(self._snapshot(d))
        return snaps

    def _simtree_at(self, d):
        os.makedirs(d, exist_ok=True)
        for f in sorted(set(R.r4d_files(True)) | set(R.w1_files(True)) | {'Preamble_Detector.v'}):
            src = os.path.join(S1_DIR, f)
            if not os.path.exists(src):
                self.skipTest(f'{src} not present')
            shutil.copy(src, os.path.join(d, f))

    def _kittree_at(self, d):
        """The shipped kit shape (TestR4B._kittree's layout) at an explicit directory."""
        srcs = {}
        for f in sorted(set(R.W1_RTL_FILES) | set(R.W1_IP_FILES)
                        | set(R.VARIANT_FILES['R4B']) | {'Preamble_Detector.v'}):
            src = os.path.join(KIT_DIR, _kit_name(f))
            if not os.path.exists(src):
                self.skipTest('seqbist kit not present')
            srcs[_kit_name(f)] = open(src).read()
        for sub in ('hdlsrc', 'ipcore/TxRxCompo_ip_v1_0/hdl',
            ...
            for n, txt in srcs.items():
                open(os.path.join(d, sub, n), 'w').write(txt)
        for sub in ('ipcore', 'vivado_ip_prj/ipcore'):
            with zipfile.ZipFile(os.path.join(d, sub, 'TxRxCompo_ip_v1_0.zip'), 'w') as z:
                for n, txt in srcs.items():
                    z.writestr('hdl/' + n, txt)
        open(os.path.join(d, 'p.xpr'), 'w').write('x')

    COMMENT_ONLY = {'QPSK_Rx.v', 'Rate_Handle.v', 'Receiver.v', 'TxRxComposite.v'}

    def test_167_task33_text_identity_is_exactly_as_stated(self):
        """The 'Rate_Handle text unchanged' claim, made precise (Task 33 fix1; W1_REGMAP
        sec 6-R4D.1, the (6) WITNESSES header above R4D_WIT_ASSIGN).  Against the injector
        that built image 9acbe2ebe1db:
          (a) the R4D+R1 --sim-tree (no W1 -- the shape the sim gate ran) is BYTE-IDENTICAL
              in every file, so the banked R4DR1 gate stands;
          (b) a W1+R4D --sim-tree differs in exactly Rate_Handle.v and the three witness
              carry wrappers, and in comment lines ONLY;
          (c) the kit-shaped W1+R4D+R1 tree differs in those same four files in comment
              lines only, plus TxRxCompo_ip_addr_decoder.v in code -- by r4d_idx alone.
        Anything more, or anything less, is a claim the map does not make."""
        # (a)
        old, new = self._pre33_vs_now(
            self._simtree_at,
            lambda m, d: (self.assertEqual(m.main(d, 'R4D', sim_tree=True), 0),
                          self.assertEqual(m.main(d, 'R1', sim_tree=True), 0)))
        self.assertEqual(sorted(old), sorted(new))
        self.assertEqual([k for k in old if old[k] != new[k]], [], 'R4D+R1 sim tree must be byte-identical')
        shutil.rmtree(os.path.join(self.d, 'pre33')); shutil.rmtree(os.path.join(self.d, 'now'))
        # (b)
        old, new = self._pre33_vs_now(
            self._simtree_at,
            lambda m, d: (self.assertEqual(m.main(d, 'W1', sim_tree=True), 0),
                          self.assertEqual(m.main(d, 'R4D', sim_tree=True), 0)))
        diff = sorted(k for k in old if old[k] != new[k])
        self.assertEqual(diff, sorted(self.COMMENT_ONLY))
        for k in diff:
            self.assertEqual(self._code(old[k]), self._code(new[k]), f'{k}: non-comment text changed')
        self.assertIn('assign r4dWit = {{16\'b0, r4d_extras},', new['Rate_Handle.v'])
        shutil.rmtree(os.path.join(self.d, 'pre33')); shutil.rmtree(os.path.join(self.d, 'now'))
        # (c)
        old, new = self._pre33_vs_now(
            self._kittree_at,
            lambda m, d: (self.assertEqual(m.main(d, 'W1'), 0), self.assertEqual(m.main(d, 'R4D'), 0),
                          self.assertEqual(m.main(d, 'R1'), 0)))
        self.assertEqual(sorted(old), sorted(new))
        diff = sorted(k for k in old if old[k] != new[k])
        self.assertTrue(diff, 'the kit tree must differ (by comments at least)')
        for k in diff:
            base = os.path.basename(k.split('::')[-1])
            if base == 'TxRxCompo_ip_addr_decoder.v':
                oc, nc = self._code(old[k]), self._code(new[k])
                self.assertNotEqual(oc, nc)
                self.assertIn('r4d_reg[address_select_level1[0]]', oc)
                self.assertIn('r4d_reg[r4d_idx]', nc)
                # exactly the r4d_idx lines account for the code difference
                norm = (nc.replace('  wire r4d_idx;\n', '')
                          .replace("  assign r4d_idx = (address_select_level1 == 8'h8E);\n", '')
                          .replace('r4d_reg[r4d_idx]', 'r4d_reg[address_select_level1[0]]'))
                self.assertEqual(norm, oc, f'{k}: the decoder differs by more than r4d_idx')
            else:
                self.assertIn(base, {_kit_name(f) for f in self.COMMENT_ONLY}, f'{k} unexpectedly differs')
                self.assertEqual(self._code(old[k]), self._code(new[k]), f'{k}: non-comment text changed')
        # every one of the five files differs in every one of its 5 copies (3 loose + 2 zip)
        bases = sorted({os.path.basename(k.split('::')[-1]) for k in diff})
        self.assertEqual(bases, sorted({_kit_name(f) for f in self.COMMENT_ONLY} | {'TxRxCompo_ip_addr_decoder.v'}))
        self.assertEqual(len(diff), 5 * 5)
...
class TestR4E(TmpMixin):
    ...
    def test_148_two_read_words_at_0x234_and_0x238_leave_w1_alone(self):
        ...
        self.assertIn("(address_select_level1 == 8'h8E);", both)
        self.assertIn("assign r4e_idx = (address_select_level1 == 8'h8E);", both)   # Task 33
        self.assertIn('r4e_reg[r4e_idx]', both)
        self.assertNotIn('r4e_reg[address_select_level1[0]]', both)
        for keep in ("assign w1_hit = (address_select_level1 >= 8'h85) &&",
                     'reg [31:0] w1_reg [0:7];'):
            self.assertIn(keep, both)

    def test_165_r4e_words_land_at_the_canonical_addresses(self):
        """Task 33: R4E shares R4D's decoder shape and had the same index rule, so it
        gets the same fix and the same resolver check (no R4E image was ever built, so
        no silicon exception exists for it)."""
        d = self._tree('s1', sub='rh')
        self.assertEqual(R.main(d, 'W1', sim_tree=True), 0)
        self.assertEqual(R.main(d, 'R4E', sim_tree=True), 0)
        rh = open(os.path.join(d, 'Rate_Handle.v')).read()
        ...
        R.patch_ip_addr_decoder_w1(p)
        R.patch_ip_addr_decoder_r4e(p)
        m = _r4x_word_map('r4e', rh, open(p).read())
        self.assertEqual(m[0x234], '{r4e_locked, r4e_skips, r4e_opens}')
        self.assertEqual(m[0x238], '{r4e_land_out, r4e_land_max, r4e_land_min, '
...
# ======================================================================== RXFIX_BS
# Task 46: the byte-seam census.  The tests that matter here are the ADDRESS-MAP ones
# (test_170/171): W1_REGMAP sec 6-R4D.2 records that image 9acbe2ebe1db returned two
# words swapped because the read-mux index rule was wrong, and BS has EIGHT words on a
# range decode, where the plausible failure is a ROTATION -- every address returns a
# real counter, just the wrong one.  A rotation survives every fail-closed field check
# the reader can make (see test_171), so the map is resolved from the GENERATED text of
# both files, the way the hardware resolves it, and nothing is taken on intent.
BRAM_FIFO = os.path.join(REPO, 'jupiter_240k5_byte/rxfifo_bram/ByteRxFifo.v')

# The canonical table, written out LONGHAND on purpose.  If this is ever derived from
# the same arithmetic the RTL uses, the test stops being a test.
BS_CANON = {
    0x23C: 'cWords',
    0x240: 'cStarts',
    0x244: 'cPush',
    ...
    0x24C: 'cDrop',
    0x250: '{cLasts, cMarkP}',
    0x254: '{tLast, tMin, tMax, dropMax}',
    0x258: "{cTrunc, q24, 8'b0}",
}
# What the "forgot the -7" cut would return: bs_reg[address_select_level1[2:0]], i.e.
# address 0x8F (LSBs 7) -> bus word 7, 0x90 (LSBs 0) -> bus word 0, ...  A rotation.
BS_ROT_FORGOT7 = {0x23C: BS_CANON[0x258]}
BS_ROT_FORGOT7.update({4 * (0x8F + i + 1): BS_CANON[4 * (0x8F + i)] for i in range(7)})


def _bs_word_map(comp, dec):
    """Task 46: resolve, from the GENERATED Verilog text alone, which census field
    answers each of the eight byte addresses 0x23C..0x258.

    Three facts are read off the text and combined the way the hardware combines them:
      * the 8-element `assign bus = {...}` concatenation in bs_seam_census -- in
        Verilog the LEFTMOST element is the MOST significant, so it is bus word 7 and
        the rightmost is bus word 0;
      * each shadow register's source (`sN <= wM;`) and each `wire [31:0] wM = EXPR;`,
        which is where the FIELD NAMES actually live;
      * the decoder's latch (word i <= bus[32*i +: 32]) and its read-mux INDEX RULE.
    Only index rules that have actually been written are recognised; anything else is a
    failure, never a guess."""
    m = re.search(r"assign bus = \{(.*?)\};", comp, re.S)
    assert m, 'no `assign bus` in the bs_seam_census text'
    order = [x.strip() for x in m.group(1).split(',')]
    assert len(order) == 8, order
    shadow = {7 - i: order[i] for i in range(8)}      # leftmost = MSB = word 7
    expr = {}
    for w in range(8):
        s = shadow[w]
        mm = re.search(r"\b%s <= (w\d);" % re.escape(s), comp)
        assert mm, f'shadow {s} has no `<= wN` source'
        me = re.search(r"wire \[31:0\] %s = (.*?);" % re.escape(mm.group(1)), comp, re.S)
        assert me, f'{mm.group(1)} has no `wire [31:0] wN = ...` definition'
        expr[w] = re.sub(r'\s+', ' ', me.group(1)).strip()
    assert "bs_reg[bs_i] <= read_bs_bus[32*bs_i +: 32];" in dec, \
        'decoder latch is not word i <= bus[32*i +: 32]'
    if ("bs_reg[bs_idx]" in dec
            and "assign bs_idx = address_select_level1[2:0] - 3'd7;" in dec):
        idx = {0x8F + i: i for i in range(8)}          # word index = address - 0x8F
    elif "bs_reg[address_select_level1[2:0]]" in dec:
        idx = {0x8F + i: (0x8F + i) & 7 for i in range(8)}   # the "forgot the -7" cut
    else:
        raise AssertionError('unrecognised read-mux index rule in the decoder text')
    return {4 * a: expr[idx[a]] for a in idx}


class TestBS(TmpMixin):
    """RXFIX_BS: the byte-seam census instrument (Task 46)."""

    def _simtree(self, sub=''):
        """s1_rtl's three BS core files, with the v5 BRAM drop-in ByteRxFifo -- which is
        what every build tree in this lineage carries and what BS requires."""
        for f in R.bs_files(True):
            src = BRAM_FIFO if f == 'ByteRxFifo.v' else os.path.join(S1_DIR, f)
            if not os.path.exists(src):
                self.skipTest(f'{src} not present')
            self.write(f, open(src).read(), sub=sub)
        return os.path.join(self.d, sub) if sub else self.d

    def _kitsrcs(self):
        srcs = {}
        for f in sorted(set(R.W1_RTL_FILES) | set(R.W1_IP_FILES)
                        | set(R.VARIANT_FILES['R4B']) | set(R.VARIANT_FILES['BS'])):
            if f == 'ByteRxFifo.v':
                srcs[_kit_name(f)] = open(BRAM_FIFO).read().replace(
                    'module ByteRxFifo ', 'module TxRxCompo_ip_src_ByteRxFifo ')
                continue
            src = os.path.join(KIT_DIR, _kit_name(f))
            if not os.path.exists(src):
                self.skipTest('seqbist kit not present')
    ...
    def _kittree(self, root='kit'):
        """The shipped kit shape (3 loose mirrors + 2 zips) under its OWN root, so a
        sim-tree copy made by the same test is not counted as a fourth mirror."""
        srcs = self._kitsrcs()
        for sub in ('hdlsrc', 'ipcore/TxRxCompo_ip_v1_0/hdl',
                    'vivado_ip_prj/ipcore/TxRxCompo_ip_v1_0/hdl'):
            for n, txt in srcs.items():
                self.write(n, txt, sub=os.path.join(root, sub))
        for sub in ('ipcore', 'vivado_ip_prj/ipcore'):
            os.makedirs(os.path.join(self.d, root, sub), exist_ok=True)
            with zipfile.ZipFile(os.path.join(self.d, root, sub,
                                              'TxRxCompo_ip_v1_0.zip'), 'w') as z:
                for n, txt in srcs.items():
                    z.writestr('hdl/' + n, txt)
        open(os.path.join(self.d, root, 'p.xpr'), 'w').write('x')
        return os.path.join(self.d, root)

    def _kit_decoder(self, *variants):
        """Apply the given variants, in order, to a kit copy of the decoder chain and
        return the addr_decoder text."""
        srcs = self._kitsrcs()
        for n, txt in srcs.items():
            self.write(n, txt, sub='dec')
        for v in variants:
            for f in R.VARIANT_FILES[v]:
                p = os.path.join(self.d, 'dec', _kit_name(f))
                if os.path.exists(p):
                    R.PATCHERS[v][f](p)
        return open(os.path.join(self.d, 'dec', 'TxRxCompo_ip_addr_decoder.v')).read()

    # ---- registration -------------------------------------------------------------
    def test_168_bs_is_registered_in_every_dispatch_table(self):
        for t in (R.VARIANT_FILES, R.PATCHERS, R.FILE_MARKER):
            self.assertIn('BS', t)
        self.assertEqual(len(R.BS_CORE_FILES), 3)
        self.assertEqual(len(R.BS_IP_FILES), 4)
        self.assertEqual(set(R.VARIANT_FILES['BS']),
                         set(R.BS_CORE_FILES) | set(R.BS_IP_FILES))
        self.assertEqual(R.bs_files(True), R.BS_CORE_FILES)
        self.assertEqual(len(R.bs_files(False)), 7)
        # the marker must not collide with any other variant's, in either direction
        for m in (R.MARKER, R.MARKER_R2, R.MARKER_R3, R.MARKER_R3S, R.MARKER_R4,
                  R.MARKER_R4B, R.MARKER_R4D, R.MARKER_R4E, R.MARKER_W1):
            self.assertFalse(R._has('x ' + R.MARKER_BS + ' y', m), m)
            self.assertFalse(R._has('x ' + m + ' y', R.MARKER_BS), m)

    def test_169_bs_patches_both_lineages_and_is_idempotent(self):
        d = self._simtree()
        self.assertEqual(R.main(d, 'BS', sim_tree=True), 0)
        for f in R.bs_files(True):
            self.assertIn('RXFIX_BS', open(os.path.join(d, f)).read(), f)
        self.assertEqual(R.main(d, 'BS', sim_tree=True), 0)   # idempotent
        # kit lineage, W1 and R4B first (BS wraps their data_read)
        k = self._kittree()
        for v in ('W1', 'R4B', 'BS'):
            self.assertEqual(R.main(k, v), 0, v)

    # ---- THE ADDRESS MAP ----------------------------------------------------------
    def test_170_bs_words_land_at_the_canonical_addresses(self):
        """0x23C..0x258 = words, starts, push, pop, drop, marks, evt, cnt -- resolved
        from the generated text of BOTH files the way the hardware resolves it."""
        d = self._simtree(sub='c')
        self.assertEqual(R.main(d, 'BS', sim_tree=True), 0)
        comp = open(os.path.join(d, 'TxRxComposite.v')).read()
        dec = self._kit_decoder('W1', 'R4B', 'BS')
        self.assertEqual(_bs_word_map(comp, dec), BS_CANON)
        # and the comment above the concatenation says the same thing the text does
        for i, name in enumerate(['BS_WORDS', 'BS_STARTS', 'BS_PUSH', 'BS_POP',
                                  'BS_DROP', 'BS_MARKS', 'BS_EVT', 'BS_CNT']):
            self.assertIn('byte 0x%X = %s' % (4 * (0x8F + i), name), comp)
        # the table the injector exports for the readers is the same table
        self.assertEqual(sorted(R.bs_word_map()), sorted(BS_CANON))

    def test_171_the_resolver_catches_the_rotation_the_field_checks_cannot(self):
        """Test of the test.  The plausible BS cut error is not a swap but a ROTATION:
        `bs_reg[address_select_level1[2:0]]` (the -7 forgotten) makes every address
        return a real counter, one word off.  The same resolver applied to that index
        rule must report exactly the rotated table.  It must also be shown that the
        reader's fail-closed field checks would NOT catch it -- which is why the
        resolver exists."""
        d = self._simtree(sub='c')
        self.assertEqual(R.main(d, 'BS', sim_tree=True), 0)
        comp = open(os.path.join(d, 'TxRxComposite.v')).read()
        dec = self._kit_decoder('W1', 'R4B', 'BS')
        bad = dec.replace("  assign bs_idx = address_select_level1[2:0] - 3'd7;\n\n", '')
        bad = bad.replace('bs_reg[bs_idx]', 'bs_reg[address_select_level1[2:0]]')
        self.assertNotEqual(bad, dec)
        self.assertEqual(_bs_word_map(comp, bad), BS_ROT_FORGOT7)
        # THE POINT: under the rotation, byte 0x258 (whose reserved low byte the reader
        # checks for zero) returns BS_EVT, whose low byte is bs_dropmax -- zero on any
        # leg with no FIFO overflow.  The fail-closed check passes and the rotation
        # goes unnoticed.  Only this resolver sees it.
        self.assertEqual(BS_ROT_FORGOT7[0x258], BS_CANON[0x254])
        self.assertTrue(BS_CANON[0x254].endswith('dropMax}'))
        # an index rule the resolver has never seen is a failure, not a guess
        with self.assertRaises(AssertionError):
            _bs_word_map(comp, dec.replace('bs_reg[bs_idx]', 'bs_reg[3]'))

    # ---- the register contract ----------------------------------------------------
    def test_172_bs_cnt_reserved_byte_is_hard_zero_and_evt_bytes_are_bounded(self):
        d = self._simtree(sub='c')
        self.assertEqual(R.main(d, 'BS', sim_tree=True), 0)
        comp = open(os.path.join(d, 'TxRxComposite.v')).read()
        self.assertIn("wire [31:0] w7 = {cTrunc, q24, 8'b0};", comp)
        # the three order statistics can only ever hold a state_wordCnt in 1..190
        self.assertIn("wire       trunc_ev = sstart && (wcnt >= 16'd1) && (wcnt <= 16'd190);",
                      comp)
        self.assertIn("tMin <= 8'd191;", comp)   # min resets high, max resets low
        self.assertIn("tMax <= 8'd0;", comp)

    def test_173_bs_adds_no_toplevel_port_and_touches_no_write_decode(self):
        k = self._kittree()
        for v in ('W1', 'R4B', 'BS'):
            self.assertEqual(R.main(k, v), 0, v)
        top = open(os.path.join(k, 'hdlsrc', 'TxRxCompo_ip.v')).read()
        # the census bus is an INTERNAL wire in TxRxCompo_ip: component.xml unchanged
        self.assertIn('wire [255:0] bs_bus_sig;', top)
        _, j = R._span(top, r'module\s+TxRxCompo_ip\s*(?:#\s*\([^()]*\)\s*)?\(',
                       'ip top ports')
        self.assertNotIn('bs_bus', top[:j])
        dec = open(os.path.join(k, 'hdlsrc', 'TxRxCompo_ip_addr_decoder.v')).read()
        # BS adds a READ decode only: not one addr_write literal is introduced, so the
        # write-only registers 0x4/0x10C/0x110/0x114/0x118/0x138/0x158/0x170/0x174/
        # 0x178/0x17C/0x180/0x184/0x1DC/0x208 are untouched by construction.
        base = open(os.path.join(KIT_DIR, 'TxRxCompo_ip_addr_decoder.v')).read()
        self.assertEqual(dec.count('addr_write'), base.count('addr_write'))
        # ... and the two DBGCAP/TXCAP read decodes survive
        for c in ("8'b10000011", "8'b10000100"):
            self.assertIn(c, dec)

    def test_174_bs_leaves_the_w1_and_r4b_words_alone(self):
        """W1's eight words and R4B's ninth are byte-identical with and without BS; the
        only line BS rewrites is the single `assign data_read` -- W1_REGMAP sec 5.1's
        rule, applied one variant further out."""
        a = self._kit_decoder('W1', 'R4B')
        b = self._kit_decoder('W1', 'R4B', 'BS')
        keep = [l for l in a.splitlines(True) if 'assign data_read' not in l]
        for l in keep:
            self.assertIn(l, b, l[:60])
        self.assertIn('assign bs_under = (r4b_hit ? r4b_reg :', b)
        self.assertIn('assign data_read = (bs_hit ? bs_reg[bs_idx] : bs_under);', b)
        self.assertEqual(b.count('assign data_read'), 1)

    def test_175_bs_hit_range_is_0x8F_to_0x96_and_nothing_below_it(self):
        dec = self._kit_decoder('W1', 'R4B', 'BS')
        self.assertIn("assign bs_hit = (address_select_level1 >= 8'h8F) &&", dec)
        self.assertIn("(address_select_level1 <= 8'h96);", dec)
        # W1 0x85..0x8C and R4B 0x8D are below it and still decoded by their own hits
        self.assertIn("assign w1_hit = (address_select_level1 >= 8'h85) &&", dec)
        self.assertIn("assign r4b_hit = (address_select_level1 == 8'h8D);", dec)

    # ---- the taps -----------------------------------------------------------------
    def test_176_the_fifo_taps_are_the_drop_ins_own_wires(self):
        d = self._simtree(sub='c')
        self.assertEqual(R.main(d, 'BS', sim_tree=True), 0)
        f = open(os.path.join(d, 'ByteRxFifo.v')).read()
        for a in ('assign bsPush   = push;', 'assign bsPop    = pop;',
                  'assign bsDrop   = drop;', 'assign bsEnbG   = enb_gated;'):
            self.assertIn(a, f)
        # read-only: not one existing assignment is redefined
        base = open(BRAM_FIFO).read()
        for l in base.splitlines():
            if l.strip().startswith('assign ') or l.strip().startswith('wire '):
                self.assertIn(l, f, l[:60])

    def test_177_bs_refuses_the_generated_64_deep_fifo_by_name(self):
        """Deviation (4): patching both ByteRxFifo forms would make the sim gate test a
        tap expression the build does not ship.  The generated module is refused, with
        the fix named."""
        self.write('ByteRxFifo.v', open(os.path.join(S1_DIR, 'ByteRxFifo.v')).read())
        with self.assertRaises(AssertionError) as cm:
            R.patch_byte_rx_fifo_bs(os.path.join(self.d, 'ByteRxFifo.v'))
        self.assertIn('rxfifo_inject.sh', str(cm.exception))

    def test_178_the_serializer_taps_are_read_only_and_gated_by_the_module(self):
        d = self._simtree(sub='c')
        self.assertEqual(R.main(d, 'BS', sim_tree=True), 0)
        s = open(os.path.join(d, 'ByteSerializer.v')).read()
        for a in ('assign bsWv = wv;', 'assign bsWl = wl;', 'assign bsStart = start;',
                  'assign bsWordCnt = state_wordCnt;',
                  'assign bsEnb = enb_1_2_0_gated;'):
            self.assertIn(a, s)
        base = open(os.path.join(S1_DIR, 'ByteSerializer.v')).read()
        for l in base.splitlines():
            if l.strip().startswith('assign '):
                self.assertIn(l, s, l[:60])

    def test_179_freeze_is_fixctl4_on_the_kit_lineage_and_tied_on_the_sim_lineage(self):
        d = self._simtree(sub='c')
        self.assertEqual(R.main(d, 'BS', sim_tree=True), 0)
        sim = open(os.path.join(d, 'TxRxComposite.v')).read()
        self.assertIn("assign bs_freeze = 1'b0;", sim)
        k = self._kittree()
        for v in ('W1', 'R4B', 'BS'):
            self.assertEqual(R.main(k, v), 0, v)
        kit = open(os.path.join(k, 'hdlsrc',
                                'TxRxCompo_ip_src_TxRxComposite.v')).read()
        self.assertIn('assign bs_freeze = fixctl[4];', kit)
        # ... the SAME bit W1 uses, so one freeze write covers both censuses
        qr = open(os.path.join(k, 'hdlsrc', 'TxRxCompo_ip_src_QPSK_Rx.v')).read()
        self.assertIn('assign w1_freeze = fixctl[4];', qr)

    def test_180_the_shadow_is_latched_on_every_clk_not_on_an_enb_tick(self):
        """BS spans two clock ENABLES on one clock, so latching the shadow on a clk edge
        (not an enb tick, as rh_w1_census does) makes one frozen sweep coherent across
        both to a single edge.  That is a deliberate difference from W1 and the reason
        the byte-plane and symbol-plane counts are comparable in one window."""
        d = self._simtree(sub='c')
        self.assertEqual(R.main(d, 'BS', sim_tree=True), 0)
        comp = open(os.path.join(d, 'TxRxComposite.v')).read()
        body = comp[comp.index('module bs_seam_census'):]
        m = re.search(r"if \( ~freeze\) begin\n(.*?)\n      end", body, re.S)
        self.assertIsNotNone(m)
        # the shadow block must NOT sit inside either enable
        pre = body[:m.start()]
        self.assertEqual(pre.count('if (enbSer) begin'), 1)
        self.assertEqual(pre.count('if (enbFifo) begin'), 1)
        self.assertLess(pre.rindex('end'), m.start())

    def test_181_bs_then_w1_fails_loudly_on_the_decoder(self):
        """BS wraps the single `assign data_read`, so W1 applied AFTERWARDS can no
        longer find its own literal anchor.  It must fail, not silently no-op."""
        srcs = self._kitsrcs()
        for n, txt in srcs.items():
            self.write(n, txt, sub='rev')
        p = os.path.join(self.d, 'rev', 'TxRxCompo_ip_addr_decoder.v')
        self.assertEqual(R.patch_ip_addr_decoder_bs(p), 'patched')
        with self.assertRaises(AssertionError):
            R.patch_ip_addr_decoder_w1(p)

    def test_182_bs_composes_with_w1_alone_and_with_w1_r4d(self):
        for chain in (('W1',), ('W1', 'R4B'), ('W1', 'R4D')):
            dec = self._kit_decoder(*(chain + ('BS',)))
            self.assertEqual(dec.count('assign data_read'), 1)
            self.assertIn('assign data_read = (bs_hit ? bs_reg[bs_idx] : bs_under);', dec)
            comp = open(os.path.join(self.d, 'dec',
                                     'TxRxCompo_ip_src_TxRxComposite.v')).read()
            self.assertIn('u_bs_seam_census', comp)
            shutil.rmtree(os.path.join(self.d, 'dec'))


# ======================================================================== RXFIX_PAD
class TestPAD(TmpMixin):
    """RXFIX_PAD (F4): truncated-frame padding in the ByteSerializer; stacks on BS."""

    def _bs_ser(self):
        src = os.path.join(S1_DIR, 'ByteSerializer.v')
        if not os.path.exists(src):
            self.skipTest(f'{src} not present')
        p = self.write('ByteSerializer.v', open(src).read())
        self.assertEqual(R.patch_byte_serializer_bs(p), 'patched')
        return p

    def test_01_registered(self):
        self.assertEqual(R.VARIANT_FILES['PAD'], ['ByteSerializer.v'])
        self.assertIn('PAD', R.PATCHERS)
        self.assertEqual(R.FILE_MARKER['PAD'], {'ByteSerializer.v': 'RXFIX_PAD'})

    def test_02_patched_and_idempotent(self):
        p = self._bs_ser()
        self.assertEqual(R.patch_byte_serializer_pad(p), 'patched')
        s = open(p).read()
        self.assertIn('RXFIX_PAD', s)
        self.assertIn("pad_1 = 8'd191 - state_wordCnt_1[7:0];", s)
        self.assertEqual(R.patch_byte_serializer_pad(p), 'already')
        self.assertEqual(open(p).read(), s)

    def test_03_refuses_without_bs(self):
        src = os.path.join(S1_DIR, 'ByteSerializer.v')
        if not os.path.exists(src):
            self.skipTest(f'{src} not present')
        p = self.write('ByteSerializer.v', open(src).read())
        with self.assertRaises(AssertionError):
            R.patch_byte_serializer_pad(p)

    def test_04_matches_banked_twin(self):
        """rxfix_inject PAD and rtl_sim/pad_patch.py must produce byte-identical RTL."""
        twin = os.path.join(os.path.dirname(os.path.abspath(R.__file__)),
                            '..', '..', 'jupiter_240k5_byte', 'rtl_sim', 'pad_patch.py')
        if not os.path.exists(twin):
            self.skipTest('pad_patch.py not present')
        a = self._bs_ser(); b = self.write('ByteSerializer_twin.v', open(a).read())
        R.patch_byte_serializer_pad(a)
        import importlib.util
        spec = importlib.util.spec_from_file_location('pad_patch', twin)
