...
FRESH=$ROOT/jupiter_byte_tgenrx_build
...
echo "TGENRX_BUILD start $(date -Is) host=$(hostname) pid=$$ ppid=$PPID"
...
rsync -a --exclude 'hdl_prj_*' --exclude 'slprj' --exclude '*.log' --exclude '*.out' \
  --exclude 'rtl_sim/obj_*' --exclude 's1_rtl' "$BASE/" "$FRESH/" || { echo TGENRX_BUILD_FAILED rsync; exit 1; }
for f in "$FRESH"/*.m; do sed -i "s#$BASE#$FRESH#g" "$f"; done
cp "$KITRTL/qpsk_traffic_gen.v" "$KITRTL/qpsk_traffic_gen_rx.v" "$KITRTL/tx_seam_checker.v" "$FRESH/" || { echo TGENRX_BUILD_FAILED cp; exit 1; }

echo "=== [2/6] patch complete_byte_t8.tcl (traffic-gen splice) ==="
python3 "$SRC/patch_tgen_tcl.py" "$FRESH/complete_byte_t8.tcl" || { echo TGENRX_BUILD_FAILED patch; exit 1; }
grep -q 'TGEN_RX_WIRE_OK' "$FRESH/complete_byte_t8.tcl" || { echo TGENRX_BUILD_FAILED patch-verify; exit 1; }
...
[ -f "$Z/vivado_prj.xpr" ] || { echo "TGENRX_BUILD_FAILED: no vivado project"; tail -20 "$FRESH/build_byte_matlab.log"; exit 1; }
[ -f "$Z/ipcore/TxRxCompo_ip_v1_0.zip" ] || { echo "TGENRX_BUILD_FAILED: no ipcore zip"; exit 1; }
grep -q 'byte_breakout' "$Z/vivado_prj.srcs/sources_1/bd/system/system.bd" 2>/dev/null \
  || { echo "TGENRX_BUILD_FAILED: BD has no byte_breakout"; exit 1; }
...
grep -q 'TGEN_WIRE_OK' "$FRESH/build_byte_vivado.log" || { echo "TGENRX_BUILD_FAILED: TX splice never confirmed"; exit 1; }
grep -q 'TGEN_RX_WIRE_OK' "$FRESH/build_byte_vivado.log" || { echo "TGENRX_BUILD_FAILED: RX splice never confirmed"; exit 1; }
grep -q 'TXCHK_WIRE_OK' "$FRESH/build_byte_vivado.log" || { echo "TGENRX_BUILD_FAILED: TX checker splice never confirmed"; exit 1; }
B=$Z/boot/BOOT.BIN
[ -f "$B" ] || { echo "TGENRX_BUILD_FAILED: no BOOT.BIN"; exit 1; }
...
echo "TGENRX_BUILD_DONE md5=$MD5 image=$B $(date -Is)"
