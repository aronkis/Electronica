...
W=$D/anyssh.sh
A=10.0.0.148
BB=$ROOT/jupiter_byte_beatobs_build/hdl_prj_jupiter_composite/vivado_ip_prj/boot/BOOT.BIN
...
rollback(){
  ...
  wait_back || { echo "FLASH_BEATOBS_FATAL: 148 not back after rollback -- PHYSICAL ATTENTION (backup still at /root/BOOT.BIN.e49c011b.bak)"; exit 2; }
...
[ -f "$BB" ] || { echo "FLASH_BEATOBS_FATAL: no image at $BB"; exit 1; }
md5sum "$BB" | grep -q "^${EXP}" || { echo "FLASH_BEATOBS_FATAL: image md5 != ${EXP}"; exit 1; }
$W $A 'md5sum /root/BOOT.BIN.e49c011b.bak' 2>/dev/null | grep -q '^e49c011b' \
  || { echo "FLASH_BEATOBS_FATAL: rollback backup missing/bad on 148"; exit 1; }
...
scpput "$BB" root@$A:/boot/BOOT.BIN.new || { echo "FLASH_BEATOBS_FATAL: scp failed"; exit 1; }
$W $A "md5sum /boot/BOOT.BIN.new | grep -q ^${EXP} || exit 1
  ...
  || { echo "FLASH_BEATOBS_FATAL: stage verify failed -- /boot/BOOT.BIN untouched"; exit 1; }
echo "  staged; rebooting 148"
$W $A 'sync; ( sleep 2; reboot ) >/dev/null 2>&1 & exit 0' 2>/dev/null
wait_back || { echo "FLASH_BEATOBS_FATAL: 148 not back after 10 min"; exit 2; }
