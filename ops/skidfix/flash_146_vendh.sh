#!/bin/bash
# flash_146_vendh.sh <md5-12> -- FIRST 146 FLASH OF THE CAMPAIGN (freeze lifted
...
#   - rollback backup created ON 146 (/root/BOOT.BIN.4be9286c.bak) by copying
#     the live /boot/BOOT.BIN before staging, verified against the nemo bank
#     jupiter_byte_tmr146_build/hdl_prj_jupiter_composite/vivado_ip_prj/boot/BOOT.BIN (md5 433fd8dab393)
...
A=10.0.0.146
RX=10.0.0.148
BB=$ROOT/jupiter_byte_tmr146_gates/variants_placement/v_endh/BOOT.BIN
BAK_MD5=4be9286ca111
EXP=${1:?usage: flash_146_vendh.sh <md5-12>}
echo "FLASH_146V start $(date -Is) pid=$$"
...
rollback(){
  ...
  $W $A 'cp /root/BOOT.BIN.4be9286c.bak /boot/BOOT.BIN && sync && ( sleep 2; reboot ) >/dev/null 2>&1 & exit 0' 2>/dev/null
  wait_back || { echo "FLASH_146V_FATAL: 146 not back after rollback -- PHYSICAL ATTENTION (backups: /root/BOOT.BIN.4be9286c.bak on 146 AND jupiter_byte_tmr146_build/hdl_prj_jupiter_composite/vivado_ip_prj/boot/BOOT.BIN on nemo)"; exit 2; }
  ...
  echo "FLASH_146V_ROLLED_BACK $(date -Is)"
  exit 1
}

echo "=== [1/6] preconditions ==="
[ -f "$BB" ] || { echo "FLASH_146V_FATAL: no image at $BB"; exit 1; }
md5sum "$BB" | grep -q "^${EXP}" || { echo "FLASH_146V_FATAL: image md5 != ${EXP}"; exit 1; }
[ -f "$ROOT/jupiter_byte_tmr146_build/hdl_prj_jupiter_composite/vivado_ip_prj/boot/BOOT.BIN" ] \
  || { echo "FLASH_146V_FATAL: nemo-side bank missing (A0 gate)"; exit 1; }
...
RXHP=$(timeout 90 bash "$D/health_probe_reset_aware.sh" $RX 12 2>/dev/null | tail -1)
echo "  148 pre-flash health: ${RXHP:-NO RESPONSE}"
echo "$RXHP" | grep -qE 'fsync=[0-9]+' || { echo "FLASH_146V_FATAL: 148 (gate instrument) unreachable/unhealthy -- fix 148 first"; exit 1; }
CUR=$($W $A 'md5sum /boot/BOOT.BIN | cut -c1-12' 2>/dev/null)
echo "  146 current image: $CUR (expect ${BAK_MD5})"
[ "$CUR" = "$BAK_MD5" ] || { echo "FLASH_146V_FATAL: running image is not ${BAK_MD5} -- identity in question, STOP"; exit 1; }

echo "=== [2/6] on-board rollback backup, then stage + flash ==="
$W $A "cp /boot/BOOT.BIN /root/BOOT.BIN.4be9286c.bak && sync
  md5sum /root/BOOT.BIN.4be9286c.bak | grep -q ^${BAK_MD5} && echo BAK_OK" 2>/dev/null | grep -q BAK_OK \
  || { echo "FLASH_146V_FATAL: on-board backup create/verify failed"; exit 1; }
echo "  on-board backup verified (/root/BOOT.BIN.4be9286c.bak)"
scpput "$BB" root@$A:/boot/BOOT.BIN.new || { echo "FLASH_146V_FATAL: scp failed"; exit 1; }
$W $A "md5sum /boot/BOOT.BIN.new | grep -q ^${EXP} || exit 1
  ...
  || { echo "FLASH_146V_FATAL: stage verify failed -- /boot/BOOT.BIN untouched"; exit 1; }
echo "  staged; rebooting 146"
$W $A 'sync; ( sleep 2; reboot ) >/dev/null 2>&1 & exit 0' 2>/dev/null
wait_back || { echo "FLASH_146V_FATAL: 146 not back after 10 min -- PHYSICAL ATTENTION"; exit 2; }
...
[ "$NAK" = "4" ] || { echo "FLASH_146V_NAK_FAIL"; rollback; }
...
[ -n "$gate_ok" ] || { echo "FLASH_146V_HEALTH_FAIL fsync=${FS:-?} wcnt=${WC:-?} after re-bring-up"; rollback; }

echo "=== [6/6] (no witness gpio on the TMR lineage -- step intentionally empty) ==="
echo "FLASH_146V_DONE $(date -Is) image=${EXP} on 146, bring-up + gates green"
