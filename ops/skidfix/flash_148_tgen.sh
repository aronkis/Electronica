#!/bin/bash
# flash_148_tgen.sh -- flash 148 with the tgen (traffic-generator) image,
...
W=$D/anyssh.sh
A=10.0.0.148
BB=$ROOT/jupiter_byte_tgen_build/hdl_prj_jupiter_composite/vivado_ip_prj/boot/BOOT.BIN
...
echo "=== [6/6] tgen config gpio readback (0x9D400000 / 0x9D400008; MUST both be 0 -- pass-through default) ==="
$W $A 'DM=$(command -v devmem || echo "busybox devmem")
  a=$($DM 0x9D400000 32 2>/dev/null || $DM 0x9D400000 2>/dev/null)
  b=$($DM 0x9D400008 32 2>/dev/null || $DM 0x9D400008 2>/dev/null)
  printf "TGEN_GPIO 0x9D400000=%s 0x9D400008=%s\n" "$a" "$b"
  if [ $((a)) -ne 0 ] || [ $((b)) -ne 0 ]; then
    echo "TGEN_GPIO_NONZERO -- config gpio not at reset default (non-fatal, no rollback, reporting only)"
  else
    echo "TGEN_GPIO_OK both zero (pass-through reset default)"
