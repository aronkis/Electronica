...
# (same rsync excludes as the seqbist kit) -> run ops/skidfix/rxfix_inject.py
...
#
# Usage:  ops/skidfix/jupiter_byte_rxfix_kit.sh 148
...
case "$VARIANTS" in
  ...
  # RXFIX Task 46/47: the byte-seam census. Eight READ-ONLY words at 0x23C..0x258
  # (AXI words 0x8F..0x96) behind W1's own freeze bit, so one frozen sweep is
  # coherent across the symbol-plane and byte-plane censuses. Read path only --
  # the write decode is unchanged, which the injector's own test_173 asserts.
  "W1 R4B BS") KIT_SUFFIX="bs"; READ_WORDS="0x214,0x218,0x21C,0x220,0x224,0x228,0x22C,0x230,0x234,0x23C,0x240,0x244,0x248,0x24C,0x250,0x254,0x258" ;;
  # F4 (2026-09-08, ledger FWD_CRC_REGRESSION_0907 §47.5-§47.6): the BS image plus
  # RXFIX_PAD -- the ByteSerializer pads a truncated frame to 191 words so one short
  # frame cannot misalign the axi_dmac transfer boundary (SYNC_TRANSFER_START wait ->
  # FIFO deadlock -> drop-oldest -> 3 lost frames per event, 98 % of the forward
  # residual).  PAD touches ONE file (ByteSerializer) and must follow BS.  Read words
  # unchanged: the census keeps counting the upstream truncation as the witness.
  "W1 R4B BS PAD") KIT_SUFFIX="pad"; READ_WORDS="0x214,0x218,0x21C,0x220,0x224,0x228,0x22C,0x230,0x234,0x23C,0x240,0x244,0x248,0x24C,0x250,0x254,0x258" ;;
  *) echo "RXFIX_KIT_BAD_VARIANTS '$VARIANTS' -- supported: 'W1' (default), 'W1 R4B', 'W1 R4D R1', 'W1 R4B BS' or 'W1 R4B BS PAD'"; exit 1 ;;
esac
case "$BOARD" in
...
INJ_DIRTY=$(cd "$REPO" && git status --porcelain -- ops/skidfix/rxfix_inject.py 2>/dev/null)
if [ -n "$INJ_DIRTY" ]; then
  echo "RXFIX_KIT_REFUSE_DIRTY_INJECTOR ops/skidfix/rxfix_inject.py has uncommitted changes ($INJ_DIRTY) -- commit it first so the kit's provenance is true"; exit 1
fi
INJ_COMMIT=$(cd "$REPO" && git log -1 --format=%H -- ops/skidfix/rxfix_inject.py 2>/dev/null)
...
# BS patches a DIFFERENT, smaller set than W1/R4B/R4D: it taps the byte plane
# (ByteSerializer, ByteRxFifo) and instantiates its census in TxRxComposite, and it
# touches Rate_Handle and the symbol-plane files not at all. Checking it against
# W1's twelve-file list reports a false ZIP_FAIL on files it was never meant to
# patch (RXFIX Task 48: the first kit derivation caught exactly that).
BS_SRC_FILES="ByteSerializer ByteRxFifo TxRxComposite"
src_files_for(){ case "$1" in R1) echo "Preamble_Detector" ;; BS) echo "$BS_SRC_FILES" ;; PAD) echo "ByteSerializer" ;; *) echo "$W1_SRC_FILES" ;; esac; }
ip_files_for(){  case "$1" in R1) echo "" ;;                  BS) echo "$W1_IP_FILES" ;;   PAD) echo "" ;;               *) echo "$W1_IP_FILES"  ;; esac; }
for V in $VARIANTS; do
  M="RXFIX_$V"
...
case " $VARIANTS " in *" R4B "*)
  ...
  # The ninth word must be IN THE READ CHAIN. With BS also present the chain gains a
  # level -- data_read answers BS first and falls through to `bs_under`, which is the
  # r4b term -- so accept either form and fail only if r4b_reg is in neither.
  { grep -q "assign data_read = (r4b_hit ? r4b_reg :" "$DEC" \
    || grep -q "assign bs_under = (r4b_hit ? r4b_reg :" "$DEC"; } \
    || { echo "RXFIX_KIT_VERIFY_FAILED ninth word not wired into the read chain"; FAIL=1; }
...
case " $VARIANTS " in *" BS "*)
  # BS: eight read-only words at AXI 0x8F..0x96, answered from a shadow the decoder
  # latches. Assert the decode range, the index rule (the "-7" whose absence is the
  # plausible failure -- a ROTATION, which passes every static field check on a clean
  # leg; see W1_REGMAP sec 7-BS.4 and the injector's test_171), the data_read wiring,
  # and that the census module itself is present in the kit.
  grep -q "assign bs_hit = (address_select_level1 >= 8'h8F) &&" "$DEC" \
    || { echo "RXFIX_KIT_VERIFY_FAILED BS read range 0x8F.. not decoded"; FAIL=1; }
  grep -q "assign bs_idx = address_select_level1\[2:0\] - 3'd7;" "$DEC" \
    || { echo "RXFIX_KIT_VERIFY_FAILED BS index rule (the -7) missing -- a rotation would ship"; FAIL=1; }
  grep -q "assign data_read = (bs_hit ? bs_reg\[bs_idx\] : bs_under);" "$DEC" \
    || { echo "RXFIX_KIT_VERIFY_FAILED BS words not wired into data_read"; FAIL=1; }
  ls "$KIT/hdl_prj_jupiter_composite/hdlsrc/commhdlQPSKTxRxLoopback/"*bs_seam_census* >/dev/null 2>&1 \
    || grep -rqs "bs_seam_census" "$KIT/hdl_prj_jupiter_composite/hdlsrc/commhdlQPSKTxRxLoopback/" \
    || { echo "RXFIX_KIT_VERIFY_FAILED BS census module not present in the kit"; FAIL=1; }
  ;;
esac

case " $VARIANTS " in *" PAD "*)
  # PAD: the padding arbiter must be in every loose copy of the serializer AND the
  # RXFIX_BS taps it stacks on must still be there (the census is the witness).
  for f in $(grep -rl "RXFIX_PAD" --include="TxRxCompo_ip_src_ByteSerializer.v" "$KIT"); do
    grep -q "pad_1 = 8'd191 - state_wordCnt_1\[7:0\];" "$f" || { echo "RXFIX_KIT_VERIFY_FAILED PAD arbiter missing in $f"; FAIL=1; }
    grep -q "assign bsWordCnt = state_wordCnt;" "$f" || { echo "RXFIX_KIT_VERIFY_FAILED PAD copy lost the BS taps in $f"; FAIL=1; }
  done
  ;;
esac
