#!/bin/bash
# run_beatila2_build.sh -- BUILD ONLY: beat-ILA observability image (BUILD 2 of the
# beat campaign, DUT overlay variant -- adds dut_bit_err_out for a HARDWARE burst trigger; parity DELTA expected).
...
FRESH=$ROOT/jupiter_byte_beatila2_build
...
export QPSK_BEATILA_ERRPORT=1   # BUILD 2: add DUT output dut_bit_err_out -> hw burst trigger
echo "BEATILA2_BUILD start $(date -Is) host=$(hostname) pid=$$ ppid=$PPID"
...
rsync -a --exclude 'hdl_prj_*' --exclude 'slprj' --exclude '*.log' --exclude '*.out' \
  --exclude 'rtl_sim/obj_*' --exclude 's1_rtl' "$BASE/" "$FRESH/" || { echo BEATILA2_BUILD_FAILED rsync; exit 1; }
...
cp "$KITRTL/qpsk_traffic_gen.v" "$KITRTL/qpsk_traffic_gen_rx.v" "$KITRTL/tx_seam_checker.v" \
   "$KITRTL/burst_onset_det.v" "$FRESH/" || { echo BEATILA2_BUILD_FAILED cp; exit 1; }

echo "=== [2/6] patch complete_byte_t8.tcl (TGEN splices + beat-ILA overlay) ==="
python3 "$SRC/patch_tgen_tcl.py" "$FRESH/complete_byte_t8.tcl" || { echo BEATILA2_BUILD_FAILED patch-tgen; exit 1; }
grep -q 'TGEN_RX_WIRE_OK' "$FRESH/complete_byte_t8.tcl" || { echo BEATILA2_BUILD_FAILED patch-tgen-verify; exit 1; }
python3 "$SRC/patch_beatila_tcl.py" "$FRESH/complete_byte_t8.tcl" || { echo BEATILA2_BUILD_FAILED patch-beatila; exit 1; }
grep -q 'BEATILA_WIRE_OK' "$FRESH/complete_byte_t8.tcl" || { echo BEATILA2_BUILD_FAILED patch-beatila-verify; exit 1; }
...
[ -f "$Z/vivado_prj.xpr" ] || { echo "BEATILA2_BUILD_FAILED: no vivado project"; tail -20 "$FRESH/build_byte_matlab.log"; exit 1; }
[ -f "$Z/ipcore/TxRxCompo_ip_v1_0.zip" ] || { echo "BEATILA2_BUILD_FAILED: no ipcore zip"; exit 1; }
grep -q 'byte_breakout' "$Z/vivado_prj.srcs/sources_1/bd/system/system.bd" 2>/dev/null \
  || { echo "BEATILA2_BUILD_FAILED: BD has no byte_breakout"; exit 1; }
...
for M in TGEN_WIRE_OK TGEN_RX_WIRE_OK TXCHK_WIRE_OK BEATILA_WIRE_OK; do
  grep -q "$M" "$FRESH/build_byte_vivado.log" || { echo "BEATILA2_BUILD_FAILED: $M never confirmed"; exit 1; }
done
grep -q "BEATILA_ERRSRC: dut_bit_err_out" "$FRESH/build_byte_vivado.log" \
  || { echo "BEATILA2_BUILD_FAILED: hw error trigger NOT wired (ERRSRC != dut_bit_err_out) -- the whole point of build2"; exit 1; }
B=$Z/boot/BOOT.BIN
[ -f "$B" ] || { echo "BEATILA2_BUILD_FAILED: no BOOT.BIN"; exit 1; }
MD5=$(md5sum "$B" | cut -c1-12)
echo "BEATILA_IMAGE_MD5 $MD5"
LTX=$(find "$Z" -name '*.ltx' | head -1)
echo "BEATILA_LTX ${LTX:-MISSING}"

echo "=== [5/6] DCP rail census (BUILD 2 adds dut_bit_err_out: parity DELTA EXPECTED, report-only) ==="
...
if [ -f "$DCP" ] && [ -f "$BASE/rail_lean_ref.txt" ]; then
  ...
  echo "RAIL_GATE tc_cells beatila2=$TCS lean=$TCL_ $( [ "$TCS" = "$TCL_" ] && echo PARITY_OK || echo PARITY_DELTA_EXPECTED_build2 )"
...
echo "BEATILA2_BUILD_DONE md5=$MD5 image=$B ltx=${LTX:-MISSING} $(date -Is)"
