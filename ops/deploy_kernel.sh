...
# needless risk. uEnv.txt is left untouched. See docs/build-and-flash.rst.
#
# SUBCOMMANDS
...
#         to restore.)
#
# DEPLOY ORDER (see docs/setup-prebuilt.rst Step 0): kernel, then dtb, then
# BOOT.BIN. After this reboot the OLD dtb is still live, so
...
# Default Image = the BANKED UIO kernel, images/Image.6.12.77-uio.a1ba00b51431.gz
# (gunzipped on demand next to it; the raw file is gitignored). Override with $2
# (positional) or the IMAGE env (e.g. a fresh kernel-tree build).
BKG=$(cd "$D/.." && pwd)/images
BANKED_GZ=$BKG/Image.6.12.77-uio.a1ba00b51431.gz
if [ -z "${IMAGE:-}" ] && [ -f "$BANKED_GZ" ]; then
  IMAGE=${BANKED_GZ%.gz}
  [ -f "$IMAGE" ] || { echo "  gunzip -k $(basename "$BANKED_GZ") -> $(basename "$IMAGE")"; gunzip -kf "$BANKED_GZ" || exit 1; }
fi
...
# section of docs/build-and-flash.rst (rebuild is reproducible from the saved patch).
...
do_deploy(){
  ...
  # uEnv would be both ineffective and unnecessary risk. See docs/build-and-flash.rst.
...
verify_board(){
  ...
  #    dtb is ALSO deployed (docs/setup-prebuilt.rst Step 0). Kernel-only step still runs the old dtb.
  ...
  if echo "$names" | grep -qE 'qpsk_(tx|rx)_dma|qpsk_byte_gpio'; then
    ...
    echo "  INFO uio: no qpsk UIO nodes yet (expected until the qpsk dtb is deployed -- docs/setup-prebuilt.rst Step 0)"
