...
rm -f "$OUT"
note "S2_GATE_v5 $(basename "$KIT") $(date '+%Y-%m-%d %H:%M')"
...
if [ -z "$SRC" ]; then note "PATCH MARKERS: MISSING in project sources -- FAIL"; fail=1; else
  ...
  M2=$(grep -rl "cadCnt" "$PROJ" --include="*.v" 2>/dev/null | head -3)
  if [ -z "$M2" ]; then note "PATCH MARKER cadCnt: MISSING -- FAIL"; fail=1; else note "PATCH MARKER cadCnt: present"; fi
fi
...
if [ $fail -eq 0 ]; then note "S2_GATE_v5: PASS"; else note "S2_GATE_v5: FAIL"; fi
exit $fail
