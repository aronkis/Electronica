% assemble_jupiter_240k5_byte.m -- assemble the FULL modem DUT
...
set_param(sys,'Description', sprintf('frame=%s variant=%s pre=%s build=%s', ...
    cfg.Frame, ...
    sprintf(['modem(sps%d/Rsym%s-T8ratefix,K5[35 23]TB25 rx+GATED-txenc,' ...
...
gFill = load(fullfile(fileparts(KITDIR),'contract', ...
