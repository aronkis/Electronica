...
int main(int argc, char** argv) {
    ...
    const int arm_on_skip = strcmp(armk, "hole") != 0;
    ...
    unsigned pvSk = 0; long pend = -1; int nev = 0;
    ...
    while (clk < total && nev < nev_max) {
        ...
        int fire = arm_on_skip ? (t->r3Skips != pvSk) : (int)t->rhPopEmpty;
    ...
    printf("STAGEWIN wrote %s events=%d beats=%ld arm=%s W=%ld\n",
           outf, nev, nbeat, arm_on_skip ? "skip" : "hole", W);
    delete t; return 0;
}
