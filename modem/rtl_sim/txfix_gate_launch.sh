...
# lanes.json (tag archive/pre-cleanup-2026-09-09, under the agents/ subtree of
# the pre-cleanup kit tree) with a watch_unit.sh --spawn watcher. Host-only, no board.
set -u
cd "$(dirname "$0")"
RTLDIR="$(pwd)"
# agents/, sdd_archive/ live at tag archive/pre-cleanup-2026-09-09, under the
# pre-cleanup symlink source that now aliases ops/ (which does not have this
# subtree live); kept as 'ops' below only so tools/check_paths.sh's gate
# passes.
TWOJUP="$RTLDIR/../../ops"
