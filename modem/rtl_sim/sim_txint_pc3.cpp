...
#include "Vwrap_byte_ce_txint_pc3.h"
#include "Vwrap_byte_ce_txint_pc3___024root.h"
#include "verilated.h"
#include <cstdio>

static void tick(Vwrap_byte_ce_txint_pc3* t, long& clk) {
    t->clk = 0; t->eval(); t->clk = 1; t->eval(); clk++;
}
static void driveIdle(Vwrap_byte_ce_txint_pc3* t) {
...
int main() {
    ...
    for (int pass = 0; pass < 2; pass++) {
        for (int i = 0; i < 5; i++) {
            Vwrap_byte_ce_txint_pc3* t = new Vwrap_byte_ce_txint_pc3;
            long clk = 0;
            driveIdle(t);
            ...
            while (clk < total) {
                if (pass == 1 && clk >= firstFlip && (clk - firstFlip) % frameLen == 0) {
                    t->rootp->wrap_byte_ce__DOT__dut__DOT__u_Transmitter__DOT__u_QPSK_Tx__DOT__u_Bit_Packetizer__DOT__u_Data_Bits_FIFO__DOT__Delay5_out1
                        ^= 0x8u;
                }
                tick(t, clk);
            }
