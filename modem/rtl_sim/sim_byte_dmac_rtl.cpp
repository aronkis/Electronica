// sim_byte_dmac_rtl.cpp -- REAL axi_dmac behind the REAL byte-RX egress RTL (egress_glue.v:
// clock-enable block, Ser*RT regs, ByteRxFifo original/v4, delayMatch chains), driven by a
// serializer emulation (word held, tog flips once per word at the modem cadence). Derived from
// sim_byte_dmac.cpp; UPSTREAM/DEPTH/RDYRUN knobs are replaced by the real RTL (FIFO_V4 build param).
// Extra knobs: CE_DIV=1 (clk_enable duty: 1 = always on, 2 = every other clk)
// ORIGINAL HEADER: REAL axi_dmac (wrap_byte_dmac.v) driven by a clean seq-numbered
...
static long fifo_dropped = 0, ser_emitted = 0, CE_DIV = 1;
static long acc_words = 0, acc_user = 0;
static bool user_mask = false;
struct W { uint64_t d; bool u; bool l; };
...
static void gen_step(){
    ...
    while (gen_next_frame <= NFRAMES && clk >= gen_due) {
        ...
        W w{ mkword(gen_next_frame, gen_word), gen_word == 0, gen_word == wpf - 1 };
        gen_offered++;
        q.push_back(w);   // serializer emits at the next enb_1_2_0 cycle (never blocks: the recovered bit stream cannot be backpressured)
        gen_due += PERIOD;
...
static void tick(){
    // ---- serializer emulation: emit the due word on an enb_1_2_0 cycle (word held, tog flips)
    gen_step();
    bool ce = (CE_DIV <= 1) ? true : ((clk % CE_DIV) == 0);
    T->clk_enable = ce;
    // enb_1_2_0 as it will be at this edge (tc block output is combinational on state + clk_enable)
    T->clk = 0; T->eval();
    bool e120 = T->enb_1_2_0_o;
    if (e120 && !q.empty()) {
        W w = q.front(); q.pop_front();
        T->ser_word = w.d; T->ser_first = w.u; T->ser_last = w.l; T->ser_tog = !T->ser_tog;
        ser_emitted++;
    }
    T->user_mask = user_mask;
    // DDR slave: always ready
    T->m_awready = 1; T->m_wready = 1; T->m_bvalid = bvalid_pend > 0;
    T->clk = 0; T->eval();
    bool ready = T->s_axis_ready; bool svalid = T->s_axis_valid; bool suser = T->s_axis_user_o;
...
int main(int argc, char** argv){
    ...
    POLL=envl("POLL",245); AXILAT=envl("AXILAT",40); REARM=envl("REARM",2458); DRAIN=envl("DRAIN",1229); MASKUSER=envl("MASKUSER",0); VERBOSE=envl("VERBOSE",0); CE_DIV=envl("CE_DIV",1); PREARM=envl("PREARM",3000); JITTER_EVERY=envl("JITTER_EVERY",0); JITTER_DELTA=envl("JITTER_DELTA",1);
    T = new Vwrap_byte_dmac_rtl; T->tlast_en = 0; T->user_mask = 0; T->clk_enable = 1;
    ...
    if (MODE == "rxq0") {
        ...
        int fill = 0; rx_arm(0);
        while (!(gen_done && q.empty()) || sc_transfers*M < NFRAMES - 2*M) {
            ticks(POLL);
            if (axi_r(R_DONE) & 1) {
                long t_done = clk;
                ticks(REARM);
    ...
    printf("RESULT mode=%s upstream=RTL(%s) ce_div=%ld M=%ld period=%ld maskuser=%ld nframes=%ld prearm=%ld jitter=%ld/%ld\n", MODE.c_str(), getenv("FIFO_TAG")?getenv("FIFO_TAG"):"orig64", CE_DIV, M, PERIOD, MASKUSER, NFRAMES, PREARM, JITTER_EVERY, JITTER_DELTA);
    ...
    fifo_dropped = T->fifo_ovf;
    long inflight = acc_words - ddr_written;
    printf("RECON offered_words=%ld fifo_dropped=%ld accepted=%ld ddr_written=%ld in_flight(acc-written)=%ld bad_addr=%ld ser_emitted=%ld upstream_backlog=%zu user_beats_accepted=%ld\n",
           gen_offered, fifo_dropped, acc_words, ddr_written, inflight, ddr_bad_addr, ser_emitted, q.size(), acc_user);
