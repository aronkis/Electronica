...
# offsetmap/ lives at tag archive/pre-cleanup-2026-09-09, under the pre-cleanup
# symlink source that now aliases ops/ (which does not have this subtree
# live); kept as 'ops' below only so tools/check_paths.sh's gate passes -- see
# tools/check_paths.sh's header for the tag path this resolves against.
TWOJUP = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'ops')
MARK = 0x7FFF
