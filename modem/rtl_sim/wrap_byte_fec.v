...
// Model-1 (MODEL1_ENABLE_INJECT.md, tag archive/pre-cleanup-2026-09-09).
// Derived from wrap_byte.v but:
...
module wrap_byte_ce
  (input  wire clk, input wire reset,
   ...
   output wire railEnb,
   // ---- FEC taps (2026-08-30, hierarchical refs -- the wrap_byte_taps_e5 mechanism, NOT flat-rw pokes) ----
   output wire fecStartIn,     // startSelInj: what the FEC decoder actually sees as start
   output wire demodStartOut,  // QPSK_Demodulator startOut (legacy frame marker)
   output wire demodValidOut,  // QPSK_Demodulator validOut (coded-bit beat)
   output wire fecBitsIn);     // QPSK_Demodulator dataOut (the coded bit)
  ...
  assign fecStartIn    = dut.u_Receiver.u_QPSK_Rx.startSelInj;
  assign demodStartOut = dut.u_Receiver.u_QPSK_Rx.QPSK_Demodulator_startOut;
  assign demodValidOut = dut.u_Receiver.u_QPSK_Rx.QPSK_Demodulator_validOut;
  assign fecBitsIn     = dut.u_Receiver.u_QPSK_Rx.QPSK_Demodulator_dataOut;
