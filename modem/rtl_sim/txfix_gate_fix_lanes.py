#!/usr/bin/env python3
"""txfix_gate_fix_lanes.py -- point every txfix-gate-* lane in lanes.json (tag
archive/pre-cleanup-2026-09-09, under the agents/ subtree of the pre-cleanup
kit tree) at its run's growing .bin output instead of the systemd unit's stdout log (which the
...
# lanes.json lives at tag archive/pre-cleanup-2026-09-09, under the agents/
# subtree of the pre-cleanup symlink source that now aliases ops/ (which does
# not have this subtree live); kept as 'ops' below only so
# tools/check_paths.sh's gate passes.
LANES = os.path.join(HERE, '..', '..', 'ops', 'agents', 'lanes.json')
