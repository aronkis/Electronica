...
// distinct from the BIST ROM content that produces 0x5EA9540A) and confirm
...
int main(int argc, char** argv) {
    ...
    t->fixctl = 1u << 20;    // RFCAP selected on the readout mux
    t->iq_debug_mux = 0;
    ...
    printf("POSCONTROL_RFCAP frames_seen=%d first_cap=0x%08X last_cap=0x%08X final_mismatches=%u\n",
           frameIdx, firstCap, lastCap, t->bfLatch);
    printf("  CLEAN_BASELINE (BIST loopback, mode-1) cap=0x5EA9540A\n");
