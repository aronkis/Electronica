...
$W $B 'DM=$(command -v devmem || echo "busybox devmem")
  ...
  echo "0x10C 0x1">$DRA   # iq_debug_mux = post symbol-sync
  ...
  waituntil $(awk -v t=$T0 "BEGIN{print t+34.90}")
  $DM 0x9D430000 32 3; echo "FORCE1 $(awk "{print \$1}" /proc/uptime) status=$($DM 0x9D430008)"
  sleep 1; $DM 0x9D430000 32 0
  waituntil $(awk -v t=$T0 "BEGIN{print t+154.65}")
  $DM 0x9D430000 32 3; echo "FORCE2 $(awk "{print \$1}" /proc/uptime) status=$($DM 0x9D430008)"
  sleep 1; $DM 0x9D430000 32 0
