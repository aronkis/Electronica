...
best=(0,0)
for ph in range(sps):
    sym=mfc[ph::sps]; sym=sym/np.sqrt(np.mean(np.abs(sym)**2))*np.sqrt(np.mean(np.abs(pre)**2))
    c=np.abs(np.correlate(sym,pre,'valid')); r=c.max()/np.median(c)
    if r>best[0]: best=(r,ph)
print(f"{sys.argv[1].split('/')[-1]}: CFO={f4:+.0f}Hz  best Barker peak/median={best[0]:.1f} (phase {best[1]})  [>8 = frames present]")
