...
for pip in $PINS; do
  G=$($W $pip 'cat /sys/bus/iio/devices/iio:device2/in_voltage0_hardwaregain' 2>/dev/null)
  GV=$(echo "$G" | grep -oE '^[0-9.]+')
  $W $pip "P=/sys/bus/iio/devices/iio:device2; echo spi > \$P/in_voltage0_gain_control_mode; echo $GV > \$P/in_voltage0_hardwaregain" 2>/dev/null
  ...
  for ip in $B_IP $A_IP; do
    $W $ip "P=/sys/bus/iio/devices/iio:device2
      for w in $SET; do case \$w in
        agc)   echo 0 > \$P/in_voltage0_agc_tracking_en; echo 0 > \$P/in_voltage1_agc_tracking_en;;
        rssi)  echo 0 > \$P/in_voltage0_rssi_tracking_en; echo 0 > \$P/in_voltage1_rssi_tracking_en;;
        bbdc)  echo 0 > \$P/in_voltage0_bbdc_rejection_tracking_en; echo 0 > \$P/in_voltage1_bbdc_rejection_tracking_en;;
        txclg) echo 0 > \$P/out_voltage0_close_loop_gain_tracking_en 2>/dev/null; echo 0 > \$P/out_voltage1_close_loop_gain_tracking_en 2>/dev/null;;
      esac; done
      echo \"tracking post-lock: agc=\$(cat \$P/in_voltage0_agc_tracking_en) bbdc=\$(cat \$P/in_voltage0_bbdc_rejection_tracking_en) rssi=\$(cat \$P/in_voltage0_rssi_tracking_en)\"" 2>/dev/null
  done
fi
