...
tail -n0 -F /dev/shm/seq_events.log 2>/dev/null | while read line; do
  ...
  echo \"SAVED ev\$N seq=\$SEQ | \$line\" >> \$OUT/hunt.log
done
EOF
...
for ip in $B_IP $A_IP; do
  $W $ip "cd /root/host_app_k5; rm -f /dev/shm/acc.log /dev/shm/seq_events.log /dev/shm/seq_raw.log; setsid sh -c './qpsk_tun -S -d $DUR > /dev/shm/acc.log 2>&1' </dev/null >/dev/null 2>&1 &" 2>/dev/null
done
...
$W $T_IP "setsid sh -c 'rm -f /dev/shm/poll.log; END=\$(( \$(date +%s) + $DUR )); DRA=/sys/kernel/debug/iio/iio:device0/direct_reg_access
  while [ \$(date +%s) -lt \$END ]; do
    echo 0x150 > \$DRA; R=\$(cat \$DRA); echo 0x154 > \$DRA; C=\$(cat \$DRA)
    G=\$(cat /sys/bus/iio/devices/iio:device2/in_voltage1_rssi 2>/dev/null | cut -d\" \" -f1)
    echo \"t=\$(date +%s.%N) rstcs=\$R cfc=\$C rssi=\$G\" >> /dev/shm/poll.log
    sleep 0.2
