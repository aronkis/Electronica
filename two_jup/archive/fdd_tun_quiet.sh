#!/bin/bash
# fdd_tun_quiet.sh -- fdd_tun_wd.sh on the QUIET pair 2.00 + 1.90 GHz (avoids 148's 2.10 Tx-LO
# leakage that jammed 146 Rx), with the host WHITENER on (low-entropy IP payloads). Cold start,
# watchdog both, qpsk_tun -F -i tun0, then PING both ways.  146=10.66.0.1 <-> 148=10.66.0.2.
set -u
TXA=2000000000; FB=1900000000    # 146 TX@2.00 -> 148 RX@2.00 ; 146 RX@1.90
TXB=1900000000; FA=2000000000    # 148 TX@1.90 -> 146 RX@1.90 ; 148 RX@2.00
...
coldstart(){ # $1 ip $2 txlo $3 rxlo $4 tunaddr $5 peer
  $W $1 "P=/sys/bus/iio/devices/iio:device2; DB=/sys/kernel/debug/iio/iio:device2
   ...
   cd /root/host_app_k5; QPSK_WHITEN=${WHITEN:-1} setsid ./qpsk_tun -F -i tun0 -s 30 </dev/null >/dev/shm/qpsk_tun.log 2>&1 &
   ...
   echo '$1 cold-started (quiet 2.00/1.90, WHITEN=1), tun0 set'" 2>/dev/null
}
echo "=== QUIET-PAIR tun0 cold start (2.00 + 1.90, whitener) $(date -Is) ==="
...
for ip in 10.0.0.146 10.0.0.148; do
  echo "--- $ip ---"
  $W $ip 'echo "  wd: $(tail -1 /dev/shm/watchdog.log 2>/dev/null)"; echo "  daemon: $(grep "qpsk_tun stats" /dev/shm/qpsk_tun.log 2>/dev/null | tail -1 | grep -oE "dma_rx_ok=[0-9]+ crc_drop=[0-9]+ idle_rx=[0-9]+")"; echo "  tun0: $(ip -o addr show tun0 2>/dev/null | grep -oE "inet [0-9.]+")"' 2>/dev/null
done
echo "=== PING 146 -> 148 (over RF, quiet pair) ==="; $W 10.0.0.146 'ping -c 10 -W 3 10.66.0.2 2>&1 | tail -4' 2>/dev/null
echo "=== PING 148 -> 146 (over RF, quiet pair) ==="; $W 10.0.0.148 'ping -c 10 -W 3 10.66.0.1 2>&1 | tail -4' 2>/dev/null
echo "=== DONE (link left UP for SSH attempt) ==="
