...
$W $B 'DM=$(command -v devmem || echo "busybox devmem")
  ...
  CAP1=$(echo 0x13C>$DRA; cat $DRA); $DM 0x9D430000 32 3; echo "FORCE1(gap?) $(awk "{print \$1}" /proc/uptime) cap_in=$CAP1 status=$($DM 0x9D430008)"
  sleep 1; $DM 0x9D430000 32 0
  waituntil $(awk -v t=$T0 "BEGIN{print t+154.30}")
  CAP2=$(echo 0x13C>$DRA; cat $DRA); $DM 0x9D430000 32 3; echo "FORCE2(inwin?) $(awk "{print \$1}" /proc/uptime) cap_in=$CAP2 status=$($DM 0x9D430008)"
  sleep 1; $DM 0x9D430000 32 0
