...
DWELL=${DWELL:-400}
HAVE_TXCAP=${HAVE_TXCAP:-0}       # set 1 when the flashed image carries TXCAP
OUT=$D/finalrun/$(date +%Y%m%d_%H%M%S)
mkdir -p "$OUT"
...
$W $B "P=/sys/bus/iio/devices/iio:device2; DB=/sys/kernel/debug/iio/iio:device2
 ...
 echo '0x208 0x0'>\$DRA; echo '0x10C 0x0'>\$DRA
 ...
 echo '  armed MODE1 ROM, fixctl=0, iq_debug_mux=0'" 2>/dev/null | tee -a "$OUT/run.log"
...
# CSV columns: t frames err  mmTX mm0 mm1 mm2 mm3  capIn capDeint capOut

log "--- [PC] POSITIVE CONTROL: counter must be FLAT parked, then CLIMB on tap switch ---"
$W $B "DRA=/sys/kernel/debug/iio/iio:device0/direct_reg_access
 echo enabled > /sys/bus/iio/devices/iio:device0/reg_access 2>/dev/null
 rd(){ echo \"\$1\">\$DRA; cat \$DRA; }
 echo '0x10C 0x3' > \$DRA; sleep 3
 echo \"  PC tap3 cap=\$(rd 0x20C) mm=\$(rd 0x210)\"
 a0=\$((\$(rd 0x210))); sleep 5; a1=\$((\$(rd 0x210)))
 echo \"  PC_PARK  tap3 delta_over_5s=\$(( (a1-a0)&0xFFFFFFFF ))   (expect 0 if invariant)\"
 echo '0x10C 0x0' > \$DRA; sleep 3
 echo \"  PC tap0 cap=\$(rd 0x20C) mm=\$(rd 0x210)\"
 b0=\$((\$(rd 0x210))); sleep 5; b1=\$((\$(rd 0x210)))
 echo \"  PC_SWITCH tap0 delta_over_5s=\$(( (b1-b0)&0xFFFFFFFF ))   (MUST be > 0 or the counter is DEAD)\"
 echo '0x10C 0x3' > \$DRA; sleep 3
 c0=\$((\$(rd 0x210))); sleep 5; c1=\$((\$(rd 0x210)))
 echo \"  PC_BACK  tap3 delta_over_5s=\$(( (c1-c0)&0xFFFFFFFF ))\"
 echo '0x10C 0x0' > \$DRA" 2>/dev/null | tee -a "$OUT/run.log"
if [ "${HAVE_TXCAP:-0}" = 1 ]; then
  log "--- [PC2] TXCAP and DEMODCAP capture registers (a zero capture = dead block) ---"
  $W $B "DRA=/sys/kernel/debug/iio/iio:device0/direct_reg_access
   echo enabled > /sys/bus/iio/devices/iio:device0/reg_access 2>/dev/null
   rd(){ echo \"\$1\">\$DRA; cat \$DRA; }
   echo '0x208 0x1000' > \$DRA; sleep 1; echo \"  TXCAP    cap=\$(rd 0x20C) mm=\$(rd 0x210)\"
   echo '0x208 0x2000' > \$DRA; sleep 1; echo \"  DEMODCAP cap=\$(rd 0x20C) mm=\$(rd 0x210)\"
   echo '0x208 0x0' > \$DRA" 2>/dev/null | tee -a "$OUT/run.log"
fi
...
$W $B "DRA=/sys/kernel/debug/iio/iio:device0/direct_reg_access
 ...
 for i in \$(seq 1 $DWELL); do
   ...
   sleep 1
   p1=\$((\$(rd 0x104))); e1=\$((\$(rd 0x108)))
   echo \"\$i \$(( (p1-p0)&0xFFFFFFFF )) \$(( (e1-e0)&0xFFFFFFFF )) \$TXM\$M \$CI \$CD \$CO\$C\" >> /dev/shm/finalrun.csv
 done
 echo POLL_DONE" 2>/dev/null | tee -a "$OUT/run.log"
$W $B 'cat /dev/shm/finalrun.csv' 2>/dev/null > "$OUT/finalrun.csv"
log "  CSV $OUT/dbgcap.csv ($(wc -l < "$OUT/finalrun.csv") samples)"
