#!/bin/bash
# =============================================================================
# link_test_f1536.sh -- OPERATOR entry point for the two-Jupiter QPSK RF link,
# F1536 (1536 B large-frame, interrupt-driven-DMA) variant.
#
# Cloned from link_test.sh (the 1.92 MSPS profile / R0 build). The ONLY changes
# are host-side frame geometry in coldstart_tun: qpsk_tun -F -> -G (f1536 mode,
# requires the 2 MB-carve host binary + the qpsk UIO dtb), tun MTU 116 -> 1516,
# advmss 56 -> 1476. Every radio-arm / modem register-poke sequence is byte-for-
# byte identical to link_test.sh (silicon-proven -- do NOT paraphrase). Requires
# boards on the f1536 stack (of_id kernel + 2 MB qpsk dtb + Image A). IRQ mode is
# expected -- qpsk_tun logs "irq mode: <tx>/<rx>" (vs "no UIO nodes, polled mode").
#
...
# ---------------------------------------------------------------------------
# apply_ssi_fix -- board 146's per-unit TX SSI delay override (task-TXCHAR): the
# driver's per-profile-load auto-tune picks a word-slip point (tx0 clk=2, radiates
# Q=I); the proven point is clk=3/data=4. MUST run after EVERY arm/coldstart of
# 146 (a profile reload reverts it). No-op guard inside the script for non-146.
# ---------------------------------------------------------------------------
apply_ssi_fix(){
  if [ "$B_IP" = "10.0.0.146" ]; then
    AIR=1 "$DIR/apply_146_ssi_fix.sh" "$B_IP" || echo "WARN: 146 SSI fix FAILED -- forward link will storm (task-TXCHAR)"
  fi
}
...
# arm_ber, but instead of the devmem byte-DMA arm it launches "qpsk_tun -G"
# (f1536 large-frame mode; owns the byte DMAs via UIO/IRQ) + lock_watchdog +
# configures tun0 at MTU 1516). QPSK_WHITEN is the local $WHITEN and MUST be
# identical on both ends. Single anyssh per board. F1536 CHANGES vs link_test.sh
# are HOST-GEOMETRY ONLY: -F->-G, mtu 116->1516, advmss 56->1476; the radio-arm
# register pokes above are byte-for-byte identical (silicon-proven).
# ---------------------------------------------------------------------------
coldstart_tun(){ # $1 ip $2 txlo $3 rxlo $4 tunaddr $5 peer
  $W $1 "P=/sys/bus/iio/devices/iio:device2; DB=/sys/kernel/debug/iio/iio:device2
   ...
   TXD=\$(for d in /sys/bus/iio/devices/iio:device*; do [ \"\$(cat \$d/name 2>/dev/null)\" = axi-adrv9002-tx-lpc ] && echo \${d##*/}; done);T=/sys/kernel/debug/iio/\$TXD/direct_reg_access
   echo '0x418 0x2'>\$T;echo '0x458 0x2'>\$T;echo '0x044 0x1'>\$T;echo '0x110 0x1'>\$DRA;sleep 0.3;echo '0x110 0x0'>\$DRA
   cd /root/host_app_k5; QPSK_WHITEN=$WHITEN QPSK_ATOMDBG=${QPSK_ATOMDBG:-0} setsid ./qpsk_tun -G -i tun0 -s 30 </dev/null >/dev/shm/qpsk_tun.log 2>&1 &
   ...
   ip addr replace $4 peer $5 dev tun0; ip link set tun0 up mtu 1516; ip route replace $5 dev tun0 advmss 1476 rto_min 25ms 2>/dev/null
...
provision_perf(){
  local ip
  for ip in "$A_IP" "$B_IP"; do
    scpput "$DIR/../host_app_k5/qpsk_perf.c" root@$ip:/root/host_app_k5/qpsk_perf.c 2>/dev/null
