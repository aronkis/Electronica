...
#         Deploy to ONE board: validate, back up /boot/system.dtb (size-checked),
...
KIT=$(cd "$D/.." && pwd)/jupiter_240k5_byte/boot   # dtso + conf live here
...
case "$1" in
  ...
  check)    shift; [ $# -ge 1 ] || die "usage: deploy_dtb.sh check <merged.dtb>"; validate_dtb "$1"; echo "CHECK_OK: $1" ;;
  rollback) shift; [ $# -ge 1 ] || die "usage: deploy_dtb.sh rollback <ip>"; do_rollback "$1" ;;
  -h|--help|help) usage ;;
  *)
    [ $# -ge 2 ] || die "usage: deploy_dtb.sh <ip> <merged-system.dtb>  |  build|check|rollback (see --help)"
    do_deploy "$1" "$2" ;;
esac
