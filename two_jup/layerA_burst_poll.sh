...
echo "=== Layer A burst poll: ${DWELL}s at 1 s resolution ==="
...
echo "--- polling ${DWELL}s at 1 s (on-board loop -> /dev/shm/burst.csv) ---"
$W $B "DRA=/sys/kernel/debug/iio/iio:device0/direct_reg_access
...
echo "--- restore rig ---"
bash "$D/restore_known_good.sh" > /tmp/burstpoll_restore.log 2>&1
