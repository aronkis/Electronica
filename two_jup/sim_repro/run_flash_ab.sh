...
bash skidfix/flash_148_rxfifo.sh $MD5 > $S/flash_rxfifo.log 2>&1; RC=$?
tail -4 $S/flash_rxfifo.log | tee -a $S/flash_ab_chain.log
grep -q "FLASH_RXFIFO_DONE" $S/flash_rxfifo.log || { log "CHAIN_STOP flash rc=$RC (rolled back or fatal; NO retry)"; exit 1; }
