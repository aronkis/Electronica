...
### §90 correction (controller, 2026-09-03 09:25): frame time is 802.93 µs, not 3.21 ms — the rate arithmetic in §90 is 4× off, the conclusion survives
[silicon] The board runs 1247 frames/s (every ARM_OK line: `fps=1247`) → 802 µs per frame. [RTL/arith] 197,328 ticks per frame at 16 ticks per symbol (12,333 slots) is consistent with TX_ORIGIN_TRACE_A.md §3's 802.93 µs, so §90's "3.21172 ms" is wrong. *(Corrected 2026-09-03, final review: this line originally inferred "the fabric clock is 245.76 MHz (= 4 × 61.44 MSPS)". That inference is refuted — `timing_gate.tcl` constrains the modem fabric to the 8.000 ns `axi_adrv9001_adc_1_clk`, **125 MHz** (ledger 10:38), so the 197,328 are **sim ticks**, not fabric clocks, and no fabric frequency follows from them. The recomputation below starts from the **measured** 802.93 µs and is unaffected.)* Recomputed with 802.93 µs/frame: one `frameCount` base increment = 947.69 frames = **0.761 s**; first `frameCount==0` after arm (base 1→3) = 1,895 frames = **1.52 s**; full mod-4 return = **3.04 s**. 120.2 s = **149,708 frames**; ~110 s first burst = ~137,000 frames. So the free-running +26 slots/frame drift is ~40–80× too fast to be the 120.2 s clock — §90's conclusion ("the drift is falsified as the rate mechanism") stands, stronger. §89's "12,333 frames = 39.61 s" → 9.90 s; also falsified as a period. Also corrected here: the silicon displaced-state durations of ~1230–1257 frames are **~1.0 s** each (not 3 s), and the ~3 s "burst" is the 3–4 state sequence (§87: 6240→0→6363→0→None→0 across 4.4 s).

**State of the rate question after §90 [sim + silicon]:** (a) a natural `frameCount` 3→0 wrap DOES fire the abort in sim once the base has ratcheted to 3 (txrate_fc3, and a producer-phase-only variant), so the trigger path is real; (b) but in sim it self-cancels within ~1 frame (a follow-on pop-wrap abort completes the interrupted frame, deficit = 2 whole frames ≡ 0), whereas silicon holds each displaced state ~1.0 s and steps through 3–4 states per burst; (c) the fullRAM runaway (§90 "bonus defect": pace toggle freezes, `dataReady` sticks high, producer pushes every tick until the uint16 count wraps at 65,536, over-writing 16,256 RAM bits) is a second real defect and the leading candidate for what makes the silicon abort land at 5760–6550 symbols and persist — untested. What sets 120.2 s is NOT identified; the only structures found so far tick at 0.76 s / 3.04 s.

**Decision offered to the operator:** the fix does not depend on the trigger — gating the pop-enable clear to the frame boundary, saturating `frameCount`, and fixing the fullRAM runaway removes every path found (forced clear, natural 3→0, runaway over-push). A fix build can be sim-gated against the k=0 forced case (must still stall on the unfixed netlist, must not on the fixed one) and the txrate_fc3 natural case, then flashed to 148 under the full rails and judged by the 0x108 beat timeline over ≥ 3 periods (≥ 6 min) plus a sel6 DDRCAP witness. The remaining scientific question (why 120.2 s) can be pursued in parallel on the unfixed image if wanted.

## §91 THE FIX CAMPAIGN, END TO END: F3 built, sim-gated, flashed on 148 and accepted — the 120.2 s transmit stall is absent within a stated boundary (Task 9 write-up, 2026-09-03)
Plan: `/home/tcollins/.claude/plans/happy-bubbling-owl.md` (operator-approved ~10:40). Ledger:
`two_jup/sdd_archive/2026-09-03-txfix/progress.md` (Rulings extracted to `RULINGS.md` in the same
directory). Controller state file: `two_jup/TXFIX_STATE.md`. Sim gate: `two_jup/TXFIX_SIM_GATE.md`
+ raw `jupiter_240k5_byte/rtl_sim/beat_runs/txfix_gate_summary.txt`.
Provenance labels used throughout: **[silicon]** measured on board 148; **[sim]** Verilator gate on
this host; **[RTL]** read in the netlist (file:line); **[vivado]** build logs on hdl-dev-2;
**[inferred]** reasoning over the above.

**Counter discipline (applies to every 0x108 number below, on BOTH images).** `0x108` is a bit-error
counter that sees only the first 120 bits of each 2,240-bit frame (§82 / memory
`bist-120bit-window.md`). Every "errs/s", "burst" and "n=0" figure in this section is a **1 s delta
of that counter over that 120-bit window**. None of them is a frame error rate, a PER, or a BER, and
none may be quoted as one.

### 1. What the three variants change [RTL]
All three are netlist patches applied by `two_jup/skidfix/txfix_inject.py <dir> F1|F2|F3` to
`jupiter_240k5_byte/s1_rtl/hdlsrc/commhdlQPSKTxRxLoopback/` (and to the identical sim/kit copies);
they are strictly cumulative, F1 ⊂ F2 ⊂ F3. Regenerating from Simulink would overwrite them — they
are netlist patches by design. Source of the summary: `TXFIX_STATE.md` §1.
- **F1 — gate the pop-enable clear on the frame boundary.** [RTL] `Data_Bits_FIFO.v:272-289`: the
  latch that enables RAM pops (`Unit_Delay_Enabled_Resettable_Synchronous_out1`) was cleared on ANY
  `enb_1_2_0` tick where the delayed frame count read zero (`Compare_To_Constant1_out1 =
  Delay3_out1 == 2'b00`, `:270`); it was reloaded only at `sampleCount == 0`
  (`Compare_To_Constant3_out1`, `:220`). F1 moves the clear inside the `sampleCount == 0` branch, so
  a zero seen mid-frame can no longer stop the pops. On every tick where the count is nonzero the
  logic is identical (`Compare_To_Constant2_out1 ≡ ~Compare_To_Constant1_out1`, `:248` vs `:270`) —
  which is why the golden digest is predicted unchanged, and is [sim G3].
- **F2 — F1 + saturate the frame counter.** [RTL] `RAM_Frame_Status_Indicator.v:66-94`: the 2-bit
  `frameCount` had unguarded ±1 (3→0 on a push-wrap, 0→3 on a pop-wrap). F2 saturates at both rails;
  a simultaneous push- and pop-wrap stays a no-op, as the original netted. This removes the way the
  mid-frame zero arises at all.
- **F3 — F2 + throttle the producer (fullRAM runaway fix).** [RTL] `Bit_Packetizer.v` end:
  `dataReady = DataReadyPaceCmp_out1 & Logical_Operator2_out1` (`Logical_Operator2_out1 = ~fullRAM`,
  declared `:59`/`:144`), so the producer's ready drops while the RAM is full instead of a frozen
  pace toggle leaving it stuck high; `MATLAB_Function1.v` (uint16 occupancy `count`, `full =
  count_temp > 49279`, `:39-69`/`:158-163`) saturates instead of wrapping past 65,535.
  **F2 and F3 differ in exactly this one thing.**
- Not corrected in any variant [inferred, plan decision]: the +26 slots/frame producer over-push
  (24,666 pushed vs 24,640 popped per frame, §90). Correcting it at source would change the data
  plane and therefore the golden capTAP digest. With F3 it self-limits at the full threshold.
- **F3b** (a −16 threshold margin, `--margin`) exists as an injector flag and was **not** needed:
  [sim G11] measured overshoot past 49,279 = 1 count, far below the plan's ">2 bits" trigger.

### 2. Injector and sim gate matrix [sim]
Injector = clone of `ddrcap2_inject.py` with idempotent `TXFIX_F1/F2/F3` markers, `patch_zip` /
`verify_zip`, and a `_sub()` that asserts each anchor occurs exactly once (refuses to patch
otherwise). Unit tests `two_jup/tests/test_txfix_inject.py` (19/19 green: patch, idempotent,
anchor-unique, anchor-missing, cumulative, synthetic two-member zip, lint). Trees
`jupiter_240k5_byte/rtl_sim/s1_rtl_txfix_F{1,2,3}` = `cp -a s1_rtl_txmark` + injector; harness
`sim_txfix_force.cpp` (adds a `popabort` **READBACK** line proving the forced write landed, so a
null on a fixed tree is non-vacuous, and `latchforce`/`latchforce_pre` controls); scored with the
UNMODIFIED `kick_seq.py` + tap3 map. 26 runs as `systemd-run --user` units, all exited by 12:31.

| Gate | Variant | Result | Evidence (final, exit-gated) |
|---|---|---|---|
| G0 | U | PASS | 106 frames, all offset 0 (no-force positive control) |
| G1 | U | PASS | forced clear k=0 → sustained offset 6144, `saw_armed_zero=1`, `armed_zero_clk=641342` — the silicon signature reproduced |
| G2 | U | PASS | natural abort: `zeroEvents=2`, `armedDropClk=4041064`, no force at the abort instant |
| G3 | F1/F2/F3 | PASS | golden digest: 1,305,858 aligned records, 0 differing, on all three; reviewer additionally found the four `none` .bin files byte-identical by plain md5 |
| G4/G5 | F1/F2/F3, k=0 and k=6 | PASS | `d3_post=0` readback, `saw_armed_zero=0`, `max_pop_quiet=2`, offset 0 throughout — 6 runs |
| G6 | F1/F2/F3 | **FAIL (control inconclusive)** | identical SUMMARY on all three: `saw_armed_one=1`, `armed_one_clk=690750`, `clk_to_reload=49410`; tail offsets decode as `None` |
| G7 | F1/F2/F3 | PASS | preamble-window latch write, `clk_to_reload=1`, offset 0 throughout |
| G8 | **F1** | **FAIL — decision gate** | `zeroEvents=14`, `armedDropClk=4045326` set, `maxPopQuietClk=106`, `pops_values=['24639','24640']` (**not** all 24,640) |
| G9 | F2/F3 | PASS | `zeroEvents=0` under the identical fcbase3 stress |
| G10 | F2/F3 | PASS | `saw_self_wrap=0` |
| G11 | F3 | PASS | `min_count=49279`, `max_count=49280` → overshoot = 1, clean hysteresis, no wrap either direction, `fullRAM_stuck_at_end=False` |
| G12 | F1/F2 | PASS (documenting the defect) | `runaway_present=True`, `max_count=49280 > 49279` — the missing guard lets the over-push run past the threshold |
| G13 | F3 | PASS [sim, derived] | `doubled_push_rate_seen=False`, `push_pointer_seq=[]` — derived from G11's log, **not an independent run**; empty-sequence evidence is absence of evidence |
| G14 | F1/F2/F3 | PASS | Verilator lint clean on all tops, both normal and STRICT passes, rerun fresh |

**G6 — the one parked control [sim].** `latchforce` writes the latch itself mid-frame. The RTL
witness fired correctly and **identically on all three variants** (`armed` 0→1 at clk 690,750,
reload 49,410 clk later), but `kick_seq.py` returns `None` for every post-force frame: the decoded
word is outside the 12,320-entry tap3 vocabulary, so the scorer's strict criterion (must land on an
enumerated offset) is not met. Read as a scoring-criterion artefact — a raw register poke mid-eval
garbles a frame into a word the map does not contain — not as a fix regression, and not as evidence
that any variant behaves differently. Detector liveness rests on G1 (same scorer, and per G3 the
fixed streams are identical to the unfixed baseline). **The control is unresolved and is an open
item** (§6).

**G12 — what is and is not established [sim].** Established: the over-push **runaway** is present on
F1 and F2 and absent on F3 (G11). **Not** established: the underflow direction. The raw
`txfix_gate_summary.txt` G12 lines still carry `underrun_toward_65536=True`, `wrap_step_seen=True`,
`min_count=0` — those lines predate the review fix round (commit `1d36163`) and the flag is not
supported by its own evidence: `near_65536_seen=False` on every G12 line and only **2 samples** were
captured in the NF=8 trim (the nearfull log is change-detection, not per-clock). `min_count=0` is
the legitimate drain floor of a correctly working FIFO. The ledger's 12:35 HEARTBEAT
("min_count=0 confirms the underflow-toward-zero defect is real") is **superseded** by
`TXFIX_SIM_GATE.md` as corrected. The underflow-toward-zero mechanism remains a plausible,
RTL-motivated hypothesis, not a measured one.

**Two scoring bugs found and fixed mid-run, both logged as `FINDING` lines** — worth carrying
forward: (1) an interim pass scored G0/G3 as PASS from `.bin`/`_frames.txt` files that *looked*
complete but whose Verilator processes were still running (buffered writes are not
atomic-at-completion); fixed by requiring a `TXFIX_GATE_RUN_EXIT=<code>` trailer written only after
the binary returns, before trusting any evidence. (2) `underflow_evidence()` mixed the FORCE line's
one-off `pre_count=`/`target=` annotations into the time-ordered `count=`/`occ=` trace, producing
false wrap positives on G11. All numbers in the table above are post-fix and exit-gated (this is why
G0 reads 106 frames, not the 77/89 of the premature scoring).

**F1 disqualification, and the F2/F3 difference.** G8 is the plan's pre-registered decision gate:
F1's pass line was `pops=24640` on every CEN line. F1 gives `24639` on some frames with
`zeroEvents=14` and `armedDropClk` set — under the *natural* fcbase3 stress the frame-boundary
reload can still sample a wrapped zero, losing ~1 pop per event (a slow bit slip that the offset map
cannot see, since 12,320 ≡ 0). **F1 was therefore disqualified before any Vivado time was spent on
it**; the second build became F2 [Ruling 12:35]. F2 vs F3 is exactly the fullRAM item (§1): F2 leaves
the runaway (G12) and therefore hides the RAM over-write with no abort to signal it — F2 is a
bisection artefact and is **not** a ship candidate on its own.

### 3. Vivado history [vivado] (hdl-dev-2, Vivado 2025.1, JOBS=6)
| variant | launched | finished | duration | outcome |
|---|---|---|---|---|
| F1 | never | — | 0 | Disqualified by sim gate G8 before a kit was made. Zero build time spent. |
| F2 | 12:37:00 | pending at write-up | — | Kit injected 12:36:30-12:37:00 (zips=2 verified); `IMPL_STRATEGY=explore`; in synthesis at 12:50; estimate routed ~13:25. **Bank only, do not flash** [Ruling]. |
| F3 attempt 1 | 10:09:34 | 10:13:05 | 3.5 min | Died in the ADI `axi_adrv9001` IP synthesis on a Vivado temp-file error (`./.Xil/.../genlib`). Environmental, main synth never ran; no image. |
| F3 attempt 2 | 10:15:12 | 11:45:23 | 90 min | Image `873902b705fb` produced, but **routed WNS −0.392 ns / TNS −17.7 / 56 failing endpoints**. Diagnosis: all 10 worst paths are inside the **tx_checker instrument** (`fill_reg[3]` → `bit_errors`/`frame_errs` counters, 24-25 logic levels), **not** in the modem DUT and **not** in any F3 patch — a pre-existing marginal path that the ddrcap2 build had closed at +0.105 ns by placement luck. Discarded, never flashed. |
| F3 attempt 3 | 11:48:00 | 12:32:41 | 45 min | `IMPL_STRATEGY=explore` (ExtraTimingOpt place, AggressiveExplore route, phys_opt pre and post): **routed WNS +0.095 ns, TNS 0**; post-synth modem WNS +1.544 ns; no latch inference. Banked `boot_known_good/BOOT.BIN.148.txfixF3.f6a8c3ea119c`. |

Two process changes came out of this and are now permanent in the flow:
- `txfix_build.tcl` **refuses to bootgen a routed-WNS-failing implementation**
  (`TXFIX_ROUTED_TIMING_FAIL`). The pre-existing gate checked post-synthesis timing only, which is
  why attempt 2 produced a BOOT.BIN at all. The no-flash-on-WNS-fail rule was upheld even though the
  violations were in an instrument and not in the fix — those counters feed the error witnesses, so
  a metastable count would corrupt the very evidence the campaign rests on [Ruling 11:47].
- The allowed remedy for a timing fail is **one** re-implementation of the same netlist with stronger
  place/route/phys-opt directives (`explore`), not a re-synthesis or a design change.

### 4. Silicon acceptance on 148 [silicon]
Pre-registered in the ledger before the flash (`PREREG T3b·F3`), verbatim criteria unchanged
afterwards. Rails honoured: 148 only, 146 untouched, full-rails flash (banked restore, readback
verify, two-pass gate, automatic rollback, no retry loop), sentinel stopped, no arm killed.

1. **Baseline positive control, 10:16-10:32, image `638b36de3493`.** Fresh arm (`ARM_OK fps=1247`,
   capTAP golden) so the control shares the post-arm phase of the fixed-image runs [Ruling 10:15],
   then a 420-row `0x108` timeline: **BURSTS n=7**, roughly every 60 rows ≈ 120 s, 3-5 rows each,
   mean per-row delta 58k-72k, against a quiet floor of ~55-100 counts/s. **The witness sees the
   beat.** Without this the whole silicon lane would have been UNINFORMATIVE and nothing would have
   been flashed.
2. **Flash, 12:36.** `f6a8c3ea119c` flashed 12:36:36 under `flash_148_txfix.sh`; readback matched
   12:37:45; **GATE_PASS ×2** (`ARM_OK fps=1248`, capTAP `0xBCF94856` on both passes — the
   pre-registered prediction from G3's byte-identity, upheld); witness `tOff 12314` steady, distinct
   1; `FLASH_DDRCAP2_OK`. The rollback image `638b36de3493` was confirmed present on-board as a
   verified `.bak` in step `[1/5]` before anything was written.
3. **Timeline, 12:41-12:54.** `TIMELINE_OK secs=420` (792 s wall at the ~2 s ssh-bound poll cadence),
   `gaps=0`, pre/post capTAP golden, **BURSTS n=0**, and the total `0x108` delta over the whole
   window = **0** (counter delta in the 120-bit window — see the discipline note above). 792 s is
   **> 6 beat periods** of the 120.2 s recurrence.
4. **sel6 DDRCAP witness, 12:55.** `txfixwit/20260903_125517_F3/cap/F3.bin`, credited
   (`bytes=536870912` **and** pre/post capTAP golden — the byte-count check was added after the
   Task-4 review found the inherited credit rule was capTAP-only): `WITNESS_F3 stalls=0`, Tier-2
   PASS, and the controller's content check — **5,449 frames, every one at offset 0** in the
   injective tap3 map (lookup live on every frame, no `None`), **0 offset transitions**, max
   constant-symbol run **7** (runs > 50: zero), `tOff` distinct 1 (12314), demod period 12,320.
   Same witnesses on the unfixed image the same day: 4-6 stall runs and 4-5 offset transitions per
   512 MB.

**Scope of each witness — do not conflate them.** The timeline covers **792 s** and says: no burst.
The sel6 capture covers 5,449 frames × 802.93 µs ≈ **4.4 s of air time** and says: no stall run, no
displacement, every frame on-offset. The content-level check is a 4.4 s snapshot; the multi-period
coverage comes from the `0x108` timeline alone. Both halves of the pre-registered conjunction were
met, so the PASS stands as pre-registered — but no reader may infer that the content-level check
spans the beat periods.

**The `0x108` liveness caveat [silicon].** *(Corrected 2026-09-03, final review.)* Earlier text in
this section and in `RULINGS.md`/`NEXT_STEPS.md`/`TXFIX_STATE.md` said the counter "read exactly
zero" / "= 0 throughout". That is **wrong about the register value and right about the verdict**.
`two_jup/beattl/20260903_124118/errps.csv` holds the **cumulative value 67 on all 420 rows —
constant, therefore delta 0** over the whole 792 s. The burst verdict is unaffected: `BURSTS n=0`
rests on the *delta*, and the delta is zero. Read every "0x108 = 0" in this campaign as
**"cumulative 67, constant → delta 0"**.

**What the 67 buys us [silicon, weak but real].** `arm148_mode1.sh` writes only `0x10C 0x60003`
(the iq_debug_mux select) and then *reads* `0x104`/`0x108`; nothing in the arm path writes `0x108`
or resets the BIST. **The counter is therefore free-running and is not reset by an arm.** The two
timelines' first rows agree: the baseline run's first sample is already **222,137** at 10:18 after
an arm at 10:16 (and it climbs to 2,087,524 by the end), while F3's first sample is **67** at 12:41
after the 12:37 boot and two gate arms. So on F3 the counter **did count** — 67 counts accumulated
between the 12:37 boot and the 12:41 timeline start, across the post-boot arm transients, and then
stopped. That is **weak but real liveness evidence** that `0x108` is alive on `f6a8c3ea119c`: a
counter stuck dead at reset would read 0, not 67. It is weak because 67 is a small sample taken
during transients, not a controlled injection.

**The 0x108 feed is timing-clean on the flashed image [silicon].** The paths that failed timing in
**F3 attempt 2** (`fill_reg[3]` → `bit_errors`/`frame_errs`, routed WNS **−0.392 ns**) *are* the
`0x108`/`0x10C` feed — the tx_checker instrument counters. That image was never flashed. The
flashed attempt-3 image closes the same paths at **+0.095 ns**, and the unfixed comparison image
(`638b36de3493`) closed them at **+0.105 ns**. The `0x108` feed is therefore timing-clean on *both*
images in the n=7 vs n=0 comparison, so the difference between the two timelines cannot be a
setup-violation artefact on the instrument.

**What remains uncertain.** `beat_timeline.sh` does not populate a packets column on either image,
so a *low-rate* counter fault and a clean link are still not fully distinguished by this instrument
alone, and no known error was ever injected on `f6a8c3ea119c`. **Ruling [12:55] stands: the primary
silicon evidence is the sel6 witness** (a live, content-level check with its own positive control
on the unfixed capture); the `0x108` null is **corroboration**, labelled as such everywhere.

**The frame-identity limit, and what closes it [inferred].** The tap3 map is injective over 12,320
slots, so a displacement of D ≡ 0 (mod 12,320) — a whole-frame stall — decodes as offset 0. "5,449
frames at offset 0" therefore does **not** by itself exclude an integer-frame stall; that is exactly
the F1 residual mode (G8, `pops=24639`) and §90's whole-frame-stall paragraph. What closes it is the
*geometry* half of the same capture: a whole-frame stall is a constant-symbol run of order 12,320
symbols, and the measured maximum run is **7**, with zero runs over 50. Offset identity and run
geometry together exclude both the sub-frame and the whole-frame stall over the captured window; the
offset lookup alone would not.

**Rate arithmetic [RTL/silicon].** Frame time is the **measured 802.93 µs** (board reports
`fps=1247`/`1248` on every ARM_OK). *(Corrected 2026-09-03: an earlier draft wrote "197,328 clk at
245.76 MHz". The 10:38 ledger line refuted the MHz figure — `timing_gate.tcl` constrains the modem
fabric to the **8.000 ns `axi_adrv9001_adc_1_clk`, i.e. 125 MHz** — so the 197,328 "clocks per
frame" are **sim ticks**, not fabric clocks, and no fabric frequency may be inferred from them. All
rate arithmetic below is derived from the measured 802.93 µs and is unaffected.)* §90's body text says 3.21172 ms and every period derived
from it there is 4× too long; the §90 correction of 2026-09-03 09:25 supersedes it. Corrected:
one `frameCount` base increment = 947.69 frames = **0.761 s**, first `frameCount == 0` after arm =
**1.52 s**, full mod-4 return = **3.04 s**. 120.2 s = **149,708 frames** — still 40-80× slower than
any structure found, which is why the trigger remains open.

### 5. Claim boundary
**Entitled [silicon], on image `f6a8c3ea119c` (F3) on board 148, 2026-09-03:** with the same two
witnesses that saw 7 bursts and 4-6 stall runs on the unfixed image on the same day, on the fixed
image there were **no bursts over 792 s (> 6 beat periods)**, **no constant-symbol stall runs and no
offset transition in a credited 512 MB sel6 capture (≈ 4.4 s of air time, 5,449 frames)**, with the
capTAP golden word and the frame rate unchanged (`0xBCF94856`, `fps=1248`). The 120.2 s transmit
stall and its data displacement are absent within that scope.

**Not entitled:**
- Anything about **F1 or F2 on silicon** — neither was ever flashed; F1 was disqualified in sim, F2
  is bank-only by ruling.
- That the **120.2 s trigger is understood**. It is not. The fix removes every abort path found
  (forced clear, natural 3→0 wrap, runaway over-push) *regardless* of what schedules them.
- That the **sim scorer would detect a stall on a fixed tree** — the G6 latch-force control is
  unresolved (a scoring-criterion artefact, sim-only). Detector liveness on the fixed trees is
  inherited from G1 + G3, not directly demonstrated.
- Any statement about **frame error rate, PER or BER** from the `0x108` timeline, on either image
  (120-bit window; see the discipline note).
- That the content-level (sel6) evidence **spans** the beat periods — it is a 4.4 s snapshot.
- That the **fullRAM underflow direction** is characterised (G12; over-push runaway is confirmed,
  the underflow direction is not).
- Any claim about **longer-term stability**: the total observation on the fixed image is under
  15 minutes.

### 6. Open items
1. **The 120.2 s trigger is unidentified.** The only structures found tick at 0.761 s / 1.52 s /
   3.04 s (§90 as corrected); 120.2 s is 40-80× slower. The fix does not depend on it, and the
   question is now only scientific — but it can no longer be studied on 148 without reflashing the
   unfixed image.
2. **G6 control redesign [sim].** A raw latch write produces a word outside the tap3 vocabulary, so
   the strict scorer returns `None` and the control cannot distinguish "clean displacement" from
   "lost demod lock". Redesign: either force through a *natural* RTL transition rather than a
   register poke, or add a byte-for-byte diff of the g6 run against G0's baseline around the force
   point to classify the tail. Until then, fixed-tree detector liveness is inherited, not shown.
3. **`0x108` liveness on the fixed image — partly answered, not closed.** The 67 constant
   cumulative counts on F3 (accumulated post-boot through the two gate arms; the counter is not
   arm-reset) are weak but real liveness evidence, and the counter's feed paths are timing-clean at
   +0.095 ns on the flashed image. What is still missing is a *controlled* demonstration: populate
   the timeline's packets column, or inject a known error and see the counter move. Until then the
   timeline's `n=0` stays corroboration.
4. **Longer soak.** Under 15 minutes of observation total. See `two_jup/NEXT_STEPS.md` for a
   pre-registration sketch.
5. **The fullRAM underflow direction is unresolved (G12).** Needs a denser (per-clock, not
   change-detection) sample around the drain event on an untrimmed-NF re-run before the
   underflow-toward-zero mechanism may be cited as a measured defect on F1/F2.
6. **F2 bitstream** not yet banked at write-up time (build running); bank-only by ruling.
7. **The task5 heartbeat gaps were caught, not avoided.** Two heartbeat stalls were logged in the
   ledger — `STALL agent=task5 age=27.8` at 11:11:48 and `age=26.8` at 11:31:49. The stall detector
   worked as designed; the *cadence discipline* did not. Credit the detector, not the process: the
   agent twice went ~27 min without a heartbeat against a ≤ 15 min instruction, and only the timer
   surfaced it.
8. **The stall-detector timer must be disabled once the campaign closes.** It is a campaign-scoped
   watchdog, not a standing service, and left running it will fire on unrelated work. **Not disabled
   by this documentation round** — a rig unit was still appending to the ledger at write-up, so
   disabling it was deliberately left to whoever closes the campaign.

### 7. Facts a future session must know (memory line)
Image md5s: **`f6a8c3ea119c`** = TXFIX **F3**, VERIFIED and **currently flashed on 148**
(`boot_known_good/BOOT.BIN.148.txfixF3.f6a8c3ea119c`); **`638b36de3493`** = DDRCAP-v2, the
**rollback bank** (unfixed — the only image on which the beat can still be studied);
**`873902b705fb`** = F3 attempt 2, routed WNS −0.392 ns, **never flashed**, kept on hdl-dev-2 as
`BOOT.BIN.attempt2.wnsfail`; **`1cd0cd752aa6`** = txmark, second-level bank; F2 md5 TBD (bank-only).
Injector: `two_jup/skidfix/txfix_inject.py <dir> F1|F2|F3`, cumulative, idempotent
`TXFIX_F1/F2/F3` markers, asserts each anchor is unique and refuses otherwise; tests
`two_jup/tests/test_txfix_inject.py`. These are **netlist** patches — a Simulink regeneration
destroys them. Build gate: `txfix_build.tcl` refuses bootgen on a **routed**-WNS fail
(`TXFIX_ROUTED_TIMING_FAIL`); post-synthesis timing is not sufficient, and the marginal paths live
in the **tx_checker instrument counters**, so a fix build may need `IMPL_STRATEGY=explore` to close
(+0.095 ns vs −0.392 ns on the identical netlist). Frame time is the **measured 802.93 µs**
(1247-1248 f/s) — any 3.21 ms figure in §90's body or `TX_ORIGIN_TRACE_A.md` is 4× wrong. **Do not
quote "245.76 MHz":** the modem fabric clock is the **8.000 ns `axi_adrv9001_adc_1_clk` (125 MHz)**
per `timing_gate.tcl` (ledger 10:38); the 197,328 "clocks per frame" are **sim ticks**.
`0x108` is a 120-bit-window bit-error counter, never a frame error rate; it is **free-running and
not reset by an arm** (`arm148_mode1.sh` writes only `0x10C`), it read a **constant cumulative 67 →
delta 0** across F3's 792 s timeline (not "zero"), and its liveness on the fixed image rests on
those 67 post-boot counts — weak but real, never a controlled injection. **Stall-detector lesson:** a Verilator/harness log that is silent is
not a stalled job — these harnesses `fprintf` only at exit, so log mtime is not a liveness signal;
register the growing `.bin` (or raise `max_age`) and confirm process liveness by CPU. **Scoring
lesson from the same buffering:** never score a run's output files until a completion trailer
(`TXFIX_GATE_RUN_EXIT=<code>`, written after the binary returns) is present — files that look
complete may still be growing.
