#!/bin/bash
# deploy_ch2.sh -- flash ONE board with the CH2+TAP BOOT.BIN (modem on RX2/TX2 +
# iq_debug_mux 0x10C + state-pair regs 0x160-0x16C), reboot, wait.
# SAFETY (deploy_rxfix.sh skeleton): new BOOT.BIN size-check >6MB; backup current
# /boot/BOOT.BIN -> /boot/BOOT.BIN.prech2 (size-checked, plus free-space check);
...
BOOT=${BOOT:-$(cd "$D/.." && pwd)/jupiter_byte_ch2tap_build/hdl_prj_jupiter_composite/vivado_ip_prj/boot/BOOT.BIN}
IP=${1:-10.0.0.148}
...
echo "  backup: $($W $IP 'CB=$(stat -c %s /boot/BOOT.BIN 2>/dev/null||echo 0); if [ "$CB" -gt 6000000 ]; then cp -f /boot/BOOT.BIN /boot/BOOT.BIN.prech2 && sync && echo "OK current=$CB -> /boot/BOOT.BIN.prech2"; else echo "REFUSE current size=$CB"; fi' 2>/dev/null)"
$W $IP 'test -f /boot/BOOT.BIN.prech2' 2>/dev/null || { echo "FATAL: backup not present -- aborting before overwrite"; exit 1; }
# push new image to /root, verify size, THEN overwrite /boot
scpput "$BOOT" root@$IP:/root/BOOT.BIN.ch2
FL=$($W $IP 'NB=$(stat -c %s /root/BOOT.BIN.ch2 2>/dev/null||echo 0); if [ "$NB" -gt 6000000 ]; then cp -f /root/BOOT.BIN.ch2 /boot/BOOT.BIN && sync && echo "FLASHED size=$NB md5=$(md5sum /boot/BOOT.BIN|cut -c1-12)"; else echo "ABORT staged size=$NB"; fi' 2>/dev/null)
echo "  flash: $FL"
