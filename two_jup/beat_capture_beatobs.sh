...
B=10.0.0.148
LTX=$R/jupiter_byte_beatobs_build/hdl_prj_jupiter_composite/vivado_ip_prj/vivado_prj.runs/impl_1/debug_nets.ltx
...
for k in 1 2 3; do
  pkill -f "[h]w_server" 2>/dev/null; pkill -f "[c]s_server" 2>/dev/null; sleep 1
  pgrep -f "[h]w_server|[c]s_server" >/dev/null 2>&1 || break
  pkill -9 -f "[h]w_server" 2>/dev/null; pkill -9 -f "[c]s_server" 2>/dev/null; sleep 1
done
pgrep -f "[h]w_server|[c]s_server" >/dev/null 2>&1 && { echo "BEATCAP_FATAL: stale hw/cs_server unkillable"; exit 1; }
echo "  servers verified clean"
rm -f "$OUT/armed_1" "$OUT/armed_2"
( cd "$OUT" && timeout 900 vivado -mode batch -notrace -source "$D/beat_capture_obs.tcl" \
...
$W $B 'DM=$(command -v devmem || echo "busybox devmem")
  ...
  echo "0x10C 0x${MUX:-2}">$DRA   # iq_debug_mux tap
  ...
  waituntil $(awk -v t=$T0 "BEGIN{print t+153.10}")
  echo "0x10C 0x2">$DRA; sleep 0.2
  CAP1=$(echo 0x13C>$DRA; cat $DRA); $DM 0x9D430000 32 3; echo "FORCE1(gap mux2fresh) $(awk "{print \$1}" /proc/uptime) cap_in=$CAP1 status=$($DM 0x9D430008)"
  sleep 1; $DM 0x9D430000 32 0
  waituntil $(awk -v t=$T0 "BEGIN{print t+154.30}")
  echo "0x10C 0x2">$DRA; sleep 0.2
  CAP2=$(echo 0x13C>$DRA; cat $DRA); $DM 0x9D430000 32 3; echo "FORCE2(inwin mux2fresh) $(awk "{print \$1}" /proc/uptime) cap_in=$CAP2 status=$($DM 0x9D430008)"
  sleep 1; $DM 0x9D430000 32 0
