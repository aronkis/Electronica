...
W=$D/anyssh.sh
A=10.0.0.148
BB=${BB:-$ROOT/jupiter_byte_rxfifo4k_build/hdl_prj_jupiter_composite/vivado_ip_prj/boot/BOOT.BIN}
EXP=${1:?usage: flash_148_rxfifo.sh <md5-12>}
BAK_MD5=${BAK_MD5:-fe5bd8a4fe19}   # restore point = the image the board runs now (override for non-comb bases)
echo "FLASH_RXFIFO_DIAG start $(date -Is) pid=$$"
...
rollback(){
  ...
  wait_back || { echo "FLASH_RXFIFO_DIAG_FATAL: 148 not back after rollback -- PHYSICAL ATTENTION (backup at /root/BOOT.BIN.$BAK_MD5.bak)"; exit 2; }
  ...
  echo "FLASH_RXFIFO_DIAG_ROLLED_BACK $(date -Is)"
  exit 1
}
...
{ [ -n "$FS0" ] && [ "$FS0" -ge 1100 ] && [ -n "$WC0" ] && [ "$WC0" -ge 1100 ]; } || { echo "FLASH_RXFIFO_DIAG_FATAL: 148 not healthy pre-flash (fsync=${FS0:-?} wcnt=${WC0:-?}) -- fix the instrument first, nothing flashed"; exit 1; }
...
[ -f "$BB" ] || { echo "FLASH_RXFIFO_DIAG_FATAL: no image at $BB"; exit 1; }
md5sum "$BB" | grep -q "^${EXP}" || { echo "FLASH_RXFIFO_DIAG_FATAL: image md5 != ${EXP}"; exit 1; }
CUR=$($W $A 'md5sum /boot/BOOT.BIN | cut -c1-12' 2>/dev/null)
[ "$CUR" = "$BAK_MD5" ] || { echo "FLASH_RXFIFO_DIAG_FATAL: 148 runs $CUR, expected $BAK_MD5 -- restore point assumption broken, nothing flashed"; exit 1; }
...
$W $A "cp /boot/BOOT.BIN /root/BOOT.BIN.$BAK_MD5.bak && sync && md5sum /root/BOOT.BIN.$BAK_MD5.bak | cut -c1-12" 2>/dev/null | grep -q "^$BAK_MD5" \
  || { echo "FLASH_RXFIFO_DIAG_FATAL: could not bank the restore point on 148"; exit 1; }; fi
...
scpput "$BB" root@$A:/boot/BOOT.BIN.new || { echo "FLASH_RXFIFO_DIAG_FATAL: scp failed"; exit 1; }
$W $A "md5sum /boot/BOOT.BIN.new | grep -q ^${EXP} || exit 1
  ...
  || { echo "FLASH_RXFIFO_DIAG_FATAL: stage verify failed -- /boot/BOOT.BIN untouched"; exit 1; }
echo "  staged; rebooting 148"
$W $A 'sync; ( sleep 2; reboot ) >/dev/null 2>&1 & exit 0' 2>/dev/null
wait_back || { echo "FLASH_RXFIFO_DIAG_FATAL: 148 not back after 10 min"; exit 2; }
...
[ -n "$NAK" ] && [ "$NAK" = "${FP0:-x}" ] || { echo "FLASH_RXFIFO_DIAG_FINGERPRINT_FAIL"; rollback; }
...
if [ $(( (w1-w0)&0xFFFFFFFF )) -gt 1000 ]; then
  ...
  echo "FLASH_RXFIFO_DIAG_NODELIVERY"; rollback
fi
fi
...
[ -n "$gate_ok" ] || { echo "FLASH_RXFIFO_DIAG_HEALTH_FAIL fsync=${FS:-?} wcnt=${WC:-?} after re-bring-up"; rollback; }
stamp "gate done"; echo "=== [6/6] fixctl=3 arm (as on the comb image) + ByteRxFifo overflow witness 0x1B0 over 10 s (non-fatal) ==="
...
if [ "$TIER" = quick ]; then echo "  (quick tier: 0x1B0 witness skipped)"; else O0=$(( $(read1b0) )); sleep 10; O1=$(( $(read1b0) )); echo "  OVF_WITNESS 0x1B0 delta over 10 s = $(( (O1-O0) & 0xFFFFFFFF )) dropped words (comb image: 170-450)"
fi
stamp "done"; echo "FLASH_RXFIFO_DIAG_DONE $(date -Is) image=${EXP} on 148, bring-up + gates green (tier=$TIER)"
