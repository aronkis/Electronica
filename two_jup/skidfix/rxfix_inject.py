#!/usr/bin/env python3
"""rxfix_inject.py <netlist_dir> R1|R2|R3|R3S|R4|R4B|R4D|R4E|W1 [--sim-tree] -- RX symbol-rate-offset fix injector
...
#     shift sidx by 4 input samples each.
#
# (6) WITNESSES.  `{r4d_locked, r4d_skips[15:0], r4d_window_opens[14:0]}` is assembled
#     in Rate_Handle as `r4dWit` and, WHEN W1 IS PRESENT, is carried to a NINTH W1 read
#     word at byte 0x234 (word 0x8D).  DEVIATION FROM THE BRIEF, STATED: the brief asks
#     for {prefilled, armed, skips[15:0], window_opens[15:0]} = 34 bits, which does not
...
R4D_WIT_ASSIGN = ("\n  // RXFIX_R4D: the silicon witness, TWO 32-bit read words (0x234, 0x238).\n"
                  "  //   word 0 = {locked, skips[15:0], window_opens[14:0]}   (R4B's layout)\n"
                  "  //   word 1 = {16'b0, extras[15:0]}                       (the FULL side)\n"
                  "  // See two_jup/rxfix/W1_REGMAP.md.  Present only when W1 is, because W1\n"
                  "  // owns the read path that carries it out.\n"
...
def patch_rate_handle_r4d(path, sim_tree=False):
    ...
    decl = ("  output  validOut;\n"
            ...
            "  input   pcEndIn;\n")
    if wit:
        decl += ("  // RXFIX_R4D: {locked, skips[15:0], window_opens[14:0]} for the ninth W1\n"
                 "  // read word at 0x234.\n"
                 "  output  [63:0] r4dWit;\n")
    decl += ("\n"
...
def _r4d_pass_through(module, inst_module, inst, what_prefix, anchor=None):
    """Add a 32-bit r4dWit output and pin it from `inst`.  W1-CONDITIONAL: if this file
    does not already carry RXFIX_W1 there is no read path for the word, so the witness
    stays internal and the file is left untouched ('skipped')."""
    def _fn(path, sim_tree=False):
        s = open(path).read()
        ...
        a = W1_DECL_ANCHOR if anchor is None else anchor
        s = _sub(s, a, a + "  // RXFIX_R4D: witness word, pass-through to the ninth W1\n"
                          "  // read word at 0x234.\n"
                          "  output  [63:0] r4dWit;\n", what_prefix + ' declarations')
...
def patch_ip_addr_decoder_r4d(path, sim_tree=False):
    ...
    s = _sub(s, W1_DATA_READ_LINE,
             ...
             "  assign data_read = (r4d_hit ? r4d_reg[address_select_level1[0]] :\n"
...
R4E_WIT_ASSIGN = ("\n  // RXFIX_R4E: the silicon witness, TWO 32-bit read words (0x234, 0x238).\n"
                  "  //   0x234 = {locked, skips[15:0], window_opens[14:0]}   (R4B's layout)\n"
                  "  //   0x238 = {land_out[3:0], land_max[3:0], land_min[3:0],\n"
                  "  //            land_last[3:0], drops[15:0]}                (the FULL side)\n"
                  ...
                  "  // See two_jup/rxfix/W1_REGMAP.md section 6.  Present only when W1 is,\n"
...
def patch_ip_addr_decoder_r4e(path, sim_tree=False):
    ...
    s = _sub(s, W1_DATA_READ_LINE,
             ...
             "              (address_select_level1 == 8'h8E);\n\n"
             "  assign data_read = (r4e_hit ? r4e_reg[address_select_level1[0]] :\n"
...
def main(d, variant, sim_tree=False):
    if variant not in VARIANT_FILES:
        print(f"RXFIX_INJECT_FAIL unknown variant {variant!r} (want R1|R2|R3|R3S|R4|R4B|R4D|R4E|W1)")
        return 2
...
if __name__ == '__main__':
    ...
    if len(args) != 2:
        print("usage: rxfix_inject.py <netlist_dir> R1|R2|R3|R3S|R4|R4B|R4D|R4E|W1 [--sim-tree]")
        sys.exit(2)
