...
FRESH=$ROOT/jupiter_byte_skid4_build
...
echo "SKID4_BUILD start $(date -Is) host=$(hostname) pid=$$ ppid=$PPID"
...
rsync -a --exclude 'hdl_prj_*' --exclude 'slprj' --exclude '*.log' --exclude '*.out' \
  --exclude 'rtl_sim/obj_*' --exclude 's1_rtl' "$BASE/" "$FRESH/" || { echo SKID4_BUILD_FAILED rsync; exit 1; }
for f in "$FRESH"/*.m; do sed -i "s#$BASE#$FRESH#g" "$f"; done
sed "s/qpsk_axis_skid_v3/qpsk_axis_skid/g" "$SRC/qpsk_axis_skid_v3.v" > "$FRESH/qpsk_axis_skid.v" || { echo SKID4_BUILD_FAILED cp; exit 1; }

echo "=== [2/6] patch complete_byte_t8.tcl (skid splice) ==="
python3 "$SRC/patch_complete_tcl.py" "$FRESH/complete_byte_t8.tcl" || { echo SKID4_BUILD_FAILED patch; exit 1; }
python3 "$SRC/patch_gcc.py" "$FRESH/complete_byte_t8.tcl" || { echo SKID4_BUILD_FAILED gcc-patch; exit 1; }
grep -q 'SKID_WIRE_OK' "$FRESH/complete_byte_t8.tcl" || { echo SKID4_BUILD_FAILED patch-verify; exit 1; }
...
[ -f "$Z/vivado_prj.xpr" ] || { echo "SKID4_BUILD_FAILED: no vivado project"; tail -20 "$FRESH/build_byte_matlab.log"; exit 1; }
[ -f "$Z/ipcore/TxRxCompo_ip_v1_0.zip" ] || { echo "SKID4_BUILD_FAILED: no ipcore zip"; exit 1; }
grep -q 'byte_breakout' "$Z/vivado_prj.srcs/sources_1/bd/system/system.bd" 2>/dev/null \
  || { echo "SKID4_BUILD_FAILED: BD has no byte_breakout"; exit 1; }
...
grep -q 'SKID_WIRE_OK' "$FRESH/build_byte_vivado.log" || { echo "SKID4_BUILD_FAILED: splice never confirmed"; exit 1; }
B=$Z/boot/BOOT.BIN
[ -f "$B" ] || { echo "SKID4_BUILD_FAILED: no BOOT.BIN"; exit 1; }
MD5=$(md5sum "$B" | cut -c1-12)
echo "SKID4_IMAGE_MD5 $MD5"
...
echo "SKID4_BUILD_DONE md5=$MD5 image=$B $(date -Is)"
