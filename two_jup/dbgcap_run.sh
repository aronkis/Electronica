...
OUT=$D/dbgcap/$(date +%Y%m%d_%H%M%S)
mkdir -p "$OUT"
...
$W $B "DRA=/sys/kernel/debug/iio/iio:device0/direct_reg_access
 ...
 for i in \$(seq 1 $DWELL); do
   ...
   sleep 1
   p1=\$((\$(rd 0x104))); e1=\$((\$(rd 0x108)))
   echo \"\$i \$(( (p1-p0)&0xFFFFFFFF )) \$(( (e1-e0)&0xFFFFFFFF )) \$TXM\$M \$CI \$CD \$CO\" >> /dev/shm/dbgcap.csv
 done
 echo POLL_DONE" 2>/dev/null | tee -a "$OUT/run.log"
$W $B 'cat /dev/shm/dbgcap.csv' 2>/dev/null > "$OUT/dbgcap.csv"
log "  CSV $OUT/dbgcap.csv ($(wc -l < "$OUT/dbgcap.csv") samples)"
