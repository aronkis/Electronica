...
SZ=${SZ:-134217728}; GOLD=${GOLD:-BCF94856}; OUT=${OUT:-$D/ddrcap2_pc/$(date +%Y%m%d_%H%M%S)}; mkdir -p "$OUT"
...
C0=$(rd 0x20C); if [ "${SKIP_GOLD:-0}" = 1 ]; then log "NOTE: SKIP_GOLD=1 pre capTAP $C0 (credit decided by bytes + positive control)"; else [ "$(norm "$C0")" = "$GOLD" ] || { log "ABORT: capTAP $C0 != golden"; exit 4; }; fi
